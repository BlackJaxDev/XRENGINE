...
The atlas solve path classifies submitted requests once into light-family/encoding buckets, then solves each bucket with a fixed-level buddy allocator. Prior resident slots are used as placement tie-breakers and are reserved directly through the buddy tree; down-sized tiles keep an aligned sub-rect of their old slot when possible, and failed in-place upgrades keep the previous tile for another frame instead of migrating. A bounded resident table keeps vanished requests for a short TTL so transient relevance changes do not immediately lose their prior slot. Stale `NotRelevant` allocations are reserved only after live allocations claim their regions; if a live tile needs the old space, the stale tile publishes a non-resident fallback instead of overlapping. Forced demotion is relevance/priority driven with a sticky cooldown so point faces and equal-priority locals do not rotate victims under steady contention.

`ShadowAtlasManager.SolveAllocations` also publishes per-frame solve diagnostics through `ShadowAtlasFrameData`, render stats packets, and the ImGui profiler. The counters split solve time from shadow rendering time: classified request counts, balanced-solve attempts, failed candidates, demotions, prior-slot reuse, page allocation/clear activity, and directional/point group publishing work are reported together. Slow-solve warnings are controlled by `XRE_SHADOW_ATLAS_SOLVE_WARN_MS` and should include the same counters. Keep new allocator changes deterministic and bounded: over-budget solves must demote in stable batches, hit a visible attempt ceiling before deterministic fallback demotion, and avoid reintroducing one-failed-candidate-per-full-restart behavior.
