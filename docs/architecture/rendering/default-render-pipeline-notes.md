...
Directional lights expose `CascadeShadowRenderMode` for the legacy cascade texture-array path: `Sequential`, `InstancedLayered`, `GeometryShader`, and `Auto`. `Sequential` remains the compatibility/default path and renders one cascade layer at a time. `GeometryShader` renders all active cascades through the layered framebuffer using `DirectionalCascadeShadowDepth.gs`. `InstancedLayered` renders one layered pass on OpenGL drivers with vertex-stage `gl_Layer` support; meshes that already use draw instancing or mesh-deform state fall back to the geometry-shader shadow variant inside that layered pass so their instance semantics are not stolen by the cascade dimension. `Auto` selects instanced layered first, then geometry shader, then sequential when capabilities are missing. `EffectiveCascadeShadowRenderMode` and `CascadeShadowRenderFallbackReason` expose the most recent decision. Directional atlas tiles currently render independently; grouped atlas cascade parity is tracked in `docs/work/todo/directional-cascade-layered-rendering-todo.md` and must fall back to sequential tile rendering whenever grouped allocation or viewport/scissor-array support is unavailable.

Point lights expose `ShadowRenderMode` for the legacy cubemap shadow path: `Sequential`, `InstancedLayered`, and `GeometryShader`. `Sequential` renders one cubemap face at a time. `GeometryShader` renders all six faces through `PointLightShadowDepth.gs`. `InstancedLayered` renders all six faces in one layered pass on OpenGL drivers with vertex-stage `gl_Layer` support; meshes that already use draw instancing or mesh-deform state fall back to the geometry-shader point-shadow variant inside that layered pass. `EffectiveShadowRenderMode` and `ShadowRenderFallbackReason` expose the most recent decision. Point atlas mode remains direct-to-atlas sequential face-tile rendering; grouped atlas point fan-out is tracked separately from the legacy cubemap mode.

Dynamic shadow atlas page ownership is light-family scoped. Each live atlas family/encoding owns one sampling `XRTexture2DArray` and one raster-depth `XRTexture2DArray`; `PageIndex` is the array layer. Page index alone is not globally unique, so consumers must use `AtlasKind` + encoding + page index, or the packed `AtlasId`. `MaxShadowAtlasPages` is honored per family/encoding, with a runtime default of `1`; memory-budget checks account for the full texture-array allocation when the first page is created. When `UseDirectionalShadowAtlas` is true, directional atlas mode is authoritative: forward and deferred passes bind dummy legacy directional maps and do not fall back to `ShadowMap` / `ShadowMapArray` just because a cascade missed the atlas. The allocator must instead reduce directional cascade tile resolutions until every required active cascade for a light is resident in the directional atlas. Receiver binding enables the directional atlas for a light only when its required active directional tiles are sampleable; each slot still publishes fallback metadata so skipped or stale tiles have defined behavior.

Spot atlas pages are currently depth-only. A spot light whose resolved `ShadowMapEncoding` is VSM or EVSM bypasses the spot atlas even when `UseSpotShadowAtlas` is enabled, renders the standalone moment shadow map, regenerates its mip chain when `ShadowMomentUseMipmaps` is enabled, and samples those mips with explicit LOD instead of main-camera screen-space derivatives. Moment atlas pages remain tracked by the VSM/EVSM shadow-filtering plan.

When `UsePointShadowAtlas` is true, dynamic point lights render six independent `PointFace` requests directly into the point atlas as 2D face tiles. Once the render scheduler starts refreshing one face for a point light, adjacent dirty faces for that same light are allowed to continue past the soft per-frame budget, matching directional cascade set behavior and preventing interactive light transforms from updating one cube face at a time. Forward and deferred receivers select the face by major axis, convert the receiver direction to face-local UV, and compare radial normalized depth against that face's tile metadata. Missing or demoted faces use their published fallback, usually contact-only during Phase 5 bring-up, instead of sampling undefined atlas texels. The legacy point cubemap path remains available when point atlas mode is disabled.
