...
The generator strips previously recognized Uber feature defines, resolves known feature/pipeline conditional blocks, and emits only residual guard or pipeline defines that still survive pruning. `Static` properties are now inlined as token-aware GLSL literals at their use sites after the corresponding uniform declaration is removed. `Animated` properties remain uniforms and continue to update through the regular material parameter path. The request key includes enabled features, animated/static property membership, static literals, render pass, vertex-permutation hash, pipeline macros, and resolved source version.
...
- Generated fragment variants carry a structured `XRShader.IsGeneratedUberVariant` flag for live runtime checks, keep a bounded-header legacy recovery marker for serialized generated fragments, embed a stable `variant-hash` comment, and the OpenGL backend uses that hash to report real backend compile-stage, link-stage, compile milliseconds, link milliseconds, and failure text back into the inspector.
...
- inlining static properties into generated GLSL without rewriting comments, directives, longer identifiers, struct fields, or field accesses
...
- excluding samplers owned by disabled features from `SamplerCount` and pruning the corresponding guarded source when the feature is disabled
...
- caching resolved canonical source across equivalent variant requests, including dependency-aware invalidation for included files and coalescing concurrent misses
...
These axes are not implicit side effects of shader-text state. `UberShaderVariantBuilder.BuildRequest(...)` serializes each axis into the hash, and the generated fragment variant embeds `// variant-hash: 0x...` so backend telemetry can correlate driver compile/link events with the exact authored state. `VariantHash`, `SourceVersion`, and `VertexPermutationHash` use the xxHash64 schema introduced with the variant-builder optimization work; legacy FNV values are intentionally not stable across this boundary. Any new axis must be added to both `UberMaterialVariantRequest` and `ComputeVariantHash(...)` in the same change to keep cache keys unambiguous.

## Program Cache Layering

Program caching is split into two explicit layers that share the variant hash as their join key:

1. Generated fragment shaders are cached inside `UberShaderVariantBuilder` keyed by `(VariantHash, SourceVersion, SourcePathHash, SourcePath)`. Identical authored state reuses the same `XRShader` instance without regenerating source.
