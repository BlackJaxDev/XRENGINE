...
- `FullBucketScan` preserves the original per-material/per-atlas-tier bucket loop. `ActiveBucketList` adds a GPU compaction pass and reads back only active bucket IDs for profiling the empty-bucket overhead. `MaterialTable` and `BindlessMaterialTable` use the active bucket list with a shared material-table shader. OpenGL bindless is gated on `GL_ARB_bindless_texture` and `GL_ARB_gpu_shader_int64`; Vulkan bindless is gated on descriptor indexing, runtime descriptor arrays, partially-bound descriptors, update-after-bind, and the global material texture descriptor table.
- See [Mesh Submission Strategies](mesh-submission-strategies.md) for the strategy table and pass contract.
...
- `DefaultRenderPipeline` and `DefaultRenderPipeline2` expose schema-backed camera post-processing stages. In the ImGui camera inspector those stages are selected from an effect dropdown (for example tonemapping, bloom, vignette, and motion blur), while the schema categories still define their ordering and grouping metadata. The bloom stage now includes an explicit `Enabled` toggle that hides the dependent bloom controls and skips the bloom pass when disabled. The `Imaging` category covers tonemapping, color grading, and vignette, and the hue/saturation/brightness grading path runs before final HDR or SDR output so the same controls work with HDR output enabled.
