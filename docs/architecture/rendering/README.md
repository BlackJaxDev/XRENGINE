...
| [CPU Scene BVH](cpu-scene-bvh.md) | Production CPU-direct scene index: flat snapshot publication, sparse refit, deterministic SAH construction, lock-free traversal, quality maintenance, and diagnostics. |
| [GPU Scene BVH](gpu-scene-bvh.md) | GPU-resident command TLAS: revision-aware lifecycle, normalization, stable radix construction, compact nodes, refit, root-down traversal, and fallback behavior. |
...
| [Retinal Visibility Cache Rendering](retinal-visibility-cache-rendering.md) | RVC architecture and validation entry point for OpenXR quad-view/foveated VR rendering: view-set contracts, quad-view plumbing, selectable RVC pipeline, visible fallback, frame-graph resources, pass stages, renderer capability hooks, and validation gates. Design source: [proposal](../../work/design/rendering/retinal-visibility-cache-rendering-design.md). |
