...
## Tick Scheduling
- All world objects, including components, inherit `RegisterTick` / `UnregisterTick` from `RuntimeWorldObjectBase`. Components typically call `RegisterTick(ETickGroup, ETickOrder, WorldTick)` inside `OnComponentActivated` and rely on the default `UnregisterTicksOnStop=true` to remove them when deactivated.
...
- Treat `OnTransformRenderWorldMatrixChanged` (documented in [Transform Architecture](../../architecture/scene/transforms.md)) as the preferred hook for responding to transform motion; it provides the final render matrix after interpolation.
...
- In the default OpenGL path, fog is raymarched at half internal resolution, temporally reprojected in a half-res history buffer, then bilateral-upsampled to the full-res `VolumetricFogColor` composite texture. The upscale pass re-tests the full-res view ray against the active fog boxes so half-res filtering does not bleed fog outside the authored bounds. Default tuning is `StepSize = 1.0` and `JitterStrength = 0.5`; the temporal pass turns the moving per-pixel jitter into stable shafts rather than full-screen speckle.
- Use the `MarchDistance` debug mode when diagnosing unexpectedly clear regions; it visualizes the depth/volume-clipped distance marched through fog before density and lighting are applied.
- Edge fade applies only at actual volume box entry and exit faces. Scene depth and `MaxDistance` still clip the march, but they are not treated as fog-volume boundaries.
...
- Per-volume controls include scattering color, density, local-space edge fade distance, noise tiling/threshold, scroll velocity, anisotropy, and light contribution from the primary directional light. When `NoiseAmount` is above zero, the same volume noise also erodes the edge fade band so bounds feather organically instead of reading as a linear box ramp.
- When the primary directional light has shadows enabled, volumetric scattering respects the same cascaded shadow data. Shadowed regions still receive a small ambient fill from `GlobalAmbient`, so they remain visible as fog instead of becoming only a scene-darkening extinction layer.
- For the unit-testing world, set `InitializeVolumetricFog` in `Assets/UnitTestingWorldSettings.jsonc` to create a demo fog volume and enable the volumetric fog post-process stage on the spawned camera automatically. The generated preset keeps `EdgeFade = 0.0` so the floor near world origin is not inside the box-face fade band by default.
