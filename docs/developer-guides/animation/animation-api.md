...
- Imported Unity humanoid clips execute through an immutable compiled avatar definition and a preallocated workspace owned by each `HumanoidComponent`. The compiler captures the finalized neutral pose, semantic joint bases, pre/post rotations, asymmetric limits, body axes, human scale, translation-DoF roles, and twist-helper topology. Each frame evaluates all normalized muscles and translation DoF in deterministic semantic order, performs scratch forward kinematics through concrete helper chains, validates the complete result, and commits it atomically with quaternion sign continuity. Legacy sampled response profiles and calibration metadata are accepted only as migration input and do not participate in compilation, signatures, or evaluation.
- Imported clips expose `HumanoidProjectedRootPose` through `HumanoidComponent.CurrentProjectedRootPose` and a consecutive `HumanoidRootMotionDelta` through `CurrentRootMotionDelta`. Body position/orientation, Hips allocation, projected XZ/yaw, and feet-based Y come from the same compiled avatar solve; no clip display name or target-specific fitted coefficient is consulted. Position uses XRENGINE handedness and engine length units in the canonical Body-yaw frame. `AnimationClipComponent` separately tracks the within-cycle pose, signed unwrapped loop pose, playback-epoch anchor, and temporal delta. `EHumanoidRootMotionApplicationMode` selects extract-only output, explicit named-target application, or external-consumer publication; no hierarchy target is inferred.
- Direct clips and state-machine leaves, transitions, and blend trees finalize their muscle/translation-DoF blend before invoking that shared native solver once. Evaluation order is authored pose, Body/root allocation and projection, then one animation-driven IK/contact pass. Animation-authored IK channels are staged and validated with the pose transaction; a non-finite position, zero/non-finite rotation, out-of-range weight, or rejected pose leaves the prior pose/root/IK frame intact. Authored channels are routed only to an existing `HumanoidIKSolverComponent`, preventing a duplicate solver from being inferred. `EHumanoidIKGoalPolicy` gates ignored, compiled-basis-ready, and unconditional application. Goal diagnostics retain authored body-space, final world-space, contact offset, stretch, feet-spacing compensation, and application status independently from Body/root projection. `EHumanoidContactCompensationMode` optionally constrains feet or feet-and-hands against an explicit world plane after authored conversion.
