...
- **AnimationClip** (`Property/Core/AnimationClip.cs`) owns a tree of `AnimationMember` nodes. When started, it registers all animations, tracks how many sub-animations are active, and loops or stops once they have finished. During evaluation it computes weighted values per member (default-to-animated interpolation) and populates the parent motion’s value dictionary. The clip can import MikuMikuDance VMD files, generating property animations and IK bindings directly from third-party data.
  - In the ImGui editor, AnimationClip assets now have a dedicated Animation Clip Editor panel with a member-tree sidebar, a lane timeline that shows keyframe rows for visible members, and a curve view that plots zoomable interpolation graphs for the selected member. Clips can be opened from the asset inspector, the View menu, or the asset browser context menu.
- **Blend Trees**:
...
- **AnimationClipComponent** (`Scene/Components/Animation/AnimationClipComponent.cs`) provides a lightweight way to play a single clip on activation. It starts the clip, registers a tick, and forwards stop events when the component is disabled. It also exposes exact-time evaluation for diagnostic tooling so clips can be sampled deterministically without waiting for frame ticks. The ImGui component inspector now includes a dedicated playback section with clip status, jump controls, and a scrub slider for exact-time sampling during editor inspection.
...
- Automatic mapping prefers a complete hips/torso/left-leg/right-leg hierarchy backed by bones used by skinned meshes. Multiple candidate rigs are ranked by distinct skinned-bone coverage; body-height measurements stay within the selected rig so mesh/helper transforms cannot distort limb scores. Side aliases support namespaced long names and short `_L`/`_R` forms. The existing confidence checks still apply, and independent clothing rigs are not automatically merged or retargeted.
...
- Imported Unity humanoid clips execute through an immutable compiled avatar definition and a preallocated workspace owned by each `HumanoidComponent`. The compiler captures finalized neutral pose, semantic joint bases, pre/post rotations, asymmetric limits, body axes, explicit `BodyDefinition` data, human scale, translation-DoF roles, and twist-helper topology. `BodyDefinition` persists a model ID, algorithm version, weighted skeletal segments, and hip/shoulder landmarks. Definitions that predate this additive data must be refreshed and reviewed/reconfirmed before playback; the runtime never invents missing Body metadata. `XRE.PublicHumanoidMassHierarchy.v1` is the ratified public-hierarchy model for the focused Unity 2022.3 gate. Canonical-pose schema 6 likewise records `XRE.MecanimCanonicalPose.2022.3.v2`; missing or mismatched authoring markers force correction regeneration and review. Legacy sampled response profiles and calibration metadata are migration input only and do not participate in compilation, signatures, or evaluation.
- Imported clips expose `HumanoidProjectedRootPose` through `HumanoidComponent.CurrentProjectedRootPose` and a consecutive `HumanoidRootMotionDelta` through `CurrentRootMotionDelta`. Body position/orientation, Hips allocation, projected XZ/yaw, and feet-based Y come from the same compiled avatar solve; no clip display name or target-specific fitted coefficient is consulted. Muscle-dependent Hips recenter/reorientation keeps the requested Body frame in component/model-root space and never becomes scene locomotion or a new root sample. Position uses XRENGINE handedness and engine length units in the canonical Body-yaw frame. `AnimationClipComponent` separately tracks the within-cycle pose, signed unwrapped loop pose, playback-epoch anchor, and temporal delta. `EHumanoidRootMotionApplicationMode` selects extract-only output, explicit named-target application, or external-consumer publication; no hierarchy target is inferred.
- Direct clips and state-machine leaves, transitions, and blend trees finalize their muscle/translation-DoF blend before invoking the shared native solver once. Accepted-frame order is final blend, FK, Body alignment/projection, pose/root commit, then one animation-driven IK/contact pass. IK consumes the committed pre-IK Body frame, not Hips or post-IK bones. Animation-authored IK channels are staged and validated with the pose transaction; a non-finite position, zero/non-finite rotation, out-of-range weight, or rejected pose leaves the prior pose/root/IK frame intact. `HumanoidComponent.CurrentBodyFrameDiagnostic` exposes the last accepted pre-IK Body solve (provisional, requested, compensated, final Hips, and projected-root values) and is not replaced by rejected scratch work. Authored channels are routed only to an existing `HumanoidIKSolverComponent`, preventing a duplicate solver from being inferred. `EHumanoidIKGoalPolicy` gates ignored, compiled-basis-ready, and unconditional application. Goal diagnostics retain authored body-space, final world-space, contact offset, stretch, feet-spacing compensation, and application status independently from Body/root projection. `EHumanoidContactCompensationMode` optionally constrains feet or feet-and-hands against an explicit world plane after authored conversion.
- Live state-machine evaluation advances active motion clocks before sampling typed values and humanoid contributions, matching direct clip playback. Reading an advanced phase alongside a previous-frame value store is invalid. The Body solve covers all 95 muscles. Phase 9A is accepted against imported Mitsuki and an independent runtime-authored procedural rig at `<= 0.25 deg` and `<= 0.5 mm`; this focused gate is not bit-exact parity or the broader Phase 10 conformance matrix.
...
- **Baking & Optimization**: Property animation classes expose options like `ConstrainKeyframedFPS`, `LerpConstrainedFPS`, `BakedValueCompressionAlgorithm`, and `Bake(framesPerSecond)` so authoring tools can reduce runtime evaluation cost while preserving spline fidelity. Use `Bake(framesPerSecond, EAnimationValueCompressionAlgorithm)` to sample keyframes into an encoded baked-value store. Supported encodings are `None`, `Constant`, `RunLength`, `Delta`, and `DeltaRunLength`; delta codecs are available for unmanaged value types. The [baked value compression architecture note](../../architecture/animation/baked-value-compression.md) explains the codec lifecycle and tradeoffs. The `GetMinMax` helpers on `PropAnimVector` compute extrema for editor bounds checking or LOD heuristics.
...
### Unity `.anim` v1 capability contract

The native YAML importer declares serialized versions 6 and 7 through the
[versioned capability manifest](unity-anim-v1-capability.json). It executes
editable and packed curve families through the same typed binding path,
including integer/discrete channels and object-reference (PPtr) channels.
Portable inputs and their expected hashes are maintained in the [fixture
README](fixtures/unity-anim-v1/README.md). Unknown behaviorally relevant
fields and opaque packed CRC bindings fail with an explicit capability or
adapter diagnostic; they are never guessed or silently treated as floats.
