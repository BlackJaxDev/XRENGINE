...
- Imported Unity humanoid clips expose `HumanoidProjectedRootPose` through `HumanoidComponent.CurrentProjectedRootPose` and a consecutive `HumanoidRootMotionDelta` through `CurrentRootMotionDelta`. Validity is per component via `EHumanoidProjectedRootChannels`; XZ and yaw follow compatible clip projection metadata, while Y requires the matching calibrated coupled Hips model. Position uses XRENGINE handedness and engine length units in the canonical Body-yaw frame. `AnimationClipComponent` separately tracks the within-cycle pose, signed unwrapped loop pose, playback-epoch anchor, and temporal delta. `EHumanoidRootMotionApplicationMode` selects extract-only output, explicit named-target application, or external-consumer publication; no hierarchy target is inferred. State-machine RootQ slots use normalized shortest-arc quaternion blending and forward compatible projection metadata into the same atomic Body transaction.
- Animation-authored humanoid IK channels are routed only to an existing `HumanoidIKSolverComponent`, preventing a duplicate solver from being inferred. `EHumanoidIKGoalPolicy` gates ignored, calibrated-only, and unconditional application. Goal diagnostics retain authored body-space, final world-space, contact offset, and application status independently from Body/root projection. `EHumanoidContactCompensationMode` optionally constrains feet or feet-and-hands against an explicit world plane after authored conversion.
