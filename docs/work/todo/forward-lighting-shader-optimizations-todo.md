...
- [ ] **Cache `XRENGINE_GetForwardViewIndex()` once per fragment.**
...
- [ ] **Cache `XRENGINE_ViewDepthFromWorldPos(fragPos)` once per fragment.**
...
- [ ] **Cache `XRENGINE_GetForwardCameraPosition()` once per fragment.**
...
- [ ] **Add a direct camera-position accessor.**
...
- [ ] **Consolidate `switch(shadowSlot)` helpers** so a single switch captures
...
- [ ] **Cache `textureSize(...)` results** on shadow atlases. Each fragment hits
...
- [ ] **Macro-expansion CSE.** `#define ShadowBlockerSamples ShadowPackedI0.x`
...
- [ ] **Fast path for `XRENGINE_GetShadowBiasRange` / `XRENGINE_GetLocalShadowBias`.**
...
- [ ] **Drop unused locals in shadow path entry blocks.**
...
- [ ] **Pin `XRENGINE_ResolveProbeWeights` (debug fallback) as debug-only.**
