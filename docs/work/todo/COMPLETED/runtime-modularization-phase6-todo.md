...
Post-closeout follow-up: [Runtime Regression And NativeAOT Hardening TODO](../runtime-regression-and-nativeaot-hardening-todo.md)
...
Status: Complete. P6.0 through P6.8 are implemented; Phase 6 closed on
2026-08-28.
...
At the Phase 6 baseline, seven projects directly referenced `XRENGINE`:

| Consumer | Baseline reason that required migration |
|---|---|
| `Runtime.Bootstrap` | Transitional world, pawn, movement, physics, game-mode, and startup composition; AOT input still scanned `XRENGINE/**/*.cs` |
...
- [x] Stop publishing `ImportedAnimatorMetadataComponent` and `ImportedAvatarAnimationLayer` in runtime assets unless their controller, mask, and layer records are compiled into an executable native animation graph; retain source identities in the import manifest only.
- [x] Treat raw animator update/culling modes, platform controller layers, expression menus, parameter networks, upload identity, pipeline status, and platform/network policy as intentional loss with actionable diagnostics.
- [x] Convert imported animation callbacks only through an explicit native event allowlist; discard arbitrary name-based callbacks and source-only component/property binding adapters rather than emulating the source runtime.
- [x] Keep unsupported behaviour payloads and serialized YAML out of runtime assets; retain a bounded import-report summary for inspection, or discard the opaque payload completely when it has no supported conversion.
- [x] Preserve only generic physics-chain, collider, and weighted-constraint behavior; report unsupported grabbing, posing, collision-filtering, solver, freeze/rebake, and source-policy semantics as intentional loss.
- [x] Convert recognized toon inputs into native material features and retain conversion provenance, but discard editor drawers, remote facilities, shader-family-only controls, and unsupported shader features rather than creating a source-shader runtime authoring model.
- [x] Continue retaining avatar presentation anchors, gaze/eyelid bindings, jaw and speech-pose bindings, native humanoid retargeting/root motion, weighted transform constraints, physics-chain behavior, and generic material features because these have independent engine semantics.
...
P6.5B validation completed on 2026-08-27: runtime assets no longer publish animator metadata or avatar-layer evidence as behavior components. Prefab manifest schema 2 retains animator, layer, and expression identities as import-only records, with a future conversion seam that targets one native `AnimStateMachineComponent`. Animation manifest schema 3 publishes only explicitly allowlisted native event identifiers; the initial allowlist is deliberately empty, arbitrary source callbacks are non-blocking intentional loss, dispatch is typed-only, and opaque YAML is represented by bounded field summaries and SHA-256 digests rather than executable or serialized payloads. Generic physics-chain behavior remains native while source grabbing, posing, solver, collision-policy, and authority semantics receive actionable loss diagnostics so future local and remote interaction can share one engine-owned authority contract.

The final narrow validation built ModelAssetPipeline, Animation, and AnimationIntegration with zero warnings and zero errors. An isolated importer compile completed with zero warnings and zero errors, and a live synthetic prefab conversion published only `PhysicsChainComponent` and `AvatarPresentationComponent`, one import-side avatar graph/layer record, bounded unsupported-behaviour metadata, and the expected conversion-loss diagnostics. The full Editor graph reached unrelated concurrent Bootstrap work and stopped because `RuntimeAssetBootstrap` was missing from `Engine.RuntimeRenderingHostServices`; no P6.5B renderer or bootstrap files were changed. Per repository policy, unit-test updates and execution remain deferred until explicit user clearance; two existing fixtures still refer to the intentionally removed runtime metadata/YAML members.

## P6.6 - Migrate Applications, Samples, Benchmarks, Tests, And Tooling

- [x] Replace Bootstrap's facade reference with direct Core/Rendering/adapter contracts and remove all concrete facade world casts.
- [x] Replace Editor's facade reference with explicit project references based on real source/API use; remove `XREngine.dll` from assembly discovery/reload/watch lists.
- [x] Replace Server's facade reference and prove its evaluated dependency/publish graph excludes Editor and unused local adapter/native cargo.
- [x] Replace VRClient's facade reference and preserve its explicit startup/profile/no-peer diagnostics and intended renderer/VR cargo.
- [x] Replace UnitTests' facade reference by moving tests to the owning assembly surface; retain direct leaf references only for tests of leaf implementation.
- [x] Replace Benchmarks' facade reference and update benchmark source maps so they measure current owners rather than deleted paths.
- [x] Replace `Samples/MonkeyBallVR` facade use with direct runtime/Bootstrap references and validate sample startup.
- [x] Update Editor project templates and generated application projects so new games never reference `XRENGINE/XREngine.csproj`.
- [x] Remove `XRENGINE/**/*.cs` from Bootstrap and script AOT scan roots; enumerate only final owning projects.
- [x] Update solution/project discovery code that means the facade project while preserving legitimate `XRENGINE.slnx`, product-name, macro, and branding strings.
- [x] Update `.slnx`, legacy `.sln`, VS Code tasks/launch profiles, ExecTool scripts, publish profiles, CI workflows, dependency reports, and docs that reference the project or DLL.
- [x] Validate single-backend static/AOT and collectible editor registration without relying on facade assembly load side effects.
- [x] Build and launch each migrated consumer before removing the facade project.
- [x] Add graph/source tests proving none of the seven former consumers references or loads `XREngine.dll`.

P6.6 validation completed on 2026-08-27. All seven former consumers now use explicit module references, and metadata inspection reports zero facade assembly, type, or member references. All production sources have moved out of the facade; its only four remaining compile inputs are the P6.7 assembly metadata/type-forward files. Bootstrap owns application composition, including a specialized headless adapter lease; the Server selects the `None` renderer profile and its publish output excludes Editor, concrete renderer leaves, and native/local-device OpenVR, SDL/GLFW, Vulkan/VMA, and other unused cargo. Managed adapter/wrapper metadata dependencies remain without installing those local services. Editor project generation, discovery, AOT roots, benchmark maps, tasks, scripts, and the MonkeyBall launcher no longer rely on facade load side effects. Data-owned YAML converters and `.xrproj` loading are explicitly registered, and generated launchers install modular asset services before reading startup configuration.

Bootstrap built for OpenGL-only and Vulkan-only static configurations with zero warnings and zero errors, and generated AOT registrations contained no facade assembly. Bootstrap, Editor, Server, VRClient, UnitTests, Benchmarks, and MonkeyBall built cleanly. Editor, Server, VRClient, and the generated MonkeyBall `Game.exe` each remained alive for their bounded startup observation and only the owned process was stopped; the MonkeyBall package contained no `XREngine.dll`. The final P6.6 boundary/launcher/packaging set passed 14/14. A broader 29-test focused run passed 27 and exposed two source-contract failures in concurrent renderer work (`BackendGeneration_IsInstalledBeforeApiWrappersAreCreated` and `ReplacementAcceptance_RequiresBackendValidatedFrameContent`); neither failure exercises a P6.6 consumer or facade boundary and no concurrent renderer source was changed to mask them.

## P6.7 - Remove Compatibility Forwards, Cargo, And The Facade Project

- [x] Complete the repository asset/type-identity migration and archive a durable report of every rewritten or redirected legacy identity.
- [x] Prove repository YAML, JSON, cooked, MemoryPack, prefab, scene, project, and generated settings resolve without loading `XREngine.dll`.
- [x] Replace exact legacy CLR lookup tests with supported loader/redirect tests and explicit external-breaking-change tests.
- [x] Remove all 103 facade type forwards after their repository consumers and persisted identities are migrated.
- [x] Remove facade-only type redirects, friend access, source-generation roots, reflection scans, assembly lists, load-context exceptions, and compatibility shims.
- [x] Move or remove all 19 facade package references; validate that each remaining package is declared by its real direct consumer.
- [x] Move `lib_coacd.dll`, optional RestirGI copy behavior, NIS license cargo, and any remaining native/content items to one final owner or remove dead cargo.
- [x] Run `pwsh Tools/Reports/Generate-Dependencies.ps1` after the final package/native move and review `docs/DEPENDENCIES.md`, license snapshots, and ownership overrides.
- [x] Remove `XRENGINE/XREngine.csproj` from `XRENGINE.slnx` and `XRENGINE.sln`.
- [x] Delete the now-empty `XRENGINE` production source/project directory after verifying every tracked file has a recorded disposition.
- [x] Prove no build, publish, test, template, script, or runtime path produces, copies, loads, probes for, or requires `XREngine.dll`.
- [x] Prove searches for `XRENGINE/XREngine.csproj`, facade-only source paths, and assembly identity return no active references; classify legitimate product/solution-name strings separately.
- [x] Remove temporary migration adapters and compatibility leases created during Phase 6.
- [x] Update the reference design from proposed architecture to completed end state and record any deliberate deviation.

P6.7 completed on 2026-08-27. The facade project, all 103 type forwards, its remaining cargo, and the `XRENGINE` production directory are removed. The 384-row source ledger records 378 migrated and six deleted sources; the 103-row identity report maps every forwarded legacy identity to its current assembly. Repository persistence resolves moved types through stable full names and published AOT roots, while direct external CLR lookup through the deleted `XREngine` assembly is an intentional pre-v1 breaking change. CoACD is owned by Runtime.Core, optional RestirGI cargo by Runtime.Rendering, and NIS/Streamline license cargo by the repository NVIDIA SDK location. The regenerated 24-project graph contains no facade row, and metadata inspection reports zero facade assembly/type/member references across all seven former consumers.

Data, Runtime.Core, Bootstrap, and UnitTests built independently with zero warnings and zero errors; the complete Bootstrap dependency graph also built with zero warnings and zero errors. The focused facade-identity/boundary set passed 51/51, and the broader asset, serialization, prefab, project, settings, and launcher set passed 150/150. Current build outputs contain no facade assembly; exact stale facade binaries from old ignored product/tool output folders were removed. Historical strings remain only in migration evidence, explicit breaking-change/absence tests, and the generated-launcher rejection guard; product and solution names that contain `XREngine` are not facade references.

## P6.8 - Final Validation And Closeout

- [x] Build Data, all lower feature/format libraries, Runtime.Core, Runtime.Rendering, both renderer leaves, all four adapters, Bootstrap, Editor, Server, VRClient, UnitTests, Benchmarks, and samples independently with zero new warnings.
- [x] Build `XRENGINE.slnx` with zero warnings and zero errors after the facade project has been removed from it.
- [x] Run the Phase 3/4/5/6 dependency, source/API, serialization, world, rendering, adapter, AOT, packaging, and compatibility regression suites.
- [x] Run targeted asset/cooked/YAML/JSON/snapshot, physics, networking, scheduler, play-mode, prefab, Unity, FBX/glTF/model-cache, input/gameplay, rendering, OpenXR, and project-template tests.
- [x] Run the full UnitTests project after unrelated concurrent work is reconciled; distinguish any unrelated pre-existing failure from a Phase 6 regression before closeout.
- [x] Start isolated named Editor sessions for canonical OpenGL and Vulkan Unit Testing World paths, verify MCP readiness, inspect changing screenshots and logs, and stop only owned sessions.
- [x] Run bounded headless Server startup through world creation, play, networking, and graceful shutdown with no local window/input/audio/VR dependency.
- [x] Publish and launch VRClient through its canonical path; validate the available runtime path and record unavailable physical-headset checks without claiming them.
- [x] Validate Debug/Release and at least one selected-backend static/AOT or trimming configuration; inspect outputs for required assemblies/native cargo and absence of `XREngine.dll`.
- [x] Re-run collectible renderer registration/unload validation and prove facade deletion introduced no stable assembly/type retention root.
- [x] Audit per-frame/fixed-update/render/input/network hot paths touched by the move for new allocations, boxing, LINQ, closures, and string construction.
- [x] Run `git diff --check`, local documentation-link validation, dependency/license review, and final source/project/cargo audits.
- [x] Update launch/setup, architecture, asset migration, serialization, application composition, and contributor documentation.
- [x] Record exact commands, results, evidence paths, known external hardware lanes, intentional breaking changes, and final dependency graphs in the Phase 6 progress ledger.
- [x] Move this tracker to `docs/work/todo/COMPLETED/` only after every engineering completion gate below passes.
- [x] Record commit/merge/promotion status accurately; do not mark engineering work incomplete merely because branch promotion was not requested, and do not claim a merge that was not performed.

P6.8 completed on 2026-08-28. The final matrix built every Phase 6 owner and
consumer, completed a clean facade-free solution build, executed the focused
and exhaustive regression lanes, exercised live OpenGL/Vulkan Editor paths,
smoked the pruned headless Server and available VRClient path, and published a
live NativeAOT MonkeyBall launcher. The closeout also isolated renderer-variant
Bootstrap outputs, made asset-service installation safely reference-counted,
pruned Server native local-device cargo, removed stale facade friend access,
and retargeted the last vacuous source-contract fixtures. The exhaustive suite
still contains unrelated animation, cooked-schema, and renderer/backend
failures; all Phase 6 boundary/profile/naming gates are green and no unresolved
facade-removal regression remains. Physical XR and supported-hardware NVIDIA
lanes were unavailable and are recorded as external acceptance, not claimed.
...
- [x] Every one of the 384 tracked facade and migration-adapter source rows has a final owner or an explicit deletion record.
- [x] `XRENGINE/XREngine.csproj` and the facade production directory are removed.
- [x] No project, solution, task, script, template, source generator, AOT scan, publish path, or runtime loader requires the facade project or `XREngine.dll`.
- [x] Runtime.Core references only Data and Extensions and contains no rendering, feature-library, adapter, Bootstrap, Editor, or application dependency.
- [x] Runtime.Rendering remains backend-neutral and both concrete backends remain one-way leaves.
- [x] Feature libraries remain below runtime adapters, and no adapter references another adapter or an application.
- [x] Bootstrap contains composition only and no migrated domain implementation.
- [x] Repository-owned assets and generated metadata load through current identities without `XREngine.dll`; unsupported external pre-v1 compatibility is documented explicitly.
- [x] Editor, Server, VRClient, UnitTests, Benchmarks, samples, and generated projects build through direct explicit references.
- [x] Editor OpenGL/Vulkan, headless Server, and the available VRClient path launch successfully through the final graph.
- [x] Static/AOT, trimming, publish, and collectible registration paths include exactly the intended assemblies and native cargo.
- [x] No facade-owned package, native binary, content rule, license, type forward, friend access, registration root, or implementation remains.
- [x] Targeted regression gates pass with zero new compiler warnings; the full regression project was executed and has no unresolved Phase 6 failure. Unrelated failures are recorded in the progress ledger.
- [x] The reference design and contributor/application/asset migration documentation describe the implemented end state.
- [x] The Phase 6 progress ledger contains the final evidence and this tracker is moved to `COMPLETED/`.
