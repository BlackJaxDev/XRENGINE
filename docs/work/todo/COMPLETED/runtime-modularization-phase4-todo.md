...
`XREngine.Scene.Physics.Physx`. At handoff the facade retained
`InstancedDebugVisualizer.cs` and the remaining GPU physics/physics-chain
...
- [x] Replace concrete construction and type tests in Runtime.Rendering, `XRENGINE`, Editor, Server, VRClient, Bootstrap, tests, and tools with stable backend IDs, capabilities, factories, or module metadata.
- [x] Ensure backend wrapper types, backend exceptions, callback delegates, tasks, worker threads, `Type` objects, and native handles are not retained by non-backend static or long-lived state after renderer teardown.
- [x] Define editor collectible registration and production/AOT static registration through the same factory contract; dynamic loading must not become a production/AOT requirement.
- [x] Add project/source-contract tests forbidding `Runtime.Rendering -> Runtime.Rendering.OpenGL`, `Runtime.Rendering -> Runtime.Rendering.Vulkan`, backend-to-backend, backend-to-Editor, and backend-to-application dependencies.
- [x] Update AOT factory generation, reflection discovery, serializers/type redirects, test discovery, application output packaging, and build orchestration for both modules.
- [x] Coordinate the resulting Vulkan source layout with the Vulkan Runtime Code Organization TODO after the frame-loop decomposition paths settle.
- [x] Statically load and validate each extracted backend before enabling collectible reload.
- [x] Run dependency/license generation after final package/native ownership moves and review the results.
