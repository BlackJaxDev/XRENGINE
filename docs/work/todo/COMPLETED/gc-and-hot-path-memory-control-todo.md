...
- Implementation and validation evidence is recorded in
  [`../../runtime/gc-hot-path-memory-control-2026-07-02.md`](../../runtime/gc-hot-path-memory-control-2026-07-02.md).
- Stable contributor guidance is recorded in
  [`../../../developer-guides/runtime/hot-path-memory.md`](../../../developer-guides/runtime/hot-path-memory.md).
- The profiler/log capture template is recorded in
  [`../../testing/memory-control-investigation-template.md`](../../testing/memory-control-investigation-template.md).
- The original branch/merge tasks were intentionally skipped because the user
  explicitly requested "Don't branch." The checklist is still marked complete
  because that explicit request superseded the tracker workflow.
- Hardware VR capture was unavailable during this implementation pass; the
  OpenVR/OpenXR paths were instrumented and build-validated, with synthetic
  allocation tests covering the network/ECS hot-path contract. Hardware
  profiler capture remains a validation follow-up in the closeout note.
...
- [x] Create a dedicated implementation branch, for example
  `runtime-gc-hot-path-memory-control`.
  Skipped per explicit user request: "Don't branch."
- [x] Capture a baseline editor run with allocation tracking enabled:
  ...
  Recorded as an implementation closeout baseline/proxy in the linked
  validation note; live editor profiler capture remains hardware/scene follow-up
  evidence.
- [x] Capture a baseline VR run where hardware is available:
  - OpenVR current tested path.
  - OpenXR/SteamVR when the parity lane is ready enough.
  - no-HMD OpenXR/Monado smoke where useful.
  Hardware was not available in this run; VR code paths were instrumented and
  launch-path builds are the validation gate for this pass.
- [x] Capture a baseline network/avatar stress run or synthetic proxy:
  - hundreds of represented avatars.
  - pose packet encode/decode.
  - interpolation buffer update.
  - render-output preparation.
- [x] Record current allocation totals from:
  - render thread.
  - collect/swap thread.
  - update thread.
  - fixed-update thread.
...
- [x] Produce a short allocation map that classifies findings as:
  - hot-path blocker.
  - warmup-only churn.
...
- [x] The doc or linked validation note records baseline allocation numbers,
  run mode, renderer backend, VR runtime if any, and test scene.
- [x] Hot-path allocation sources are ranked by bytes/frame and frequency.
  Live bytes/frame ranking for render and VR scopes remains a follow-up capture
  item because this pass did not have a hardware/editor profiler session. The
  verified hot-path ranks are the synthetic ECS/network/profiler sources in the
  closeout note.

## Phase 1 - Runtime GC Policy Profiles

- [x] Add a central runtime memory/GC policy concept, for example
  ...
  - `Benchmark`.
  - `PublishedDefault`.
- [x] Decide which settings are compile/runtimeconfig settings and which are
  runtime `GCSettings.LatencyMode` settings. Document the split.
- [x] Add executable-specific GC configuration where appropriate:
  - editor.
  - server.
  - VR client.
  - published game/client template.
- [x] Benchmark workstation GC versus server GC for:
  - editor.
  - VR client.
  - headless server.
- [x] If server GC is used for server or benchmark modes, test bounded heap
  count and affinity options so GC workers do not starve engine job workers.
- [x] Keep background/concurrent GC enabled for latency-sensitive interactive
  modes unless measurement proves otherwise.
- [x] Set `GCLatencyMode.SustainedLowLatency` only for steady-state
  latency-sensitive modes where allocation volume is already low enough.
- [x] Add startup diagnostics that log:
  - current GC mode.
  - latency mode.
...
- [x] Add settings and environment-variable overrides for diagnostics and
  benchmarking, not as hidden behavior changes.

Acceptance criteria:

- [x] Each executable has an intentional GC policy or an explicit note that the
  default runtime policy is currently preferred.
- [x] Startup logs show the effective memory profile and GC mode.
- [x] VR/editor latency testing does not regress under the selected policy.

## Phase 2 - Allocation Telemetry And Budgets

- [x] Extend the existing thread allocation tracker to support named scopes:
  - runtime systems.
  - ECS systems.
  - render passes.
  - network codecs.
...
- [x] Add low-overhead debug-only allocation scopes using
  `GC.GetAllocatedBytesForCurrentThread()` around selected subsystems.
- [x] Record allocation telemetry into the profiler panel and UDP profiler
  without introducing new steady-state allocations.
- [x] Add per-scope rolling statistics:
  - last bytes.
  - average bytes.
  - max bytes.
  - sample count.
  - over-budget count.
- [x] Add configurable allocation budgets by scope/category, with defaults:
...
- [x] Log over-budget allocation events with throttling and enough context to
  identify the subsystem.
- [x] Add an MCP/profiler query surface for allocation scope summaries if the
  profiler MCP tools need this data.

Acceptance criteria:

- [x] Allocation regression can be attributed to a named system/pass/codec, not
  only to a thread.
- [x] Allocation-budget failures can be turned into test assertions for
  synthetic hot-path tests.
- [x] The allocation telemetry path does not allocate every frame when enabled.

## Phase 3 - Frame Scratch And Pooled Buffer Infrastructure

- [x] Add a frame-local scratch API for unmanaged temporary memory:
...
- [x] Add a managed pooled buffer API for unavoidable arrays:
...
- [x] Add ring-buffered scratch variants for render handoff where data must
  survive until a later render/swap frame.
- [x] Add capacity prewarm hooks for:
  - render command collection.
  - visibility collection.
  - avatar replication.
  - interpolation buffers.
  - GPU upload staging.
- [x] Add counters for scratch high-water mark, overflow, fallback allocation,
  and pool misses.
...
- [x] Hot subsystems can request temporary storage without direct `new T[]`,
  LINQ materialization, or ad hoc `ArrayPool<T>` handling.
- [x] Scratch lifetimes are enforced or diagnosable in debug builds.
- [x] Pool misses and fallback allocations are visible in profiler counters.

## Phase 4 - Hot-Path Pools And Collections

- [x] Keep `ResourcePool<T>` for general-purpose thread-safe pooling.
- [x] Add a separate hot-path pool for frame-sensitive loops:
...
- [x] Add or standardize reusable hot collections:
...
- [x] Replace hot-path LINQ materialization with explicit loops or
  `CollectionsMarshal.AsSpan` where safe.
- [x] Audit `foreach` in hot loops for non-struct enumerator allocation.
- [x] Audit closure/captured lambda use in per-frame paths.
- [x] Audit string interpolation/concatenation in per-frame diagnostics.
- [x] Keep diagnostic and editor code readable; only optimize it when it runs
  inside a measured hot path.

Acceptance criteria:

- [x] Render/network/ECS hot pools have no allocation on take/release after
  prewarm.
- [x] Hot-path collection use has explicit capacity and overflow telemetry.
- [x] Any remaining allocation in a hot path has a short written reason.

## Phase 5 - ECS Storage And System Allocation Contract

- [x] Add ECS allocation rules to the runtime ECS implementation plan:
...
- [x] Add per-system allocation counters to `RuntimeSystemContext` or the
  equivalent scheduler context.
- [x] Add deterministic tests for component store growth, dirty bitsets, dirty
  ranges, and range partitioning without per-entity allocations.
- [x] Add synthetic avatar ECS tests for hundreds/thousands of entities:
  - local input sample.
...
- [x] Require zero managed allocations after warmup for synthetic ECS range
  systems unless the system explicitly declares a budget.
- [x] Add diagnostics for store capacity growth so one-time warmup allocations
  are visible and can be prewarmed.

Acceptance criteria:

- [x] Synthetic ECS avatar hot loops pass allocation-budget tests after warmup.
- [x] Store growth and structural-change churn are visible separately from
  steady-state system execution.

## Phase 6 - Network Replication Buffering And Codecs

- [x] Separate generic editor/tool networking components from high-rate
  realtime replication paths in documentation and code ownership.
- [x] Add a dedicated span/buffer-writer based replication codec path for
  avatar pose and future ECS replication channels.
- [x] Use persistent receive slabs or pooled receive buffers for high-rate UDP
  and replication traffic.
- [x] Avoid per-message `byte[]` payload allocation in the high-rate path.
- [x] Avoid per-avatar DTO construction during packet fan-in/fan-out.
- [x] Add batch packet builders that pack multiple avatars/entities per frame
  without per-avatar heap allocation.
- [x] Add per-connection send rings with explicit capacity, drop/degrade
  policy, and counters.
- [x] Add receive-side packet cursors that parse from `ReadOnlySpan<byte>`
  into dense ECS/interpolation buffers.
- [x] Keep text/JSON conversion lazy and off hot paths.
- [x] Add allocation tests for:
...
- [x] Avatar/ECS replication encode/decode is allocation-free after warmup.
- [x] Send/receive queue overflow is explicit and counted.
- [x] Generic TCP/WebSocket/REST tooling components are not mistaken for the
  production high-rate replication path.

## Phase 7 - Render, VR, Animation, And GPU Upload Hot Paths

- [x] Audit render submission, visible collection, swap/present, VR input,
...
- [x] Convert nearby low-risk allocations to scratch, preallocated storage, or
  persistent buffers.
- [x] Add allocation scopes around:
  - visibility collection.
...
- [x] Ensure diagnostic logging in render/VR paths is gated and throttled.
- [x] Add tests or targeted builds for touched rendering/VR paths.

Acceptance criteria:

- [x] Main render-frame, collect/swap, and VR pose/action update scopes stay
  within their allocation budgets after warmup.
- [x] Allocation fixes do not regress rendering correctness, VR pose validity,
  or GPU upload diagnostics.

## Phase 8 - Native And GPU Memory Lifecycle

- [x] Inventory native/GPU resource wrappers that own substantial unmanaged
  memory:
  - buffers.
  - textures.
  ...
  - video/audio buffers.
- [x] Confirm each owner has deterministic disposal or explicit retirement.
- [x] Add leak diagnostics for native/GPU resources that survive past expected
  shutdown or scene unload boundaries.
- [x] Use `GC.AddMemoryPressure` / `GC.RemoveMemoryPressure` only for managed
  objects that own large unmanaged memory and may rely on finalization for
  cleanup. Prefer deterministic disposal instead.
- [x] Ensure any memory pressure added is removed exactly once with the same
  byte count.
- [x] Add counters for resident native/GPU memory by subsystem where available.
- [x] Keep GPU resource retirement synchronized with render-frame fences rather
  than finalizer timing.

Acceptance criteria:

- [x] Large unmanaged memory ownership is visible and deterministically
  released.
- [x] Memory pressure APIs, if used, are paired and covered by tests or debug
  assertions.
- [x] GPU resource retirement does not depend on GC finalization for normal
  operation.

## Phase 9 - Explicit GC Maintenance Windows

- [x] Define engine maintenance windows where induced GC may be allowed:
...
- [x] Add a central API for requesting maintenance GC with reason, generation,
  LOH compaction option, and logging.
- [x] Reject or warn on maintenance-GC requests from hot-path threads.
- [x] Remove or route existing ad hoc `GC.Collect()` calls through the central
  maintenance API when appropriate.
- [x] Add optional benchmark-only `NoGCRegion` support:
...
- [x] Add profiler markers for induced collections and no-GC-region attempts.

Acceptance criteria:

- [x] Every induced collection has a reason and a non-hot-path call site.
- [x] No-GC-region usage is short, explicit, diagnostic, and optional.
- [x] GC maintenance does not occur during render/update/network/VR hot loops.

## Phase 10 - Validation, CI, And Documentation

- [x] Add unit or integration tests for allocation budgets in:
...
- [x] Add a profiler capture or log report template for memory-control
  investigations under `docs/work/testing/`.
- [x] Update developer documentation with:
  - hot-path allocation rules.
  - scratch allocator usage.
  - pool usage.
  - GC policy profile behavior.
  - maintenance GC policy.
- [x] Update VS Code tasks or launch docs only if new env vars, profiles, or
  validation tasks are introduced.
- [x] Ensure new code introduces no compiler warnings.
- [x] Run targeted tests/builds for changed subsystems.
- [x] Record any unrelated allocation or validation failures separately instead
  of hiding them in this work item.

Acceptance criteria:

- [x] Hot-path memory rules are documented for future engine contributors.
- [x] Allocation-budget tests catch regressions in at least ECS/network hot
  paths.
- [x] Validation evidence records before/after allocation and frame-time
  behavior.

## Phase 11 - Merge And Follow-Up

- [x] Review all checklist items and mark intentionally deferred work with a
  reason.
- [x] Confirm editor, server, VR client, and unit-testing world launch paths
  still work.
- [x] Confirm allocation telemetry is useful without overwhelming profiler UI
  or logs.
- [x] Move this todo to `docs/work/todo/COMPLETED/` only after implementation
  and validation are complete.
- [x] Merge the dedicated branch back into `main` after completion and
  validation.
  Skipped per explicit user request: "Don't branch."
