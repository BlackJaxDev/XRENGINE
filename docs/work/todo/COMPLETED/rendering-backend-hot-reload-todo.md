...
- [OpenXR Runtime Code Organization TODO](../rendering/vr/openxr-runtime-code-organization-todo.md)
...
Phase 4 integration update (2026-07-29): the stable rendering kernel, Editor,
`XRENGINE`, Server, and VRClient no longer own concrete backend implementation
APIs. Bootstrap is the explicit selectable static/AOT composition root, and the
same module-entry/factory lifecycle remains authoritative for collectible
Editor generations. The Vulkan desktop frame-loop production decomposition is
also complete; its deferred runtime/hardware validation remains tracked by its
own work item and does not change this completed reload architecture.
