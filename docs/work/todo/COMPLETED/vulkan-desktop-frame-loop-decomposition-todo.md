...
Status: Code Complete; Validation Deferred
Target Branch: `rendering-vulkan-frame-loop-decomposition`

The production implementation is complete. Historical branch/baseline audit
evidence plus runtime, hardware, performance, and remaining test-matrix
execution are intentionally deferred; their unchecked boxes remain as evidence
gates and do not represent missing production code.
...
- [OpenXR Monado Framerate Investigation](../../investigations/rendering/archive/openxr-monado-framerate-2026-07-06.md)
- [Editor Origin/Eye Camera Flicker Investigation](../../investigations/rendering/archive/editor-origin-eye-camera-flicker-2026-06-28.md)
...
- [x] **1.6** Add a concurrency test for atomic desktop activity publication:
  ...
  mutable slot.
- [x] **1.7** Add or expose deterministic fault-injection seams at acquire,
...
- [x] Result and ownership policy can be tested without a physical Vulkan
  device.
- [x] The tests fail for unresolved or double-consumed acquired work.
- [x] Fault injection does not allocate or add virtual/interface dispatch to the
  normal frame hot path.
- [x] Existing source contracts retain coverage until behavioral replacements
  exist.
...
- [x] Every architecture-affecting item in "Open Questions To Resolve In Phase 0
...
- [x] `FrameLoop.cs` contains no generic render-state API, frame-op API, or
  render-object factory code.
- [x] OpenXR can query desktop activity without combining unsynchronized field
  reads.
- [x] Diagnostics for a frame attempt use its captured slot and frame id.
- [x] Mechanical moves compile and pass focused tests before phase extraction
  starts.
- [x] OpenXR retirement entry cannot race a new desktop attempt into destroying
...
- [x] **3.5** Move the existing body temporarily behind an
  `ExecuteDesktopFrame(ref DesktopFrameAttempt)` seam before extracting phases;
  the temporary seam was removed after the final phase coordinator replaced it.
...
- [x] One context owns attempt-local mutable state.
- [ ] No context copy, boxing, closure, task, or heap allocation occurs per
  frame.
- [x] Finalization runs for every entered attempt.
- [x] The temporary coordinator seam preserves baseline behavior before phase
  movement.
...
- [x] Preflight never acquires an image or records/submits commands.
- [x] Every pre-acquire skip has a named reason and tested frame-op policy.
- [x] Interactive resize remains non-blocking.
- [x] Swapchain implementation is not duplicated in the preflight partial.
...
- [x] **5.8a** Move the consecutive `NotReady`/`Timeout` policy into the named,
  allocation-free `VulkanDesktopAcquireAvailabilityTracker`.
- [ ] **5.8b** Execute tests for successful-acquire reset, interactive-resize
  preservation, and recreate thresholds.
...
- [x] Acquire dispatch, result policy, and ownership establishment are distinct
  reviewable operations.
- [x] No acquired image/semaphore can exit the phase without its ownership and
  required terminal action recorded in the attempt.
- [x] Native and Streamline acquire paths produce the same typed outcomes.
- [x] Resource retirement happens only after the captured slot completes.
...
- [x] Recording phase performs no queue submit or present.
- [x] Command-buffer ordering and swapchain final-layout ownership match the
  baseline.
- [x] Every recording failure returns one typed recovery obligation.
- [x] Detailed command recording remains in the `Commands/` subsystem.
...
- [ ] **7.8** Add deterministic fault tests at every recovery operation,
  ...
  swapchain recreate.
- [x] **7.9a** Preserve the abort-layout rule: clear conservative tracked
...
- [ ] **7.9b** Execute the dedicated abort-layout fault matrix.
...
- [x] There is one auditable post-acquire recovery policy.
- [x] Acquire semaphore, image, upload buffer, and abort command buffer each
  have exactly one terminal ownership transition.
- [x] Recovery cannot issue GPU work after device loss.
- [x] Lost-device timeline zeroing cannot authorize normal completion or
  retirement.
- [x] The former approximately 196-line local recovery function no longer
...
- [x] Submission and presentation have separate ownership boundaries.
- [x] No stack pointer or Vulkan submit/present struct escapes its helper.
- [x] Timeline publication, upload retirement, collect release, and slot advance
  occur once and in the required order.
- [x] Auxiliary marker/diagnostic/trim failures preserve their visibility
  without stranding acquired or submitted presentation ownership.
- [x] All queue operations use the hardened shared gateway.
...
- [ ] **9.8** Execute the allocation/generated-code audit over every new phase
  method. Any observed closure, LINQ, boxing, unconditional interpolation,
  accidental context copy, or other steady-state allocation becomes a code
  defect rather than an accepted validation result.
...
- [x] The coordinator reads as the target lifecycle without opening another
  file to understand phase order.
- [x] Each partial has one declared responsibility and no catch-all helper file
  exists.
- [x] No method replaces the old monolith with another several-hundred-line
  phase.
- [x] Profiler and stats comparisons remain possible across the refactor.
...
- [x] **12.8** Finish the tracked progress/validation note with base/final
  ...
  hardware path. Deferred evidence is recorded explicitly rather than inferred.

Acceptance criteria:

- [x] Durable architecture docs describe the code that actually shipped, not
  the pre-refactor callback.
- [x] A contributor can locate each frame phase, understand its allowed
  dependencies, and trace normal/recovery/OpenXR coexistence flow.
- [x] Streamline/DLSS, device-loss, and validation limitations are explicit.
...
- [x] A successful `SuboptimalKhr` acquire continues through
  record/submit/present with acquired ownership. It schedules recreation after
  settlement unless interactive present scaling can safely retain the current
  swapchain.
- [x] `ErrorSurfaceLostKhr` is a visible renderer/platform-surface restart
  failure. A swapchain-only rebuild is never presented as surface recreation.
- [x] `DesktopFrameActivityState` atomically publishes the active immutable
  frame/slot identity. `_desktopFrameRetirementGate` excludes OpenXR retirement
  drains from new attempt entry; the former separate in-progress flag is gone.
- [x] Attempt-local lifecycle types remain nested in `VulkanRenderer`. Pure
  reusable policies, outcomes, activity publication, availability tracking, and
  fault-injection state are internal top-level types for direct inspection.
- [x] The complete coordinator set remains as responsibility-specific direct
  `Frame/` siblings. No second desktop-frame-loop directory taxonomy is added.
- [x] A healthy submit failure first cancels unsubmitted upload work and settles
  the acquired image through recovery presentation or swapchain invalidation,
  then propagates a result-specific failure. Device loss bypasses recovery GPU
  work.
- [x] The desktop slot advances exactly once after successful or recovered
  presentation/settlement. It does not advance while device-loss ownership is
  indeterminate.
- [x] `_lastDesktopFrameTickObservedTimestamp` means an accepted callback
  reached its completion/skip observation point. Submission and presentation
  readiness remain separate timeline and bookkeeping states.
- [x] OpenXR session-start readiness is a bounded shared lifecycle gate. It
  reads coherent activity/attempt state and uses explicit dirty-quiet and
  pending-timeline bypass limits; retirement exclusion does not retain the
  desktop gate across queue work.
- [x] Existing profiler scope names and queue-operation caller labels are
  tooling contracts. Extracted submit/present paths pass explicit stable labels
  instead of inheriting new helper names.
...
- [x] `WindowRenderCallback` is a readable 60-120-line coordinator with named
  phases and one outer finalization path.
- [x] Responsibility-specific partial files match the target ownership map; no
  `Helpers`, `Misc`, or replacement monolith exists.
- [x] Generic render-state/frame-op APIs and the render-object factory no longer
  live in the frame-loop file.
- [x] A stack-only attempt context captures immutable frame identity and all
  post-acquire ownership state.
- [x] Every early exit has a named, tested disposition and cleanup obligation.
- [x] Acquire semaphore, swapchain image, upload command buffer, timeline value,
  and frame slot each transition exactly once.
- [x] Ownership is represented by typed states/transition methods rather than
  contradictory boolean combinations.
- [x] `SuboptimalKhr` acquire behavior is correct and covered by tests.
- [x] `ErrorSurfaceLostKhr` follows an explicit surface-recreate or visible
  restart/failure policy rather than an endless swapchain-only retry.
- [x] Dirty-after-record work is never submitted.
- [x] Queue submit/present paths use the hardened shared gateway and stop after
  device loss.
- [x] OpenXR reads a coherent desktop activity/slot contract and never drains
...
- [x] Queue-operation caller labels and first-failure diagnostic provenance are
  stable or have a documented, tested migration.
- [x] Fallible marker, trimming, telemetry, and invariant work cannot strand an
  acquired/submitted image or mask the primary exception.
- [x] Collect-visible release remains before potentially blocking desktop
  present.
...
- [x] Architecture docs, rendering README, code map, Frame README, and progress
  evidence describe the final source layout and lifecycle accurately.

## Final Promotion

- [x] Review the final diff by lifecycle responsibility and ownership transition,
