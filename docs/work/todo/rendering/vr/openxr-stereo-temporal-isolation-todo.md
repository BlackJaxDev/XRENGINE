...
- [x] Decoupled OpenXR per-eye swapchain resolution from the desktop/editor window size.
  ...
  - Headset/custom presets are no longer clamped back to `XrViewConfigurationView.MaxImageRect*`; windowed Monado can report that max from the current window-sized recommendation, which defeated deterministic headset-resolution testing. The engine now logs `exceedsRuntimeMax` and requests the configured extent.
- [x] Decoupled Monado's simulated HMD panel from the editor window/default runtime recommendation for deterministic unit testing.
  ...
  - Follow-up fix after repeated Monado preview validation: the staged Windows Monado build now treats the windowed simulated HMD as a fixed-size device panel, not a resizable desktop viewport. Its Win32 compositor window is created without resize/maximize affordances and restores the preferred client extent if Windows reports a different live client size.
