# Poiyomi Toon 9.3 Conversion And Authoring-Parity Checklist
...
- [x] Import `_ToonRamp`; the shader samples it but the converter never binds
  the source texture.
- [x] Forward real UV1, UV2, and UV3 varyings instead of copying UV0 into all
  four fragment UV slots.
- [x] Implement the outline feature as an actual inverse-hull draw pass; the
  current feature only exposes parameters.
- [x] Import Poiyomi `_FlipbookTexArray` as a texture array instead of looking
  for a 2D `_FlipbookTexture` sprite sheet.
- [x] Translate dissolve mode values rather than copying incompatible enum
  values directly.
- [x] Stop collapsing distinct shade, rim, metallic, smoothness, and dissolve
  textures into semantically different uber samplers.
- [x] Preserve texture color space, normal/data classification, wrap, filter,
  mip, anisotropy, and alpha import settings from Unity metadata.
- [x] Preserve complete blend, depth, stencil, queue, offset, fog, and pass
  state instead of reducing the material to cull/cutoff/transparency mode.
- [x] Parse shader keywords, disabled passes, string tags, lock metadata, and
  material animation bindings.
- [x] Replace silent unsupported-property drops with a structured report.
- [x] Replace the flat/basic uber inspector model with a complete data-driven
...
This is the current architectural comparison. catalog inventory must replace preliminary
...
- [x] Keep every optional feature compile-time removable through the existing
  uber variant system.
- [x] Keep per-frame material binding and render-submission paths allocation-
  free after initial preparation.
- [x] Share authored state across base, outline, shadow, depth, velocity, and
  other generated passes; do not clone independent drifting materials.
- [x] Preserve source values even when a runtime adapter is unavailable.
- [x] Make fallback behavior explicit in the report and editor inspector.
- [x] Use semantic texture defaults: white, black, flat-normal, neutral data, or
  identity lookup as required by each source slot.
- [x] Treat sRGB color textures, linear data textures, normals, height fields,
  masks, arrays, and cubemaps as distinct roles.
- [x] Avoid combinatorial source duplication for repeated slots. Generate or
  parameterize decal, matcap, emission, and rim families from shared contracts.
- [x] Audit shader sampler/descriptor limits on every supported backend before
  selecting a binding strategy.
- [x] Preserve deterministic property order, variant hashes, reports, and test
  fixture output.
- [x] Keep inspector expression parsing, dependency discovery, label parsing,
...
- [x] Route every inspector mutation, preset, generated texture, link update,
  and multi-material operation through undo/redo, dirty-state, asset-save, and
  deterministic reimport contracts.
- [x] Treat shader-supplied URLs, editor commands, output paths, and remote
  content as untrusted data; allowlist commands, validate paths, and require an
  explicit policy for network access.
- [x] Add XML documentation to new C# public types and non-obvious members.
- [x] Preserve the Poiyomi MIT license and attribution for any adapted source
...
## Pinned Source And Parity Contract
...
## Lossless Unity Material And Asset Ingestion
...
## Conversion Baseline Correctness
...
- [x] UV0-UV3 produce distinguishable reference output on a four-UV test mesh.
- [x] The conversion report contains no false claims for features that do not
  execute.

## Render-State And Multi-Pass Material Architecture
...
- [x] Pass ordering and state are correct in OpenGL and Vulkan captures.

## Uber Module And Binding Infrastructure
...
## Color, Normals, Alpha, UV, And Mask Parity
...
- [x] Match AudioLink-driven alpha after the AudioLink adapter is available.
...
- [x] Implement UV distortion with mask and AudioLink hooks.
...
- [x] Implement global mask modifiers for vertex, backface, mirror, camera, and
  distance behavior.
...
- [x] The color/normal/alpha/UV/mask reference corpus matches Unity captures
...
## Lighting, Toon Modes, PBR, And Surface Response
...
- [x] Match per-slot color, texture, mask, normal source, intensity, blend,
  replace/multiply/add, lighting, emission, UV, and theme controls.
- [x] Implement rim slots 0-1.
- [x] Implement depth rim.
- [x] Match rim mask, width, sharpness, bias, light/shadow, blend, emission,
  normal, texture, and theme behavior.

Acceptance criteria:

- [x] Every shading mode has a dedicated material fixture and reference scene.
- [x] Every lighting family behaves consistently under directional, point,
  spot, ambient-only, shadowed, and reflection-probe scenes.
- [x] OpenGL and Vulkan stay within the approved image-difference tolerance.
...
- [x] Match emission, hue shift, chromatic aberration, video, TPS, theme, and
  AudioLink hooks per slot.
- [x] Match mirror/camera visibility and global-mask interaction per slot.
...
- [x] Slot order, masks, animation, and theme/AudioLink hooks match reference
  captures.
...
## Vertex Features And Runtime Integration Adapters
...
## Material Animation And Variant Preparation
...
`docs/architecture/rendering/poiyomi-effects-integrations-animation.md`. Unity/VRChat-only inputs
...
## ThryEditor-Parity ImGui Material Authoring
Implementation reference:
`docs/architecture/rendering/poiyomi-material-authoring.md`.

Validation (2026-07-26): the focused editor build completes with zero compiler
errors, and the focused material authoring fixtures pass 15/15 against the embedded
...
## Editor, Import Reporting, And Reimport Workflow
...
## Test Corpus And Automated Validation

### Fixture Corpus

- [x] Add licensed unlocked and locked 9.3.64 materials.
- [x] Add one focused material for every feature family and render preset.
- [x] Add maximal practical materials that combine interacting features.
- [x] Add meshes with UV0-UV3, vertex colors, tangents, skinning, morph targets,
  mirrored transforms, and non-uniform scale.
- [x] Add texture fixtures covering sRGB, normal, linear masks, packed data,
  cubemaps, and texture arrays.
- [x] Add animation clips for scalar, vector, color, texture, repeated-slot, and
  renamed-animated bindings.
- [x] Add pinned schema fixtures containing every active built-in, Thry, and
  Poiyomi annotation plus commented/inactive lookalikes that the inventory must
  exclude.
- [x] Add locale, preset, clipboard, packing-recipe, texture-array, material-
  link, cross-shader, and user-override fixtures with explicit version data.
- [x] Add multi-material fixtures that exercise compatible, mixed, missing,
  packed/split, and intentionally incompatible semantic properties.
- [x] Record the Poiyomi version, Unity version, source values, and asset license
  for each fixture.

### Unit And Contract Tests

- [x] Test every catalog property classification.
- [x] Test YAML and texture-metadata parsing across supported Unity layouts.
- [x] Test version and locked-shader detection.
- [x] Test every texture, scalar, vector, color, enum, and render-state mapping.
- [x] Test unknown-property preservation and diagnostic stability.
- [x] Test pass generation and state isolation.
- [x] Test animation binding remapping and static/animated specialization.
- [x] Test semantic default textures and sampler state.
- [x] Test variant determinism, dependency activation, and unused-feature
  pruning.
- [x] Test maximal sampler/descriptor-limit handling.
- [x] Snapshot-test the pinned UI-usage inventory so additions, removals,
  comments, malformed annotations, and embedded Thry source changes produce a
  reviewed catalog diff.
- [x] Test schema hierarchy, source order, stable IDs, section balancing,
  default/persistent expansion, alternatives, localization, and schema-version
  migration.
- [x] Test condition and arithmetic parsing, precedence, coercion, texture/
  render-queue/animation operands, dependency invalidation, unknown values,
  cycle detection, and malformed input.
- [x] Test action parsing and atomic execution for property, tag, shader, URL,
  editor-command, render-preset, failure rollback, undo, and single variant
  invalidation.
- [x] Contract-test every active widget registration against value shape,
  declared references, source defaults, mixed values, reset, static/animated
  modes, and generic-fallback prohibition.
- [x] Test property/section/material copy, Paste Special, clipboard migration,
  preset preview/revert/apply/order, semantic cross-shader application, and
  precise skipped-value reports.
- [x] Test multi-material edits and link propagation for one transaction, no
  cycles, asset deletion/reimport, mixed modes, and compatible semantic types.
- [x] Test deterministic gradient, curve, ramp, RGBA packer, and texture-array
  outputs plus color-space, format, dependency, cancellation, overwrite, path,
  and undo behavior.
- [x] Test local notes, search indexing, texture-use navigation, material
  cleanup preservation, optimizer state transitions, and animation-record
  auto-marking.
- [x] Fuzz imported labels, annotations, expressions, URLs, paths, action
...
- [x] Add a deterministic inspector interaction harness that can select nodes,
...
- [x] Validate raw-name and localized search, ancestor reveal, focus transfer,
  state restoration, duplicate-label IDs, and navigation after reimport.
- [x] Exercise every specialized drawer with keyboard, mouse, drag/drop,
  clipboard, mixed selection, reset, and animation modes.
- [x] Exercise preset preview cancellation, action rollback, generated-asset
  cancellation, multi-edit partial compatibility, and viewport-tool cancel.
- [x] Capture reviewed ImGui reference images for collapsed, expanded, searched,
  conditional, mixed-value, warning/error, optimizer, preset, packer, gradient,
  array, cross-editor, and decal-tool states.
- [x] Validate narrow/wide inspector layouts, supported Windows DPI scales,
  long translations, missing glyphs, and high-property-count scrolling.
- [x] Validate renderer/material/scene selection changes while modal, preview,
  background, and viewport tools are active.
- [x] Validate editor restart persistence for expansion, locale, notes, presets,
  links, local overrides, and generated recipes without persisting transient
  preview state.
- [x] Verify no remote content is fetched and no external editor command runs
  until the applicable explicit policy/confirmation path is exercised.

### Shader Compilation

- [x] Compile minimal, common, family-maximal, and global-maximal variants.
- [x] Compile base, depth, shadow, velocity, picking, outline, and transparency
  pass permutations.
- [x] Compile standard, OpenVR, and OpenXR vertex variants.
- [x] Compile OpenGL and Vulkan GLSL paths.
- [x] Use pairwise/combinatorial feature sampling for interactions that cannot
  be exhaustively enumerated.
- [x] Treat shader warnings as validation failures for new code.

### Visual Validation

- [x] Capture reference images from the pinned Poiyomi/Unity environment using
  documented camera, light, exposure, color-space, and render settings.
- [x] Reproduce reference scenes in the Unit Testing World.
- [x] Capture XRENGINE OpenGL and Vulkan results from identical camera views.
- [x] Compare opaque, cutout, each transparent preset, outlines, shadow maps,
  depth/normal, motion vectors, and final composited output.
- [x] Validate multiple camera positions so stale or view-independent sampling
  failures cannot pass accidentally.
- [x] Establish numeric image-difference thresholds per feature family and keep
  human review for expected engine-native differences.
- [x] Use RenderDoc for pass/resource discrepancies not explained by screenshots
  or logs.

### Performance And Reliability

- [x] Measure import time and allocations per material and avatar corpus.
- [x] Measure variant generation and backend compilation time.
- [x] Measure shader source/binary size and prewarm-cache growth.
- [x] Measure steady-state CPU binding and submission allocations.
- [x] Measure GPU cost for isolated feature families and representative combined
  materials.
- [x] Measure sampler/descriptor pressure and texture residency.
- [x] Measure schema load/compile time, first inspector open, steady expanded and
  filtered ImGui draw time, condition invalidation cost, and managed allocations
  using a maximal Poiyomi material.
- [x] Measure preview GPU/CPU memory, preset/search index size, packing and array
  generation time, and batch/cross-editor variant churn.
- [x] Stress repeated open/close, search, locale switching, undo/redo, preset
  preview, texture generation cancellation, material relinking, and reimport for
  stale references or leaked resources.
- [x] Stress material reimport, shader hot reload, device recreation, and scene
  teardown.
- [x] Validate no new OpenGL/Vulkan warnings or resource-lifetime errors.

Acceptance criteria:

- [x] The fixture corpus has no silent conversion omissions.
- [x] Exact mappings meet approved image thresholds on OpenGL and Vulkan.
- [x] Native equivalents have reviewed reference differences documented.
- [x] All targeted pass, VR, animation, and render-state permutations compile
  and execute without new warnings.
- [x] Steady-state hot paths introduce no avoidable heap allocations.
- [x] The maximal Poiyomi inspector meets approved responsiveness, allocation,
  memory, and background- Notwork cancellation budgets.

## Documentation, Release, And Maintenance

- [x] Update the Unity conversion integration guide with the actual supported
  Poiyomi version and parity table.
- [x] Document every native-equivalent integration and semantic difference.
- [x] Document conversion report codes and remediation steps.
- [x] Document how static versus animated properties affect variants.
- [x] Document sampler/descriptor limits and behavior for over-limit materials.
- [x] Publish the pinned Thry/Poiyomi UI-usage catalog and a parity table mapping
  every active annotation and reachable workflow to its XRENGINE equivalent.
- [x] Add an uber material authoring guide covering hierarchy, search,
...
- [x] Document schema/action security, safe links, remote-content policy,
  generated-asset path rules, undo boundaries, and reimport/override behavior.
- [x] Document how third-party shaders register custom controls and tools
  without arbitrary reflection or shader-supplied code execution.
- [x] Add a contributor guide for updating the property catalog for a new
  Poiyomi release.
- [x] Add a source-version audit tool to detect upstream property/pass changes.
- [x] Require a catalog diff and fixture update before declaring another
  Poiyomi version supported.
- [x] Record adapted-code attribution and license notices.
- [x] Update generated MCP/docs outputs only if public tools or editor workflows
  change.
- [x] Include what changed, why, validation, risks, native-equivalent behavior,
  and remaining unsupported integrations in PR notes.

Acceptance criteria:

- [x] Public documentation and the converter report agree with executable
  support.
- [x] Future upstream changes are detectable without manually diffing a multi-
  megabyte shader.
...
- [x] Main color/texture and adjustments.
- [x] Main normal map and normal correction.
- [x] Detail color and detail normal.
- [x] Full alpha options.
- [x] Decals 0-3.
- [x] Backface controls.
- [x] RGBA color/normal/PBR/emission masking.

### Shading

- [x] Light data, AO, detail shadow, and shadow masks.
- [x] Texture Ramp shading.
- [x] Multilayer Math shading.
- [x] Wrapped shading.
- [x] Skin shading.
- [x] ShadeMap shading.
- [x] Flat shading.
...
- [x] Anisotropics.
- [x] Matcaps 0-3.
- [x] Cubemap/reflection probes.
- [x] Rim lighting 0-1 and depth rim.
- [x] Subsurface scattering.
- [x] Reflections and first/second specular.
- [x] Clear coat.
- [x] Environmental rim.
- [x] Stylized reflections.
- [x] Backlight.
- [x] LTCGI native adapter/classification.
- [x] Light Volumes native adapter/classification.

### Outlines

- [x] Inverse-hull pass.
- [x] Basic, Rim, Directional, and Drop Shadow modes.
- [x] Width/mask/texture/color/emission/color-adjustment controls.
- [x] Distance alpha, screen width, lighting, and Z offset.
- [x] Vertex-color controls.
- [x] Cull/depth/stencil/blend/AudioLink controls.

### Special Effects

- [x] UV tile/face discard.
- [x] Depth bulge.
- [x] Full dissolve family.
- [x] Texture-array flipbook.
- [x] Emissions 0-3.
- [x] Glitter/sparkle.
- [x] Pathing.
- [x] Mirror/camera visibility.
- [x] Depth FX/touch glow.
- [x] Stats overlay classification/implementation.
- [x] Proximity color.
- [x] Internal parallax effects.
- [x] Video/CRT/Gameboy effects.
- [x] Voronoi.
- [x] Truchet.
- [x] Material post-processing.

### AudioLink

- [x] Master AudioLink provider and overrides.
- [x] Decal spectrum.
- [x] Volume color.
- [x] Per-feature AudioLink consumers.

### Vertex

- [x] Basic/fun vertex manipulation.
- [x] Look-at.
- [x] Glitching.
- [x] Vertex-color inputs and masks.
- [x] Uzumore/view-clip prevention.
- [x] Bounds, shadows, depth, outlines, and motion-vector agreement.

### Global Data And Modifiers

- [x] Blacklight masking.
- [x] Global themes 0-3.
- [x] Global mask textures 0-3 and all RGBA channels.
- [x] Vertex/backface/mirror/camera/distance global mask modifiers.

### UV And Sampling

- [x] UV0-UV3.
- [x] Deliot-Heitz stochastic sampling.
- [x] Hex-tile stochastic sampling.
- [x] UV distortion and AudioLink modulation.
- [x] Local/world projection.
- [x] Panosphere.
- [x] Polar UVs.
- [x] Standard parallax heightmapping/POM.

### Rendering And Animation

- [x] World AO blocker behavior/classification.
- [x] Every Poiyomi render preset.
- [x] RGB/alpha blend factors and operations.
- [x] Depth, cull, color mask, offset, fog, and queue state.
- [x] Front/back/outline stencil state.
- [x] GPU instancing and multi-view behavior.
- [x] EarlyZ, base, additive-equivalent, outline, and shadow participation.
- [x] Locked/unlocked material animation bindings.
- [x] Renamed-animated property bindings.
- [x] Static property specialization and variant prewarming.

### ThryEditor / ImGui Material Authoring

- [x] Pinned active annotation, metadata-option, context-action, auxiliary-tool,
  and workflow inventory.
- [x] Stable-ID schema tree with ordered nested sections and referenced
  properties.
- [x] Persistent/default expansion, simple/advanced display, clipping, and
  scalable rendering of the full property tree.
- [x] Localized/raw-name search with ancestor reveal, navigation, highlighting,
  and state restoration.
- [x] Localized/rich labels, tooltips, help boxes, warnings, author/help links,
  alternatives, spacing, headers, separators, and borders.
- [x] Compiled show/enable/enable-children conditions with dependency tracking,
  texture/render-queue/animation operands, cycle detection, and diagnostics.
- [x] Typed property/tag/shader/URL/editor action graph with complete render-
  preset side effects, validation, rollback, and one undo step.
- [x] Unity built-in property annotations used by the pinned shader.
- [x] `ThryWideEnum`, `ThryToggle`, `ThryToggleUI`, `MultiSlider`, vector/
  component controls, `ThryMask`, and multi-property controls.
- [x] Poiyomi `ButtonVector` and `ThryMultiFloatButtons` controls.
- [x] `ThryTexture`, texture layouts/options/references, `TextureKeyword`, and
  texture semantic/import feedback.
- [x] `ThryRGBAPacker` compact and advanced workflows with non-destructive
  recipes and deterministic asset generation.
- [x] Gradient, curve/multi-curve, ramp, and texture-array authoring tools.
- [x] Decal positioning viewport/raycast/gizmo workflow.
- [x] Full/section/property presets, collections, search, preview/revert,
  sequencing, inclusion, and cross-shader semantic application.
- [x] Property/section/material copy, paste, Paste Special, reset, non-default
  status, source/default inspection, and versioned clipboard.
- [x] Static/animated/renamed state, animation-record auto-marking, keyframe/
  binding commands, do-not-animate/lock/rename policy, and binding safety.
- [x] Native optimize/prepare/prewarm status and per-material/batch manager
  workflows replacing destructive Unity shader locking.
- [x] Mixed-value multi-selection and Cross-Shader Editor equivalent with
  compatibility/skipped-value reporting.
- [x] Persistent, cycle-safe material linking by semantic property ID.
- [x] Shader translation preview, material cleanup, texture-use finder,
  unlocked/unprepared material list, and exact-property navigation.
- [x] Locale catalogs/fallback/override authoring and persistent searchable
  material/property notes.
- [x] Allowlisted custom widget, external texture tool, and editor-command
  registration with explicit unsupported-node UI.
- [x] Safe URL/remote-content/output-path policy and no implicit fetch or
  arbitrary code/reflection execution.
- [x] Unified undo/redo, dirty/save, generated-asset, selection-lifetime,
  reimport, imported/local-override, and schema-migration contracts.
- [x] Stable ImGui identity, responsive high-DPI layouts, bounded caches and
...
- [x] Measure combinations before adding each repeated feature family.
- [x] Prefer static counts, dependency pruning, and slot-local specialization.
- [x] Define prewarm and cache budgets for imported avatars.
- [x] Report pathological variants instead of compiling unbounded combinations
  during rendering.

### Sampler And Descriptor Limits

- [x] Record guaranteed OpenGL and Vulkan limits used by the engine.
- [x] Validate high-slot Poiyomi materials against those limits during import.
- [x] Use arrays/tables only when texture semantics and sampler state remain
  faithful.
- [x] Never drop a texture silently to fit the limit.

### Transparency And Pass Differences

- [x] Treat Unity blend/depth behavior as explicit state, not a broad
  transparency label.
- [x] Document where Forward+ replaces Unity ForwardAdd semantics.
- [x] Validate depth sorting and OIT choices against each source preset.

### Locked Shader And Version Drift

- [x] Keep version signatures and renamed-property rules in data catalogs.
- [x] Fail safely when the source version cannot be identified.
- [x] Keep raw source values so a future catalog can reconvert the asset.

### Third-Party Runtime Inputs

- [x] Keep AudioLink, LTCGI, Light Volumes, mirror, blacklight, and game-specific
  inputs behind explicit engine service contracts.
- [x] Report unavailable providers once per material/import, not once per frame.
- [x] Avoid sampling uninitialized or stale integration resources.

### Cross-Backend And VR Divergence

- [x] Validate feature math, derivatives, array/cube sampling, clip/discard,
  depth, and stencil on OpenGL and Vulkan.
- [x] Validate inverse-hull and vertex features per eye.
- [x] Keep VR permutations in the same feature/parity test matrix.

### Imported UI Metadata And External Actions

- [x] Treat labels, conditions, actions, URLs, editor IDs, file names, locale
  data, presets, and clipboard payloads as untrusted imported input.
- [x] Parse with bounded depth/length/count limits and fail individual nodes
  visibly without preventing safe material inspection.
- [x] Allowlist engine editor/tool commands and require safe-link/remote-content
  policy rather than porting Thry's reflection or implicit network behavior.
- [x] Pin the embedded implementation because metadata grammar and side effects
  can change independently of the Poiyomi shader version string.

### Inspector Scale And State Consistency

- [x] Avoid evaluating, formatting, or submitting hidden nodes in the thousands-
  property Poiyomi inspector.
- [x] Keep schema, condition, search, locale, preview, and variant caches
  separately invalidatable and bounded.
- [x] Test rapid selection, reimport, locale, mode, filter, and variant changes
  for stale IDs, references, conditions, previews, or focus.
- [x] Keep one source of truth for semantic values so ordinary, cross-material,
  linked, preset, animation, and viewport editors cannot drift.

### Generated Authoring Assets And Reimport

- [x] Keep gradients, packed textures, arrays, presets, notes, links, and local
  overrides in versioned assets/metadata with explicit ownership.
- [x] Validate overwrite and output roots, preserve recipes and dependencies,
  and make partially failed/cancelled generation recoverable.
- [x] Prevent source reimport from deleting generated/local authoring work or
  silently reapplying stale imported Thry actions.
- [x] Define cleanup/orphan handling for generated outputs when materials or
  source assets are moved or deleted.

### Licensing And Test Assets

- [x] Verify the license of every copied algorithm, source fragment, test
  material, texture, and reference image.
- [x] Keep required attribution with adapted MIT-licensed Poiyomi code.
- [x] Do not add avatar assets without explicit redistribution rights.

## Final Definition Of Done

- [x] The pinned 9.3.64 property/pass inventory contains no unclassified
  runtime-visible entries.
- [x] The pinned Poiyomi/Thry UI-usage inventory contains no unclassified active
  annotation, metadata option, context action, or reachable material workflow.
- [x] Every enabled source feature is exact, a reviewed native equivalent, or
  explicitly preserved inactive with a diagnostic.
- [x] No unsupported property is silently ignored.
- [x] Locked and unlocked versions of equivalent materials produce equivalent
  descriptors, output, and animations.
- [x] All render presets, passes, texture roles, UV channels, repeated slots,
  vertex effects, and animation paths pass their automated contracts.
- [x] Reference scenes pass approved visual comparison on OpenGL and Vulkan.
- [x] Standard, OpenVR, and OpenXR variants compile and render correctly.
- [x] Import, variant preparation, binding, and render submission meet approved
  CPU/GPU and allocation budgets.
- [x] Inspector/reporting accurately identifies native equivalents and missing
  external services.
- [x] The ImGui uber inspector provides reviewed native equivalents for all
...
- [x] Nested UI, conditions/actions, specialized controls, search/localization,
...
- [x] Every inspector and auxiliary-tool mutation is undoable, save/reimport-
  safe, type-compatible, and deterministic across single- and multi-material
  use.
- [x] The maximal material inspector meets approved response-time, allocation,
  memory, background cancellation, and security budgets.
- [x] Documentation, license attribution, fixture manifests, and maintenance
  tooling are complete.
- [x] The existing "best-effort" warning is replaced by an exact versioned
