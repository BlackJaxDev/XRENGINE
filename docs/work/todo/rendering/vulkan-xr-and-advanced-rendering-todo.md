...
- [ ] **ARP-V17** — Capture stationary, camera-only, object-only, and combined motion for rigid, skinned, and blendshape fixtures. Close with dense correct velocity/reactive/disocclusion output where history is valid and explicit neutral/reactive output where it is invalid; record each fixture's result separately.
  ...
  **Outstanding Advanced TSR quality validation (2026-09-24):** localized thin-edge shimmer remains on Vulkan mono, particularly at 0.67 render scale. The latest stationary comparison reduced mean edge delta from 2.644 to 2.513, but temporal standard deviation remained essentially unchanged (2.894 to 2.919), and the 95th-percentile delta worsened from 9 to 13. Native-resolution improvement and successful motion/transition spot checks do not establish upscaled quality acceptance or exclude single-frame ghosting. Keep this row open pending user-scene confirmation, dense consecutive-frame edge/disocclusion checks, live OpenGL/stereo checks of the latest changes, and GPU cost measurements with dense thin geometry. See the [final coverage validation and remaining limitations](../../investigations/rendering/2026-09-24-advanced-vulkan-tsr-jitter.md#final-coverage-validation). Earlier resource-binding acceptance does not close this quality gate.
...
- [ ] **ARP-V64** — Validate native Vulkan DLSS frame generation on a device/runtime that admits the feature. Record actual generated-frame dispatch and presentation, swapchain/resource ownership, pacing and failure diagnostics; super-resolution evidence is insufficient.
  ...
  The 2026-09-24 implementation separates frame generation from upscaler selection in both pipelines, preserves TSR with FG-only requests, and accepts display-size resolved color with render-size depth/motion. DLSS SR suppresses authored engine AA in the effective frame profile and restores the selection when disabled. Actual generated presentation has been measured in both pipelines at approximately two SDK presentations per rendered frame. Proxy-fence admission, completed screenshot-reference retention, and retirement-marker completion publication were corrected; final repeated native/proxy transitions and resize runs drain retired swapchains to zero. `slReflexSleep` now paces scene preparation. Background/occluded runs can report only one presentation per rendered frame; foreground and background evidence must remain separate. Keep broader acceptance open for dense moving-scene quality, full Reflex simulation-marker/latency validation, validation-layer/GPU capture evidence, and the remaining TSR thin-edge shimmer. See the [live investigation](../../investigations/rendering/2026-09-24-vulkan-dlss-aa-frame-generation.md) for the separate TSR-plus-FG and SR-plus-FG results and hardware scope.
