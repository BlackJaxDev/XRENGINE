...
| Basic GPU LOD table + selection path exists | `GPUScene.LODTableBuffer`, `GPURenderLODSelect.comp`, `LODRequestBuffer`, `LodTransitionBuffer` | The implementation is real, but the selection metric still needs to become projected screen-space radius for production acceptance. 2026-07-01: the request loop is now wired end-to-end behind `StreamMeshLodsOnDemand` (see Phase G0). |
...
Architectural refactors required for full virtual mesh streaming in *and* out, mirroring the virtual-texture streaming runtime (budgeted uploads, residency feedback, non-blocking readback, eviction with hysteresis). These are the whole-mesh-LOD precursors of the paged XVG residency model in [xre-virtual-geometry-design.md](../../../design/rendering/xre-virtual-geometry-design.md); G1's page-table abstraction is shared by both.
...
- [ ] **G6.** GPU LOD usage-feedback buffer. `LODRequestBuffer` only fires on residency *misses*, so stream-out is blind once a level is loaded: nothing tells the CPU a resident LOD is still in use. Add a per-logical-mesh usage word written by `GPURenderLODSelect.comp` (resolved-level bitmask, or last-used frame stamp per level) and drain it alongside the request buffer. This is the mesh analog of the texture streaming registry's visibility/recently-bound feedback and is the prerequisite for any eviction policy that doesn't thrash.
- [ ] **G7.** Fence-gated ring readback for the LOD request/usage buffers. `DrainLODRequests()` currently maps a buffer the GPU wrote this frame, which can force a sync (today only bounded by the drain-interval throttle). Replace with a small ring of copy-target buffers consumed N frames late behind `XRGpuFence`, matching the texture streaming path's non-blocking record-lock contract and the zero-readback strategy invariants in [mesh-submission-strategies.md](../../../../architecture/rendering/mesh-submission-strategies.md).
- [ ] **G8.** Mesh LOD eviction policy (stream-out). Drive `ReleaseLOD` from G6 usage feedback + G2 scoring: evict a level only after M unused frames under memory pressure, never evict LOD0 or a command's own mesh (the resident-clamp guarantees), and apply load/evict hysteresis so a level that was just streamed in cannot be evicted for a cooldown window. Failure mode to test explicitly: alternating request/evict thrash on a camera hovering at a LOD boundary.
- [ ] **G9.** Per-LOD cooked payload split + lazy CPU residency. Cooked `XRMesh` is a monolithic MemoryPack blob and `SubMesh`/`RenderableMesh` resolve and load every LOD eagerly, so GPU-side deferral saves VRAM but no RAM or IO. Split cooked LOD payloads into independently loadable units (or per-LOD asset references resolved lazily), let `RenderableMesh` defer runtime-mesh creation for non-resident levels, and route stream-in through async IO + deserialization off the render thread. After eviction, drop the CPU-side copy too (re-load from cooked data on the next request). This is the structural refactor the XVG `.xvg` chunked format generalizes.
- [ ] **G10.** Unified geometry upload budget. `RequestLODLoad` currently uploads synchronously inside the LOD-select pass. Route mesh LOD (and later page) uploads through the same bytes-per-frame budgeted scheduler as texture streaming uploads so geometry and texture streaming share one render-work budget instead of competing blindly.
- [ ] **G11.** Atlas range reclamation for evicted LODs. `ReleaseLOD` today only drops the table entry and refcount; the tier's SoA arrays never return the vertex/index range. Add free-list reuse (and periodic compaction/defrag under the existing atlas-rebuild machinery) so stream-out actually reduces resident bytes. Ties into the G1 page-table view.
- [ ] **G12.** Mesh streaming telemetry parity with texture streaming: resident bytes per tier and per logical mesh, request-to-resident latency, oldest pending request age, resolved-vs-selected level histogram (how often the clamp degraded quality), eviction and thrash counters. Extends G5 and feeds the XVG diagnostics panel design.

Acceptance: dynamic tier evicts under stress without thrashing visible-frame data. Sparse POC runs B1 with virtualized vertex/index buffers without correctness regression. Profiler exposes upload backpressure metrics. Mesh LOD streaming holds a fixed geometry budget on a LOD-diverse scene with camera sweeps: levels stream in and out with no clamp-to-missing artifacts, no request/evict thrash, and resident atlas bytes actually decrease after eviction.
