...
- [x] **E4.** Production meshlet capability honesty. Split direct mesh-task dispatch from production indirect/count meshlet dispatch; model dialects as `None`, `OpenGLNV`, `OpenGLEXT`, and `VulkanEXT`; make unsupported `GpuMeshlet` visibly fall back to `GpuIndirectZeroReadback`. Done 2026-05-19 in [gpu-meshlet-zero-readback-rendering-todo.md](gpu-meshlet-zero-readback-rendering-todo.md) Phase 1.
...
- [x] **E9.** Material/pass parity and validation for `GpuMeshlet`: opaque, alpha-tested, shadow/depth, velocity, skinned, stereo, bindless/descriptor material table, fallback-only environments, and zero-readback stats. Done 2026-05-19 at renderer/source-contract scope: runtime meshlet dispatch uses the shared material-table policy where supported, binds `MaterialStateBuffer`/texture handle tables, disables unsafe stereo Hi-Z, routes override/depth-normal/shadow/skinned unsupported cases through traditional zero-readback indirect with visible warnings, and publishes meshlet counters/profile-capture stats. Hardware visual parity, fallback smoke, 100K/readback stress, performance, and soak validation remain tracked by Phase H plus [gpu-meshlet-zero-readback-rendering-todo.md](gpu-meshlet-zero-readback-rendering-todo.md) Phase 10.
