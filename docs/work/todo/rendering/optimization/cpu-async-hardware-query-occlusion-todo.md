...
Owner: Rendering
Status: Active TODO
Target Branch: `rendering-cpu-async-query-occlusion`
...
- [ ] Create dedicated branch `rendering-cpu-async-query-occlusion`.
- [ ] Capture the current moving-camera failure in a high-occlusion scene with
  `GpuOcclusionCullingMode=CpuQueryAsync`.
- [ ] Capture the same baseline in at least one VR mode:
  OpenVR two-pass, OpenVR single-pass stereo, scene-only emulated VR, or
  Monado-backed OpenXR if available.
- [ ] Record whether the failure is CPU direct, GPU-dispatch `CpuQueryAsync`, or
  both.
- [ ] Record per-frame values for:
...
- [ ] Add temporary or durable counters for camera invalidation tier, temporal
  resets, pending query count, average query latency in frames, query budget
  exhaustion, and forced-visible reason.
- [ ] Confirm whether repeated `ResetTemporalState()` / `ResetTemporalOcclusionState()`
  is the cause of "moving camera renders every mesh".
- [ ] Confirm whether VR camera object identity, predicted-vs-late pose updates,
  asymmetric projection changes, or per-eye pass sequencing are causing
  first-seen/seed-visible behavior every frame.

Acceptance criteria:

- [ ] The failure has a short profiler/log excerpt showing why meshes are being
  rendered.
- [ ] The doc records which run mode and backend reproduced it.
- [ ] At least one stereo/VR capture records active runtime, stereo mode,
  render path, pose policy, and whether the query state is mono, per-eye, or
  stereo-pair scoped.

## Phase 1 - Stop Global Reset On Normal Motion

- [ ] Replace binary camera invalidation with explicit motion tiers:
  `Stable`, `SmallMotion`, `MediumMotion`, `LargeMotion`, and `CameraCut`.
- [ ] Include a separate `VrHeadPoseMotion` tier or equivalent policy so
  normal HMD prediction, IPD eye offset, and late-latch pose changes never act
  like global invalidation.
- [ ] Preserve per-command query history across `SmallMotion` and
  `MediumMotion`; mark it dirty or lower-confidence instead of setting
  `LastAnySamplesPassed=true`.
- [ ] For `LargeMotion`, keep history but shorten occluded revalidation age and
  expand recovery-query budget.
- [ ] Reserve full conservative-visible behavior for `CameraCut`, projection
  discontinuity, missing camera state, or explicit diagnostic force-visible.
- [ ] Apply the same policy to the GPU-dispatch `CpuQueryAsync` scaffold so
  `HasSignificantCameraChange` does not blindly clear all temporal state.
- [ ] Replace camera-object hash identity with a stable occlusion view key that
  ...
  every frame.
- [ ] Keep scene mutation keyed by `StableQueryKey`: adding/removing commands
  should orphan only removed state, not invalidate unrelated commands.

Acceptance criteria:

- [ ] Continuous editor-camera translation/rotation does not reset all query
  states every frame.
- [ ] Continuous HMD head motion does not reset all query states every frame.
- [ ] A true camera cut still produces a visible, counted conservative frame.
- [ ] Unit tests cover small motion, medium motion, large motion, camera cut,
...
- [ ] Extend query state from `LastAnySamplesPassed` plus hysteresis to an
...
- [ ] Track `LastVisibleFrame`, `LastOccludedFrame`, `LastQueryFrame`,
  `PendingSinceFrame`, `ConsecutiveVisibleFrames`, and
  `ConsecutiveOccludedFrames`.
- [ ] Separate query reasons:
...
- [ ] Add a fixed per-pass query budget split between visible-demotion and
  occluded-recovery probes.
- [ ] Budget CPU query work per stereo frame, not per eye, so sequential
  left/right rendering does not silently double proxy-query cost.
- [ ] Prioritize occluded-recovery probes by screen-space area, age since last
  visible, camera-motion tier, near frustum edge/reveal risk, and distance.
- [ ] In VR, compute screen-space priority from the max projected size or reveal
  risk across both eyes.
- [ ] Prioritize visible-demotion probes only for objects worth demoting:
  large screen area, high draw cost, recent occluder changes, or stale query
  age.
- [ ] Stop submitting a proxy query for every currently visible mesh every time
  its previous query resolves.

Acceptance criteria:

- [ ] Query submissions stay below the configured budget in a dense scene.
- [ ] Occluded objects continue to get bounded recovery opportunities while the
  camera moves.
- [ ] VR query submissions stay below the configured per-frame stereo budget
  and do not scale by two unless explicitly configured.
- [ ] Visible objects no longer starve recovery probes.

## Phase 3 - CPU Two-Stage Query Flow

- [ ] Formalize the current CPU direct draw into two stages:
  predicted-visible color/depth draw, then depth-only proxy-query stage after
  complete opaque depth exists.
- [ ] Ensure visible-demotion and occluded-recovery proxy queries both run in
  the deferred proxy stage.
- [ ] For two-pass eye rendering, ensure each proxy query is recorded against
  the same eye depth target and camera/projection that will consume the result.
- [ ] For shared stereo or single-pass rendering, either issue a stereo-safe
...
- [ ] If a render mode cannot produce stereo-safe query results, explicitly
  force visible for that mode and report the unsupported reason in telemetry.
- [ ] Resolve available query results only at pass/frame boundaries, never in
  the middle of a draw loop if that could accidentally block or reorder state.
- [ ] Promote an occluded object to visible immediately on the next frame after
  a recovery query reports samples passed.
- [ ] Demote a visible object only after the configured zero-sample confidence
  threshold, so a single late/stale result does not pop it out.
- [ ] Keep near-plane-unsafe proxy bounds force-visible, with a telemetry
  reason and a test.

Acceptance criteria:

- [ ] The CPU direct path has a documented pass contract matching the code:
  normal draws first, proxy queries after complete depth, result consumption in
  a later frame.
- [ ] No proxy draw writes color or persistent depth.
- [ ] No query result is overwritten by reusing a pending query object.
- [ ] No left-eye-only result can cull a command from a right-eye or shared
  stereo draw.

## Phase 4 - Hierarchical Or Batched Queries

- [ ] Reuse or add a CPU visibility hierarchy over render commands, keyed by
  stable command identity and pass/camera eligibility.
- [ ] Include stereo eligibility in hierarchy keys so mono desktop, per-eye VR,
  and stereo-pair query state do not cross-contaminate each other.
- [ ] Query coarse groups for previously occluded regions before individual
  child meshes.
- [ ] Traverse or schedule groups front-to-back where the current pass ordering
  allows it.
- [ ] Add CHC-style batching for previously invisible groups so query state
  changes and proxy draws are grouped.
- [ ] Pull visibility up/down the hierarchy from child results so large occluded
  areas can be skipped with one query.
- [ ] Fall back to per-command probes only for small groups, dynamic outliers,
  or debug modes.

Acceptance criteria:

- [ ] Query count scales with visible/occluded boundary regions rather than the
  total number of mesh commands.
- [ ] Dense static occlusion scenes perform fewer proxy draws than the current
  per-mesh approach.
- [ ] Stereo static occlusion scenes reuse hierarchy work across eyes where safe
  and preserve visible-if-either-eye semantics.
- [ ] Hierarchy updates do not allocate in steady-state render hot paths.

## Phase 5 - VR And Stereo Contract

- [ ] Define an `OcclusionViewKey` / `OcclusionViewScope` contract that covers:
  `MonoDesktop`, `EditorDesktopWhileVr`, `VrLeftEye`, `VrRightEye`,
  `VrStereoPair`, `VrSinglePassStereo`, `VrFoveatedView`, and `MirrorOnly`.
- [ ] Map OpenVR two-pass to either per-eye query state or a stereo-pair state
  with OR-combined per-eye results.
- [ ] Map OpenVR single-pass stereo to stereo-pair visibility only; do not use
  a left-eye-only query to decide a multiview draw.
- [ ] Map OpenXR combined-stereo `CollectVisible` and shared
  `MeshRenderCommandsOverride` to a stereo-pair visibility scope.
- [ ] Account for OpenXR predicted `CollectVisible` pose and late render pose:
  normal predicted-to-late deltas should reduce confidence or increase
  recovery priority, not reset all state.
- [ ] Respect `OpenXrCollectVisiblePosePolicy=PaddedFrustum` by treating the
  padded frustum as conservative visibility input; query probes still use the
  actual eye render depth.
- [ ] Keep desktop editor and desktop mirror occlusion state separate from HMD
  eye state unless the mirror is a pure blit of eye textures.
- [ ] Add per-eye/per-stereo-pair telemetry: tested, drawn, culled, forced
  visible, query submitted, query resolved, and unsupported stereo query mode.
- [ ] Add tests that an object visible only to the right eye is not culled by a
  left-eye or cyclopean query result in shared stereo rendering.
- [ ] Add tests that transient eye-camera object replacement does not make every
  command look first-seen.
- [ ] Add tests that normal OpenXR predicted-to-late pose deltas do not trigger
  camera-cut invalidation.

Acceptance criteria:

- [ ] VR sequential eye rendering, single-pass stereo, and OpenXR combined
  stereo collection have explicit, documented occlusion view scopes.
- [ ] Shared stereo draws are culled only when both eyes are classified
  occluded or a stereo-safe query proves no samples pass in either eye.
- [ ] Unsupported stereo query modes are conservative-visible with telemetry,
  not silently mono-cull.
- [ ] VR desktop mirror rendering does not poison or consume HMD eye occlusion
  state.

## Phase 6 - GPU-Dispatch `CpuQueryAsync` Cleanup

- [ ] Decide whether GPU-dispatch `CpuQueryAsync` remains supported or becomes
  a diagnostic-only OpenGL path behind explicit settings.
- [ ] If retained, remove readback-dependent behavior from production GPU
  submission modes; zero-readback modes must not depend on CPU enumeration of
  GPU-visible command buffers.
- [ ] Apply the same motion-tier and confidence model used by CPU direct.
- [ ] Keep query submission capped and prioritize recovery over demotion.
- [ ] Make Vulkan support explicit: either wire command-buffer query recording
  correctly or report unsupported with diagnostics instead of pretending the
  path is active.
- [ ] For GPU-dispatch VR, use the active `GPUViewSet` / per-view draw-count
...
- [ ] `GpuIndirectZeroReadback` never silently degrades to a CPU query path that
  needs current-frame count readback.
- [ ] GPU-dispatch telemetry distinguishes unsupported backend, readback
  disabled, query budget exhausted, and active query filtering.
- [ ] GPU-dispatch VR telemetry distinguishes mono, per-eye, stereo-pair, and
  unsupported query scopes.

## Phase 7 - Settings, Telemetry, And Editor UI

- [ ] Add settings for query budget, visible-demotion budget fraction,
...
- [ ] Update the ImGui Occlusion panel so CPU query, CPU SOC, and GPU Hi-Z show
  separate health summaries.
- [ ] Add warnings when CPU query occlusion is configured but cannot cull due to
  unsupported backend, no camera, shadow pass, transparent pass, readback mode,
  or repeated conservative-visible invalidation.
- [ ] Document environment/settings names if any new user-facing controls are
  added.

Acceptance criteria:

- [ ] A user can tell from the profiler panel whether CPU async query occlusion
  ...
  visibility.
- [ ] New settings have defaults that preserve correctness and do not introduce
  compiler warnings.

## Phase 8 - Validation

- [ ] Add deterministic unit tests in `XREngine.UnitTests/Rendering` for motion
...
- [ ] Add source-contract tests for the GPU-dispatch `CpuQueryAsync` policy if
...
- [ ] Merge `rendering-cpu-async-query-occlusion` back into `main` after
  implementation, validation evidence, docs updates, and profiler-panel updates
  are complete.
