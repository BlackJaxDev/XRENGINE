...
- [ ] In `GLMeshRenderer.GetCombinedProgram(...)`, require `vertexProgram.Use()`
  to succeed before returning `true`.
- [ ] If `Use()` fails, set both out parameters to `null`, log a throttled
...
- [ ] Log per-camera CPU collection counts with command id, mesh id, material
