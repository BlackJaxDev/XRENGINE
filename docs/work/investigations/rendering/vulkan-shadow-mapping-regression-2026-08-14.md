...
**Implementation checkpoint 1:** Work began on 2026-08-14 after explicit user approval. CPU-direct requests now retain the enqueue-time resolved material, expanded instance count, transform id, and one cache-shared immutable `VulkanMeshDrawViewSnapshot` captured while the shadow scopes are active. Materialization consumes that packet instead of re-reading cleared global material, layered-shadow, camera, or target state. The compatibility signature is derived from the same packet. Immutable uniform restoration now covers both instanced and geometry layered material kinds; point geometry receives both matrix aliases and an exact mask derived from the captured logical face indices. Sequential point rendering now owns six stable per-face framebuffers instead of mutating one shared framebuffer before Vulkan snapshots the native subresource. The Vulkan rendering project builds with zero warnings and zero errors. Runtime, RenderDoc, and receiver-layout validation for this checkpoint are pending; this entry is not a correctness claim.

**Implementation checkpoint 2:** The first isolated Vulkan CPU-direct runtime session (`vk-shadow-impl-sequential-20260814`) was run with validation enabled and all three atlases disabled. Settled sequential directional rendering produced a valid Sponza `FinalPostProcessOutputTexture`; toggling `CastsShadows` changed the captured final target from average intensity `0.07569` to `0.11538`, confirming that the active receiver samples the current standalone directional resource. The settled sequential interval was validation-clean. Recreating that light's shadow resource by toggling shadow casting did, however, briefly publish an unresolved old `ShadowMapArray` descriptor before the replacement image existed, confirming the missing receiver-readiness/generation contract. A dynamically added sequential point light then reported `LastRenderedShadowFaceMask=0x39`, not all six faces. While its shaders and descriptors were still compiling, final-target capture exposed incompatible descriptor-set layouts and dynamic-offset counts; the next frame reused pending scene/UI command buffers, overflowed the graphics timeline semaphore state, and lost the Vulkan device at frame 2741. The session was stopped through its named manager. Evidence is in `Build/_AgentValidation/00000000-000000-shared/mcp-sessions/20260814-181148-vk-shadow-impl-sequential-20260814/logs/` and `Build/_AgentValidation/20260814-110450-vulkan-shadow-phase0/mcp-captures/implementation-sequential/directional/`. The device loss prevents treating this session as point/spot validation. It elevates packet-compatible descriptor allocation/dynamic-offset identity and frame-slot command-buffer completion from audit risks to implementation blockers alongside shadow resource readiness.

**Implementation checkpoint 3:** Code review localized the point-light failure to frame-wide descriptor prewarming, not a mapped-arena race. One renderer may prewarm several program variants; the final variant left active could therefore differ from the frozen draw's pipeline layout even though the request itself retained the correct material. Recording now restores the exact enqueue-time view/materialization identity, reselects the captured program and binding snapshot after frame-wide prewarm, and reacquires descriptors plus frame-source bindings for that program before numeric state is sealed. The immutable view cache key now includes descriptor view-family identity, and each drained request derives its materialization snapshot from its own captured context instead of one last-active context for the whole drain. The Vulkan rendering project builds with zero warnings and zero errors after this repair.

The follow-up isolated session (`vk-shadow-impl-sequential3-20260814`) ran Vulkan validation, command-buffer labels, and forced `CpuDirect` with every atlas disabled. A dynamically added sequential point light reached a nonzero live six-face writer; at the observed active checkpoint its relevance and rendered masks both equaled `0x22`, proving that the partial mask was intentional per-camera face relevance rather than missing face recording. A dynamically added sequential spot light reported `ShadowFrustumRelevant=true` and a non-null standalone `ShadowMap`. Fixed-camera final-target pairs changed for both lights when shadow casting changed: point hashes `5320E1C9...` versus `678A1A6E...` and spot hashes `077EE6EA...` versus `D8B48851...`; the spot average intensity changed from `0.20716393` with shadows to `0.20875084` without shadows. These pairs establish live receiver changes and stable recording, but they are not yet controlled pixel-equivalence tests against layered or atlas routes. Across thousands of frames and both dynamic light warmups, the session produced zero descriptor-layout/dynamic-offset errors, zero pending-command-buffer reuse errors, and zero device-loss reports, then stopped normally through its named session manager. Evidence is under `Build/_AgentValidation/00000000-000000-shared/mcp-sessions/20260814-183454-vk-shadow-impl-sequential3-20260814/` and `Build/_AgentValidation/20260814-110450-vulkan-shadow-phase0/mcp-captures/implementation-sequential/{point2,spot2}/`. This closes the standalone directional/point/spot CPU-direct stability gate. It does not close startup image-layout VUIDs, atlas freshness/storage, grouped rendering, layered equivalence, or warmed pacing validation.

**Implementation checkpoint 4:** Atlas storage/publication and group scheduling were repaired before the first live grouped-atlas probe. Frame data now separates layout, content, storage, and overall publication generations; point/spot receiver state includes exact texture identity and sampleability; recorded same-command-stream completions are overlaid for receiver publication without being mislabeled GPU fence completion; array storage is stable for the configured capacity; point-group ownership suppresses only exact members; failed/deferred entries no longer hide unrelated later work; and one explicitly reported critical budget bypass remains available. Directional and point grouped Vulkan capability gates are enabled, indexed viewport/scissor slots remain aligned with packed matrices, and UV transforms address inner-rectangle texel centers.

The isolated forced-`CpuDirect` probe `vk-shadow-impl-atlas1-20260814` reached `EffectiveCascadeShadowRenderBackend=AtlasPage` and `EffectiveCascadeShadowRenderMode=InstancedLayered`. Live profiler telemetry remained Vulkan-validation-clean (`message_count=0`, `error_count=0`, descriptor binding failures zero), and the session stopped normally through its named manager. It is not yet a grouped-shadow correctness pass: the session-local logging directory remained empty after the `-NoBuild` launch, the MCP getter for the composite `ShadowAtlasDiagnostic` repeatedly failed to return while scalar getters remained responsive, and the selected exterior/focus views did not provide known-lit/known-shadowed controls. Captures and the resource inventory are under `Build/_AgentValidation/20260814-110450-vulkan-shadow-phase0/mcp-captures/implementation-atlas/` and `mcp-output/atlas1-resources.json`; session metadata is under `Build/_AgentValidation/00000000-000000-shared/mcp-sessions/20260814-191447-vk-shadow-impl-atlas1-20260814/`. The next atlas run must enable file logging, expose scalar render/publication diagnostics that cannot hang serialization, use a fixed interior camera, and compare grouped output to the already validated sequential receiver before accepting publication or physical tile routing.

**Implementation checkpoint 5:** The remaining layered/performance contracts and two concrete Vulkan layout defects were repaired in source before the next live run. Layered draws now carry a conservative per-caster cascade/face relevance mask through the immutable request, pending draw, auto-uniform publication, generated instanced vertex shader, and directional/point geometry shaders, so a union caster is emitted only to targets its world bounds can intersect. The compatibility-bypassing Uber shortcut was removed, geometry variants restore the same immutable matrices/indices/masks as instanced variants, and live layered scopes now use a bounded allocation-free LIFO snapshot stack that restores outer directional and point payloads after nested scopes. The unused `$CpuDirectDynamicData` type and all three per-draw capture attempts were removed. Atlas filtering now has an explicit no-gutter-read contract: every local sample is clamped to `[0,1]`, while the UV transform maps those endpoints to the first and last rendered texel centers; no filter kernel can address untouched gutter texels, so no full-gutter clear or dilation is required for the current filters. Scalar per-light atlas diagnostics were added for remote inspection without serializing the composite record.

The startup layout audit then identified two independent command-recording defects. Bindless sampled-image publication could bake a transient attachment or transfer layout into a combined-image-sampler descriptor; it now resolves the descriptor's stable sampled-consumption layout. Separately, executing a secondary merged its final image journal into the primary journal but left the primary recording batch's faster lookup index at the pre-secondary state. Later primary barriers therefore selected stale `oldLayout` values even though the journal contained the correct post-secondary state. Secondary execution now mirrors each touched subresource into that lookup index under the established batch-to-image-journal lock order, without duplicating submission deltas. These changes directly target the observed `VUID-vkCmdDraw-None-09600` descriptor-layout mismatches and `VUID-VkImageMemoryBarrier-oldLayout-01197` barriers. The Vulkan rendering project builds with zero warnings and zero errors. This checkpoint is source/build evidence only: the next named Vulkan session must prove that both VUID classes are absent and that atlas writes transition to descriptor-compatible reads before receiver draws. No unit test was added or run; test changes remain held until explicit test clearance after live feature validation.

**Implementation checkpoint 6:** The fresh isolated session `vk-shadow-impl-layout2-20260814` ran the current implementation with Vulkan validation, command-buffer labels, forced `CpuDirect`, and a directional depth atlas. The light reached `AtlasPage` with four resident cascades and both requested layered writers. After camera-forced redraws at the same interior Sponza view, geometry and instanced output were visually equivalent and had close final-target averages (`0.21074` and `0.20901`). Camera motion advanced `ShadowAtlasLastRenderedFrame`, so the atlas did not trail the moved view in this probe. Changing `CascadeShadowRenderMode` without moving the camera exposed a different deterministic transition defect: the component cleared its published cascade slots, the manager legitimately reused the unchanged content hash without redrawing, and the receiver remained overbright/unshadowed even after a three-second settle. Source was corrected to preserve valid resident samples across writer-only changes and invalidate only the cached caster collection for the next real refresh. Evidence is under `Build/_AgentValidation/20260814-110450-vulkan-shadow-phase0/mcp-captures/implementation-final/directional-atlas-{instanced,geometry}/`.

This session also disproved the validation-clean expectation of checkpoint 5. Its raw startup log contains 10 `VUID-VkImageMemoryBarrier-oldLayout-01197` and 10 `VUID-vkCmdDraw-None-09600` reports even though the live profiler's current-frame counters later returned to zero. The errors span ordinary pipeline resources and the swapchain as well as shadow-adjacent recording, and occur during first-frame pipeline/resource warmup. The descriptor canonicalization and secondary fast-index merge were therefore insufficient; they remain hypotheses until a lights-off fresh-process control and exact command-stream ownership trace isolate whether either change is useful. Do not use current-frame profiler zeroes as historical validation evidence, and do not accept same-submission atlas safety until the raw log is clean.
