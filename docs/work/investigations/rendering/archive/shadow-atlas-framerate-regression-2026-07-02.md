...
Status: Closed as historical evidence on 2026-08-04. The July regression and
its attempted fixes are retained here, but this document owns no current work.
The active directional cascade, atlas, camera-motion, and acceptance work is
consolidated in the
...
- Interactive camera-motion confirmation, directional atlas inspection,
  primary-recording spikes, and the final user-facing acceptance pass moved to
  the active directional-light investigation linked above.
- The automated camera-move regression remains deferred until the live feature
  is correct and the user explicitly clears regression-test work.
- Startup-only swapchain warnings, imported-texture residency dirties, and
  CPU-query counter semantics are separate concerns and do not keep this
  historical shadow-atlas investigation open.

No remaining work is owned by this document.
