...
- `DefaultRenderPipeline` and `DefaultRenderPipeline2` expose schema-backed camera post-processing stages. The `Imaging` category covers tonemapping, color grading, and vignette, and the hue/saturation/brightness grading path runs before final HDR or SDR output so the same controls work with HDR output enabled.
