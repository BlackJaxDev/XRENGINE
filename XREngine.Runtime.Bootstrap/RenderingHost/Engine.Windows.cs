...
{
    /// <summary>
    /// Bootstrap-owned window and viewport composition.
    /// </summary>
    public static partial class Engine
    {
        ...
        {
            RuntimeApplicationCapabilities capabilities = RuntimeApplicationCapabilityServices.Current;
            if (capabilities.IsConfigured && !capabilities.AllowsWindows && windows.Count > 0)
            {
                throw new InvalidOperationException(
                    "The active application profile does not install local window services. " +
                    "Use a desktop application profile or keep the startup-window list empty.");
            }

            WindowStartupValues[] startupWindowValues = new WindowStartupValues[windows.Count];
            for (int i = 0; i < windows.Count; i++)
                startupWindowValues[i] = ToRuntimeWindowValues(windows[i]);
            bool splitWindowPumpStarted = RuntimeWindowApplicationServices.Current.TryStartForStartupWindows(startupWindowValues);
        ...
        {
            RuntimeApplicationCapabilities capabilities = RuntimeApplicationCapabilityServices.Current;
            if (capabilities.IsConfigured && !capabilities.AllowsWindows)
            {
                throw new InvalidOperationException(
                    "The active application profile does not install local window services.");
            }
            ...
            bool createOnWindowPumpHost = RuntimeWindowApplicationServices.Current.ShouldCreateWindowOnHost(
                ToRuntimeWindowValues(windowSettings, interactiveResizeStrategy));
        ...
        private static WindowStartupValues ToRuntimeWindowValues(
            GameWindowStartupSettings settings,
            EInteractiveWindowResizeStrategy? resolvedResizeStrategy = null)
            => new(
                ...
                settings.LocalPlayers,
                settings.VSync,
                settings.TransparentFramebuffer,
                settings.OutputHDR,
                settings.UseNativeTitleBar,
                (resolvedResizeStrategy ?? ResolveInteractiveResizeStrategy(settings)) switch
                {
        ...
        public static void RemoveWindow(XRWindow window)
        {
            ...
            RuntimeWindowApplicationServices.Current.UnregisterWindow(window);
