...
public static class BootstrapPhase524bValidationBuilder
{
    ...
    private const string DesktopCameraNodeName = "Editor View";
    ...
    // Keep the SPS motion oracle inside a deterministic near-field depth band.
    // Monado's initial stationary pose can vary slightly between service starts;
    // a farther headset-relative cube could land behind unit-world geometry and
    // turn a valid motion-vector test into an occlusion test.
    private static readonly Vector3 HeadsetRelativeMovingSentinelCenter = new(-0.22f, 0.15f, -0.65f);
    private static readonly Vector3 HeadsetRelativeMovingSentinelAmplitude = new(0.06f, 0.03f, 0.0f);
    private static readonly Vector3 DesktopCameraRelativeTopEdgeSentinelTranslation = new(0.0f, 0.60f, -1.20f);
    private static readonly Vector3 HeadsetRelativeTopEdgeSentinelTranslation = new(0.0f, 1.05f, -1.20f);
    ...
    public static void AddValidationWorkload(SceneNode rootNode)
    {
        ...
        SceneNode? desktopCameraNode = rootNode.FindDescendantByName(DesktopCameraNodeName);
        SceneNode desktopSentinelParent = desktopCameraNode ?? validationRoot;
        AddBox(
            desktopSentinelParent,
            DesktopTopEdgeSentinelNodeName,
            desktopCameraNode is null
                ? WorldRelativeTopEdgeSentinelTranslation
                : DesktopCameraRelativeTopEdgeSentinelTranslation,
            new Vector3(0.80f, 0.12f, 0.08f),
        ...
        SceneNode desktopMovingSentinel = AddBox(
            ...
            desktopCameraNode is null ? new Vector3(0.70f) : new Vector3(0.16f),
        ...
        scenario.Configure(
            ...
            desktopMovingSentinelIsCameraRelative: desktopCameraNode is not null);
    ...
    public static Vector3 CalculateTemporalMovingSentinelTranslation(int sequenceFrame, bool headsetRelative)
    {
        ...
        {
            if (Phase524bTemporalScenarioDiagnostics.TryGetBoundaryCaptureMotionTick(out uint boundaryTick))
            {
                return headsetRelative
                    ? CalculateHeadsetRelativeMovingSentinelTranslation(boundaryTick)
                    : CalculateMovingSentinelTranslation(boundaryTick);
            }
