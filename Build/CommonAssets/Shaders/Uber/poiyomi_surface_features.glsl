#ifndef XRENGINE_POIYOMI_SURFACE_FEATURES_GLSL
#define XRENGINE_POIYOMI_SURFACE_FEATURES_GLSL

// Portable XRENGINE implementation of behavior specified by Poiyomi Toon
// 9.3.64. Source semantics are adapted under the MIT license; see
// docs/licenses/Poiyomi-Toon-9.3.64-MIT.txt.

float poiHash21(vec2 value)
{
...
vec4 poiSampleStochastic(sampler2D source, vec2 uv, int mode)
{
    ...
    vec2 offsetA = vec2(poiHash21(cellA), poiHash21(cellA + 17.0));
    vec2 offsetB = vec2(poiHash21(cellB), poiHash21(cellB + 17.0));
    vec2 offsetC = vec2(poiHash21(cellC), poiHash21(cellC + 17.0));
...
float poiApplyExtendedAlpha(float alpha, ToonMesh mesh)
{
    ...
    if (_AlphaDither > 0.0)
    {
        ...
        float objectNoise = poiHash21(floor(mesh.localPos.xy * 64.0));
