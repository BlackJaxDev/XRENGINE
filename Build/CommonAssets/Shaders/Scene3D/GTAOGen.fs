...
vec3 ComputeSliceViewDirection(vec2 uv, float depth, vec3 centerPos, vec2 sliceDirection)
{
    vec2 texelSize = 1.0f / vec2(textureSize(DepthView, 0));
...
void main()
{
    ...
    for (int sliceIndex = 0; sliceIndex < sliceCount; ++sliceIndex)
    {
        ...
        vec3 planeSide = ComputeSliceViewDirection(uv, depth, centerPos, direction);
        ...
        for (int stepIndex = 0; stepIndex < stepCount; ++stepIndex)
        {
            ...
            forwardHorizon = UpdateHorizonAngle(forwardHorizon, centerPos, uv + offset, radiusVS, falloffStart, planeForwardBase, planeSide, planeNormal);
            backwardHorizon = UpdateHorizonAngle(backwardHorizon, centerPos, uv - offset, radiusVS, falloffStart, planeForwardBase, -planeSide, planeNormal);
        }
