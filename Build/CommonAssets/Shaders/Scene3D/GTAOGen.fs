...
void main()
{
    ...
    vec2 noiseCoord = XRENGINE_ScreenNoiseCoord(gl_FragCoord.xy, vec2(0.0), vec2(textureSize(DepthView, 0)));
    float baseAngle = (InterleavedGradientNoise(noiseCoord) - 0.5f) * (PI / float(sliceCount));
    float stepJitter = InterleavedGradientNoise(noiseCoord + vec2(17.0f, 43.0f));
