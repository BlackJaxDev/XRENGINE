...
void ComputeNeighborhoodBounds(vec2 uv, out vec3 minColor, out vec3 maxColor)
{
    ...
    for (int x = -1; x <= 1; ++x)
    {
        for (int y = -1; y <= 1; ++y)
        {
            vec3 sampleColor = texture(PostProcessOutputTexture, uv + vec2(float(x), float(y)) * SourceTexelSize).rgb;
            mean += sampleColor;
...
float EvaluateDepthDiscontinuity(vec2 uv)
{
    ...
    for (int i = 0; i < 4; ++i)
    {
        float sampleDepth = texture(DepthView, uv + offsets[i] * SourceTexelSize).r;
...
void main()
{
    ...
    vec3 currentFiltered = SampleCatmullRomFilteredColor(PostProcessOutputTexture, uv, SourceTexelSize);
    ...
    if (canUseHistory)
    {
        ...
        if (abs(historyDepth - currentDepth) <= DepthRejectThreshold)
            historyColor = SampleCatmullRomFilteredColor(TsrHistoryColor, historyUV, HistoryTexelSize);
        else
            canUseHistory = false;
    }
    ...
    vec3 resolved = mix(currentFiltered, clippedHistory, historyWeight);
    OutColor = vec4(resolved, 1.0f);
}
