...
void main()
{
        ...
        if (DeferredDebugMode > 0)
        {
                ...
                if ((DeferredDebugMode >= 6 && DeferredDebugMode <= 9) || (DeferredDebugMode >= 11 && DeferredDebugMode <= 18)) { OutLo = vec4(InLo, 1.0f); return; } // Directional debug from the light shader
