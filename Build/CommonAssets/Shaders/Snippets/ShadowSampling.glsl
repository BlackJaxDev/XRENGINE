...
vec3 XRENGINE_ContactShadowWorldPosFromDepth(
    ...
    int depthMode)
{
    vec3 viewSpacePosition = XRENGINE_ContactShadowViewPosFromDepth(depth, uv, inverseProjMatrix, depthMode);
...
float XRENGINE_SampleShadowMapPCF(sampler2D shadowMap, vec3 shadowCoord, float bias, int kernelSize)
{
    ...
    for (int x = -halfKernel; x <= halfKernel; ++x)
    {
        for (int y = -halfKernel; y <= halfKernel; ++y)
        {
            ...
            shadow += XRENGINE_ShadowLit(shadowCoord.z, pcfDepth, bias);
        }
    }
...
float XRENGINE_SampleShadowMapTent4(sampler2D shadowMap, vec3 shadowCoord, float bias, float filterRadius)
{
    ...
    lit += XRENGINE_ShadowLit(shadowCoord.z, texture(shadowMap, shadowCoord.xy + vec2(-0.5, -0.5) * radius).r, bias);
    lit += XRENGINE_ShadowLit(shadowCoord.z, texture(shadowMap, shadowCoord.xy + vec2(0.5, -0.5) * radius).r, bias);
    lit += XRENGINE_ShadowLit(shadowCoord.z, texture(shadowMap, shadowCoord.xy + vec2(-0.5, 0.5) * radius).r, bias);
    lit += XRENGINE_ShadowLit(shadowCoord.z, texture(shadowMap, shadowCoord.xy + vec2(0.5, 0.5) * radius).r, bias);
...
float XRENGINE_SampleShadowMapArrayTent4(sampler2DArray shadowMap, vec3 shadowCoord, float layer, float bias, float filterRadius)
{
    ...
    lit += XRENGINE_ShadowLit(shadowCoord.z, texture(shadowMap, vec3(shadowCoord.xy + vec2(-0.5, -0.5) * radius, layer)).r, bias);
    lit += XRENGINE_ShadowLit(shadowCoord.z, texture(shadowMap, vec3(shadowCoord.xy + vec2(0.5, -0.5) * radius, layer)).r, bias);
    lit += XRENGINE_ShadowLit(shadowCoord.z, texture(shadowMap, vec3(shadowCoord.xy + vec2(-0.5, 0.5) * radius, layer)).r, bias);
    lit += XRENGINE_ShadowLit(shadowCoord.z, texture(shadowMap, vec3(shadowCoord.xy + vec2(0.5, 0.5) * radius, layer)).r, bias);
...
{
    float penumbra = abs(receiverDepth - avgBlockerDepth) / max(abs(avgBlockerDepth), 0.0001) * max(lightSourceRadius, 0.0);
...
float XRENGINE_BlockerSearch2D(sampler2D shadowMap, vec2 uv, float receiverDepth, float searchRadius, int sampleCount)
{
    ...
    for (int i = 0; i < XRENGINE_MaxVogelShadowTaps; ++i)
    {
        ...
        float sampleDepth = texture(shadowMap, sampleUv).r;
        if (XRENGINE_IsShadowBlocker(sampleDepth, receiverDepth))
        {
    ...
    int kernelSize)
{
    ...
    for (int x = -halfKernel; x <= halfKernel; ++x)
    {
        for (int y = -halfKernel; y <= halfKernel; ++y)
        {
            ...
            shadow += XRENGINE_ShadowLit(localCoord.z, pcfDepth, bias);
        }
    }
    ...
    float bias)
{
    ...
    return XRENGINE_ShadowLit(localCoord.z, depth, bias);
}
    ...
    float filterRadius)
{
    ...
    for (int i = 0; i < 16; ++i)
    {
        ...
        lit += XRENGINE_ShadowLit(localCoord.z, sampleDepth, bias);
    }
    ...
    float filterRadius)
{
    ...
    lit += XRENGINE_ShadowLit(localCoord.z, XRENGINE_ReadShadowAtlasDepth(shadowMap, localCoord.xy + vec2(-0.5, -0.5) * radius, uvScaleBias), bias);
    lit += XRENGINE_ShadowLit(localCoord.z, XRENGINE_ReadShadowAtlasDepth(shadowMap, localCoord.xy + vec2(0.5, -0.5) * radius, uvScaleBias), bias);
    lit += XRENGINE_ShadowLit(localCoord.z, XRENGINE_ReadShadowAtlasDepth(shadowMap, localCoord.xy + vec2(-0.5, 0.5) * radius, uvScaleBias), bias);
    lit += XRENGINE_ShadowLit(localCoord.z, XRENGINE_ReadShadowAtlasDepth(shadowMap, localCoord.xy + vec2(0.5, 0.5) * radius, uvScaleBias), bias);
...
float XRENGINE_SampleShadowAtlasCHSS(
    ...
    float maxPenumbra)
{
    float receiverDepth = XRENGINE_ApplyShadowBias(localCoord.z, bias);
...
vec3 XRENGINE_ShadowAtlasArrayUvFromLocal(vec2 localUv, float layer, vec4 uvScaleBias)
{
    return vec3(XRENGINE_ShadowAtlasUvFromLocal(localUv, uvScaleBias), layer);
}

float XRENGINE_ReadShadowAtlasDepth(sampler2DArray shadowMap, vec2 localUv, float layer, vec4 uvScaleBias)
{
    return texture(shadowMap, XRENGINE_ShadowAtlasArrayUvFromLocal(localUv, layer, uvScaleBias)).r;
}
    ...
    int kernelSize)
{
    ...
    for (int x = -halfKernel; x <= halfKernel; ++x)
    {
        for (int y = -halfKernel; y <= halfKernel; ++y)
        {
            float pcfDepth = XRENGINE_ReadShadowAtlasDepth(shadowMap, localCoord.xy + vec2(x, y) * texelSize, layer, uvScaleBias);
            shadow += XRENGINE_ShadowLit(localCoord.z, pcfDepth, bias);
        }
    }
...
float XRENGINE_SampleShadowAtlasSimple(
    ...
    float bias)
{
    float depth = XRENGINE_ReadShadowAtlasDepth(shadowMap, localCoord.xy, layer, uvScaleBias);
    return XRENGINE_ShadowLit(localCoord.z, depth, bias);
}
    ...
    float filterRadius)
{
    ...
    for (int i = 0; i < 16; ++i)
    {
        ...
        vec2 sampleUv = localCoord.xy + XRENGINE_ShadowPoissonDisk[i] * radius;
        float sampleDepth = XRENGINE_ReadShadowAtlasDepth(shadowMap, sampleUv, layer, uvScaleBias);
        lit += XRENGINE_ShadowLit(localCoord.z, sampleDepth, bias);
    }
    ...
    float filterRadius)
{
    int clampedTaps = clamp(tapCount, 1, XRENGINE_MaxVogelShadowTaps);
    if (clampedTaps <= 1)
        return XRENGINE_SampleShadowAtlasSimple(shadowMap, localCoord, layer, uvScaleBias, bias);
    ...
    float filterRadius)
{
    ...
    lit += XRENGINE_ShadowLit(localCoord.z, XRENGINE_ReadShadowAtlasDepth(shadowMap, localCoord.xy + vec2(-0.5, -0.5) * radius, layer, uvScaleBias), bias);
    lit += XRENGINE_ShadowLit(localCoord.z, XRENGINE_ReadShadowAtlasDepth(shadowMap, localCoord.xy + vec2(0.5, -0.5) * radius, layer, uvScaleBias), bias);
    lit += XRENGINE_ShadowLit(localCoord.z, XRENGINE_ReadShadowAtlasDepth(shadowMap, localCoord.xy + vec2(-0.5, 0.5) * radius, layer, uvScaleBias), bias);
    lit += XRENGINE_ShadowLit(localCoord.z, XRENGINE_ReadShadowAtlasDepth(shadowMap, localCoord.xy + vec2(0.5, 0.5) * radius, layer, uvScaleBias), bias);
    ...
    float maxPenumbra)
{
    ...
    return XRENGINE_SampleShadowAtlasVogel(shadowMap, localCoord, layer, uvScaleBias, localTexelSize, bias, filterSamples, penumbra);
}
...
{
    if (softShadowMode == 3) // VogelDisk
        return XRENGINE_SampleShadowAtlasVogel(shadowMap, localCoord, layer, uvScaleBias, localTexelSize, bias, vogelTapCount, filterRadius);
    ...
    if (softShadowMode == 1) // FixedPoisson
        return XRENGINE_SampleShadowAtlasSoft(shadowMap, localCoord, layer, uvScaleBias, localTexelSize, bias, clampedFilterSamples, filterRadius);

    if (clampedFilterSamples <= 4)
        return XRENGINE_SampleShadowAtlasTent4(shadowMap, localCoord, layer, uvScaleBias, localTexelSize, bias, filterRadius);

    return XRENGINE_SampleShadowAtlasPCF(shadowMap, localCoord, layer, uvScaleBias, localTexelSize, bias, 3);
}
...
{
    return XRENGINE_LinearDepth01ToPerspectiveDepth(
        XRENGINE_ReadShadowAtlasDepth(shadowMap, localUv, layer, uvScaleBias),
        nearZ,
        farZ);
}
...
float XRENGINE_SampleLinearDepthShadowAtlasSimpleAsPerspective(
    ...
    float farZ)
{
    float depth = XRENGINE_ReadLinearDepthShadowAtlasAsPerspective(shadowMap, localCoordLinear.xy, layer, uvScaleBias, nearZ, farZ);
    return (receiverPerspectiveDepth - bias) > depth ? 0.0 : 1.0;
}
    ...
    float farZ)
{
    ...
    for (int i = 0; i < 16; ++i)
    {
        ...
        vec2 sampleUv = localCoordLinear.xy + XRENGINE_ShadowPoissonDisk[i] * radius;
        float sampleDepth = XRENGINE_ReadLinearDepthShadowAtlasAsPerspective(shadowMap, sampleUv, layer, uvScaleBias, nearZ, farZ);
        lit += (receiverPerspectiveDepth - bias) <= sampleDepth ? 1.0 : 0.0;
    }
    ...
    float farZ)
{
    ...
    lit += (receiverPerspectiveDepth - bias) <= XRENGINE_ReadLinearDepthShadowAtlasAsPerspective(shadowMap, localCoordLinear.xy + vec2(-0.5, -0.5) * radius, layer, uvScaleBias, nearZ, farZ) ? 1.0 : 0.0;
    lit += (receiverPerspectiveDepth - bias) <= XRENGINE_ReadLinearDepthShadowAtlasAsPerspective(shadowMap, localCoordLinear.xy + vec2(0.5, -0.5) * radius, layer, uvScaleBias, nearZ, farZ) ? 1.0 : 0.0;
    lit += (receiverPerspectiveDepth - bias) <= XRENGINE_ReadLinearDepthShadowAtlasAsPerspective(shadowMap, localCoordLinear.xy + vec2(-0.5, 0.5) * radius, layer, uvScaleBias, nearZ, farZ) ? 1.0 : 0.0;
    lit += (receiverPerspectiveDepth - bias) <= XRENGINE_ReadLinearDepthShadowAtlasAsPerspective(shadowMap, localCoordLinear.xy + vec2(0.5, 0.5) * radius, layer, uvScaleBias, nearZ, farZ) ? 1.0 : 0.0;
