...
namespace XREngine.Scene.Transforms
{
    public class Spline3DTransform : TransformBase
    {
        ...
        private void Tick()
        {
            ...
            long nextTicks = _animationTicks + Math.Max(0L, (RuntimeTransformServices.Current?.UpdateDeltaTicks ?? 0L));
        ...
        protected override Matrix4x4 CreateLocalMatrix()
        {
            ...
            if (RotateToVelocity)
            {
                ...
                if (forward.LengthSquared() > 0.0f)
                {
                    ...
                    if (SmoothRotation)
                        matrix = Matrix4x4.Lerp(LocalMatrix, matrix, RotationSpeed * (RuntimeTransformServices.Current?.DilatedUpdateDeltaSeconds ?? 0.0f));
