...
namespace XREngine.Scene.Transforms
{
    ...
    public class SmoothedTransform : Transform
    {
        ...
        private void Lerp()
        {
            _currentRotation = Quaternion.Slerp(_currentRotation, Rotation, RotationSmoothingSpeed * (RuntimeTransformServices.Current?.DilatedUpdateDeltaSeconds ?? 0.0f));
            _currentTranslation = Vector3.Lerp(_currentTranslation, Translation, TranslationSmoothingSpeed * (RuntimeTransformServices.Current?.DilatedUpdateDeltaSeconds ?? 0.0f));
            _currentScale = Vector3.Lerp(_currentScale, Scale, ScaleSmoothingSpeed * (RuntimeTransformServices.Current?.DilatedUpdateDeltaSeconds ?? 0.0f));
            MarkLocalModified();
        }
    }
}
