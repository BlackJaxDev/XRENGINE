...
/// </summary>
internal sealed class UnityAnimationBindingRuntime(XRComponent owner)
{
    ...
    private readonly Dictionary<UnityAnimationBindingDescriptor, ResolvedBinding> _resolved = [];
    private readonly Dictionary<UnityAssetReference, object?> _resolvedAssets = [];
    private readonly Dictionary<UnityAnimationQuaternionBindingKey, QuaternionChannelAccumulator> _quaternionBindings = [];

    public bool TryValidate(AnimationClip clip, out string diagnostic)
    {
        ArgumentNullException.ThrowIfNull(clip);

        UnityAnimationBindingDescriptor[] bindings = clip.UnityGenericBindings;
        for (int i = 0; i < bindings.Length; i++)
        {
            UnityAnimationBindingDescriptor binding = bindings[i];
        ...
        ref bool ambiguous)
    {
        foreach (TransformBase child in node.Transform.Children)
        {
            ...
            if (ComputeUnityCrc32(childPath) == expectedHash)
            {
    ...
    private bool TryResolveAdapter(
        ...
        out string diagnostic)
    {
        IUnityAnimationBindingAdapter? selected = null;
        for (int i = 0; i < node.Components.Count; i++)
        {
            if (node.Components[i] is not IUnityAnimationBindingAdapter adapter
        ...
        if (UnityAnimationQuaternionBindingKey.TryCreate(binding, out _))
        {
        ...
        resolved = binding.ValueKind == EUnityAnimationBindingValueKind.ObjectReference
            ? new ResolvedBinding
            {
                ObjectSetter = (UnityAssetReference value, out string setterDiagnostic)
    ...
    private static void ApplyQuaternionToAdapter(
        IUnityAnimationBindingAdapter adapter,
        UnityAnimationBindingDescriptor binding,
        Quaternion value)
    {
        ...
        {
            UnityAnimationBindingDescriptor componentBinding = CreateQuaternionComponentBinding(
                binding,
                baseAttribute,
                component);
    ...
    private static UnityAnimationBindingDescriptor CreateQuaternionComponentBinding(
        UnityAnimationBindingDescriptor binding,
        ...
        out string diagnostic)
    {
        ...
        if (binding.ValueKind == EUnityAnimationBindingValueKind.ObjectReference)
        {
            resolved = new ResolvedBinding
            {
                ObjectSetter = (UnityAssetReference reference, out string setterDiagnostic) =>
                {
            ...
            foreach (var lod in subMesh.LODs)
            {
                ...
                for (int nameIndex = 0; nameIndex < names.Length; nameIndex++)
                {
                    ...
                    string serializedName = name.StartsWith("blendShape.", StringComparison.Ordinal)
                        ? name
                        : $"blendShape.{name}";
                    if (ComputeUnityCrc32(serializedName) != binding.AttributeHash)
                        continue;
        ...
        out string diagnostic)
    {
        ...
        try
        {
            if (valueType == typeof(Quaternion)
                && component >= 0
                && UnityAnimationQuaternionBindingKey.TryCreate(binding, out _))
            {
    ...
    private static Expression RebuildNormalizedQuaternion(Expression current, Expression input, int component)
    {
        ...
        return component == 3
            ? Expression.Call(typeof(UnityAnimationBindingRuntime), nameof(NormalizeQuaternion), Type.EmptyTypes, rebuilt)
            : rebuilt;
    }
    ...
    {
        if (!UnityAnimationQuaternionBindingKey.TryCreate(binding, out UnityAnimationQuaternionBindingKey key))
    ...
    private static string Describe(UnityAnimationBindingDescriptor binding)
    {
    ...
    internal static uint ComputeUnityCrc32(string value)
    {
    ...
    private delegate bool ObjectReferenceSetter(UnityAssetReference value, out string diagnostic);
    ...
    private sealed class ResolvedBinding
    {
        ...
        public UnityAssetReference LastObjectReference { get; set; }
