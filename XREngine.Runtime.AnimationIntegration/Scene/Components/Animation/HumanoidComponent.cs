...
namespace XREngine.Components.Animation
{
    ...
    public partial class HumanoidComponent : XRComponent, IRenderable, IHumanoidHeightReference, IHumanoidVrCalibrationRig, IImportedHumanoidProjectionPoseSink
    {
        ...
        private static readonly (string Left, string Right)[] HumanoidArmSideMirrorPairs =
        [
        ...
        private const string ImportedAvatarContentBasisNodeName = "UnityHumanoidContentBasis";
        private SceneNode? _importedAvatarContentBasisNode;
        private Quaternion _importedAvatarContentBasisCorrection = Quaternion.Identity;
        ...
        private void RebuildSceneNodeBindings(bool clearExistingBindings)
        {
            ...
            TryApplyPersistedAvatarDefinitionBindings();
        ...
        /// <summary>
        /// Model units per meter declared by the importer which produced this
        /// scene hierarchy, or zero when the producer supplied no unit evidence.
        /// </summary>
        public float SourceModelUnitsPerMeter => _sourceModelUnitsPerMeter;

        private void ObserveSourceModelUnitMetadata()
        {
        ...
        /// <summary>
        /// Static content-space correction that aligns the imported skeleton and meshes with
        /// the Unity avatar-root axes. It is deliberately separate from projected root motion.
        /// </summary>
        public Quaternion ImportedAvatarContentBasisCorrection => _importedAvatarContentBasisCorrection;

        private bool TryApplyImportedAvatarContentBasis()
        {
            HumanoidAvatarBodyAxes? bodyAxes = AvatarDefinition.Status != EHumanoidAvatarDefinitionStatus.Uninitialized
                ? AvatarDefinition.BodyAxes
                : null;
            if (bodyAxes is null)
                return false;
            if (!bodyAxes.IsFiniteOrthonormal())
                return false;
            if (Hips.Node is null || Spine.Node is null)
                return false;

            GetBindBodyBasis(out Vector3 currentLeft, out Vector3 currentUp, out Vector3 currentForward);
            Vector3 targetLeft = -bodyAxes.Right;
            Vector3 targetUp = bodyAxes.Up;
            Vector3 targetForward = bodyAxes.Forward;
            Matrix4x4 currentFrame = CreateBodyBasisMatrix(currentLeft, currentUp, currentForward);
            Matrix4x4 targetFrame = CreateBodyBasisMatrix(targetLeft, targetUp, targetForward);
            if (!Matrix4x4.Invert(currentFrame, out Matrix4x4 inverseCurrentFrame))
                return false;

            Matrix4x4 correctionMatrix = inverseCurrentFrame * targetFrame;
            Quaternion correction = Quaternion.Normalize(Quaternion.CreateFromRotationMatrix(correctionMatrix));
            if (!IsFiniteNonZero(correction))
                return false;

            float correctionDegrees = QuaternionAngleDegrees(correction);
            if (correctionDegrees <= 0.05f)
                return false;

            TransformBase[] contentTransforms = [.. SceneNode.Transform.Children];
            SceneNode? basisNode = ResolveImportedAvatarContentBasisNode();
            if (basisNode is null)
            {
                basisNode = new SceneNode(SceneNode) { Name = ImportedAvatarContentBasisNodeName };
                _importedAvatarContentBasisNode = basisNode;
                for (int i = 0; i < contentTransforms.Length; i++)
                    basisNode.Transform.AddChild(contentTransforms[i], false, EParentAssignmentMode.Immediate);
            }

            if (basisNode.GetTransformAs<Transform>(true) is not Transform basisTransform)
                return false;

            Quaternion existingRotation = IsFiniteNonZero(basisTransform.Rotation)
                ? Quaternion.Normalize(basisTransform.Rotation)
                : Quaternion.Identity;
            Matrix4x4 correctedRotationMatrix =
                Matrix4x4.CreateFromQuaternion(existingRotation) * correctionMatrix;
            Quaternion correctedRotation = Quaternion.Normalize(
                Quaternion.CreateFromRotationMatrix(correctedRotationMatrix));
            basisTransform.SetLocalTranslationRotation(basisTransform.Translation, correctedRotation);
            _importedAvatarContentBasisCorrection = correctedRotation;

            Debug.Animation(
                $"[HumanoidProfile] Applied Unity content-basis correction {correctionDegrees:F3} deg " +
                $"to '{SceneNode.Name}' on child '{ImportedAvatarContentBasisNodeName}'.");
            return true;
        }

        private SceneNode? ResolveImportedAvatarContentBasisNode()
        {
            if (_importedAvatarContentBasisNode is { IsDestroyed: false })
                return _importedAvatarContentBasisNode;

            foreach (TransformBase child in SceneNode.Transform.Children)
            {
                if (child.SceneNode is SceneNode node
                    && string.Equals(node.Name, ImportedAvatarContentBasisNodeName, StringComparison.Ordinal))
                {
                    _importedAvatarContentBasisNode = node;
                    return node;
                }
            }
        ...
        private static Matrix4x4 CreateBodyBasisMatrix(Vector3 left, Vector3 up, Vector3 forward)
        {
            left = NormalizeOrFallback(left, Vector3.UnitX);
        ...
        private static float QuaternionAngleDegrees(Quaternion rotation)
        {
            ...
            float absoluteW = Math.Clamp(MathF.Abs(normalized.W), 0.0f, 1.0f);
            return 2.0f * MathF.Acos(absoluteW) * (180.0f / MathF.PI);
        }
        ...
        TransformBase IHumanoidHeightReference.RootTransform => Transform;
        TransformBase IHumanoidVrCalibrationRig.RootTransform => Transform;
        SceneNode? IHumanoidVrCalibrationRig.HeadNode => Head.Node;
        SceneNode? IHumanoidVrCalibrationRig.HipsNode => Hips.Node;
        ...
        private EHumanoidNeutralPosePreset _neutralPosePreset = EHumanoidNeutralPosePreset.HumanoidRetargeting;
        ...
        internal static float ConvertImportedHumanoidAmount(EHumanoidValue value, float amount, bool flipMuscleZ)
        {
            if (!flipMuscleZ)
                return amount;
        ...
        private bool TryCaptureMuscleSnapshot(float[] destination)
        {
            lock (_muscleValuesLock)
            {
                if (_muscleValues.Count == 0
                    ...
                    && !_hasInvalidImportedTranslationDof
                    // Root-only clips and zero-muscle state leaves still require one
                    // canonical neutral transaction to finalize Body/Hips/root.
                    && !_isImportedBodySampleTransactionActive
                    && _pendingStateMachineRootMotionFrame is null)
                    return false;
        ...
        /// <summary>
        /// Loads avatar-specific Unity humanoid calibration exported by
        /// <c>HumanoidPoseAuditExporter</c> and immediately refreshes the live pose.
        /// </summary>
        public void LoadImportedHumanoidAvatarProfile(string filePath)
        {
            ImportedHumanoidAvatarProfile profile = ImportedHumanoidAvatarProfileImporter.Import(filePath);
            MigrateLegacyImportedAvatarProfile(profile, applyPreview: _sceneNodeInitializationComplete);
        }

        private static int CountDescendantsNamed(SceneNode node, string name, ref SceneNode? firstMatch)
        {
            int count = 0;
            ...
            foreach (TransformBase childTransform in node.Transform.Children)
                if (childTransform.SceneNode is SceneNode child)
                    count += CountDescendantsNamed(child, name, ref firstMatch);
            return count;
        }

        private float _sourceProfileUnitsPerMeter;

        /// <summary>
        /// Model-space units per Unity meter derived from corresponding neutral
        /// bone translations in the active avatar profile.
        /// </summary>
        public float SourceProfileUnitsPerMeter => _sourceProfileUnitsPerMeter;

        private void ApplyNeutralPoseBindRelativeRotations(
        ...
        {
            // A generic preset is authored from this skeleton, not from a table
        ...
        private static Quaternion ResolveNeutralPoseLocalRotation(
            ...
            bool mirrorArmSideConvention)
        {
            if (!mirrorArmSideConvention || !TryGetMirroredHumanoidArmBoneName(boneName, out string mirroredBoneName))
                return fallback;
        ...
        private static bool TryGetMirroredHumanoidArmBoneName(string boneName, out string mirroredBoneName)
        {
            foreach ((string left, string right) in HumanoidArmSideMirrorPairs)
            {
        ...
        private Quaternion GetNeutralPoseBoneRotation(SceneNode? node)
        {
            ...
            if (TryGetCompiledAvatarDefinition(out CompiledHumanoidAvatarDefinition compiled))
                return compiled.GetCanonicalPoseCorrection(node);
        ...
        {
            // A finalized avatar owns the neutral transform used by playback.
            // Captured/live bind state remains available only to the authoring and
            // migration paths that run before a definition has been compiled.
            if (TryGetCompiledAvatarDefinition(out CompiledHumanoidAvatarDefinition compiled)
                && compiled.TryGetNeutralLocalState(node, out scale, out rotation, out translation))
                return true;
        ...
        /// <summary>Returns the captured humanoid bind-local rotation for pose-audit comparison.</summary>
        internal bool TryGetDiagnosticBindLocalRotation(SceneNode node, out Quaternion rotation)
            => TryGetHumanoidBindLocalState(node, out _, out rotation, out _);

        /// <summary>Returns the configured zero-muscle rotation relative to the captured bind pose.</summary>
        internal Quaternion GetDiagnosticNeutralBindRelativeRotation(SceneNode node)
            => GetNeutralPoseBoneRotation(node);

        private Matrix4x4 GetHumanoidBindWorldPose(SceneNode node)
        {
            if (TryGetCompiledAvatarDefinition(out CompiledHumanoidAvatarDefinition compiled)
                && compiled.TryGetRole(node, out _))
                return compiled.GetNeutralWorldTransform(node);

            return _humanoidBindWorldPoses.TryGetValue(node, out Matrix4x4 worldBindPose)
                ? worldBindPose
        ...
        private void ApplyNeutralBindRelativePose(
            SceneNode node,
            Quaternion deltaRotation,
            Vector3 positionDelta,
            Vector3? calibratedNeutralPosition = null)
        {
            if (!TryGetHumanoidBindLocalState(node, out Vector3 scale, out Quaternion bindRotation, out Vector3 bindTranslation))
                return;

            Quaternion neutralRotation = GetNeutralPoseBoneRotation(node);
            Quaternion rotation = Quaternion.Normalize(bindRotation * neutralRotation * deltaRotation);
            // Coupled profile deltas are measured from Unity's zero-muscle local pose.
            // Prefer that calibrated origin when present; the FBX Hips bind translation
            // can include Unity's body-offset representation and is not equivalent.
            Vector3 translation = (calibratedNeutralPosition ?? bindTranslation) + positionDelta;
            if (node.GetTransformAs<Transform>(true) is Transform transform)
            {
        ...
        private void ApplyMusclePose()
        {
            ...
            HumanoidIKSolverComponent? ikSolver =
                GetSiblingComponent<HumanoidIKSolverComponent>(false);
            if (!IsAnimatedPosePreviewActive)
            {
                if (ikSolver?.HasUnresolvedAnimationDrivenGoalFrame == true)
                    ikSolver.RejectAnimationDrivenGoalFrame();
                return;
            }

            // Authored IK is part of the same visible-frame transaction. Validate it
            // before Body/root state or any concrete bone is allowed to commit.
            if (ikSolver?.IsAnimationDrivenGoalFrameRejected == true)
            {
                LogRejectedImportedHumanoidFrame(
                    "authored IK position, rotation, or weight was invalid");
                RejectCurrentNativeHumanoidFrame(ikSolver);
                return;
            }

            if (!TryCaptureMuscleSnapshot(_muscleValueSnapshot))
            {
                // An IK-only humanoid clip still needs one native neutral solve so
                // its goal is applied after a known authored-pose baseline.
                if (ikSolver?.HasPendingAnimationDrivenGoalFrame != true
                    && !_isImportedBodySampleTransactionActive
                    && _currentImportedMappedBodySample.Channels == EHumanoidImportedBodySampleChannels.None)
                    return;
                Array.Clear(_muscleValueSnapshot);
            }
            ...
            // Direct evaluators defer RootT/RootQ commit until this canonical native
            // frame is known to be valid. The state-machine path owns its separate
            // final blended Body/root frame and has no active scalar transaction here.
            if (!TryValidateActiveImportedBodySampleTransaction())
            {
                RejectCurrentNativeHumanoidFrame(ikSolver);
                return;
            }

            HumanoidBodyFrameTransactionState savedBody = CaptureBodyFrameTransactionState();
            bool reuseCommittedBody = !_isImportedBodySampleTransactionActive && !_hasPendingProjectedRootMotion;
            HumanoidStateMachineRootMotionFrame? feetProjectionFrame = _pendingStateMachineRootMotionFrame;
            feetProjectionFrame?.BeginFeetProjectionTransaction();
            bool committed = false;
            try
            {
                if (!TryEvaluateNativeHumanoidPose(compiled, muscleSnapshot, includeTranslationDof: true)
                    || !CommitActiveImportedBodySampleTransaction())
                    return;

                bool hasStateFrame = _pendingStateMachineRootMotionFrame is not null;
                Matrix4x4 requested;
                Matrix4x4 beforeProjection;
                bool prepared = hasStateFrame
                    ? TryPrepareStateMachineBodyFrame(compiled, muscleSnapshot, out requested, out beforeProjection)
                    : TryPrepareDirectBodyFrame(compiled, muscleSnapshot, reuseCommittedBody, out requested, out beforeProjection);
                if (!prepared || !_nativePoseWorkspace.TryAlignBody(compiled, requested))
                    return;

                // This is the sole authored skeleton commit. Root history and IK
                // become observable only after every Body/local/FK guard passed.
                _nativePoseWorkspace.Commit(compiled);
                muscleSnapshot[..MuscleValueCount].CopyTo(_appliedMuscleValueSnapshot);
                bool hasRootMotionInput = _hasPendingProjectedRootMotion;
                if (_hasPendingProjectedRootMotion)
                    CommitPendingProjectedRootMotion(_pendingProjectedRootMotionOwner);
                PublishBodyFrameDiagnostic(compiled, beforeProjection, hasRootMotionInput);
                committed = true;
                _lastNativeFrameAccepted = true;
                ikSolver?.ResolveAnimationDrivenGoalFrame(true);
                LogFirstPlaybackDiagnostic();
                LogMusclePoseSnapshot(_muscleValueSnapshot);
            }
            finally
            {
                feetProjectionFrame?.ResolveFeetProjectionTransaction(committed);
                if (!committed)
                {
                    RestoreBodyFrameTransactionState(savedBody);
                    LogRejectedImportedHumanoidFrame(string.IsNullOrEmpty(_nativePoseWorkspace.FailureReason)
                        ? "Body/root frame could not be solved" : _nativePoseWorkspace.FailureReason);
                    RejectCurrentNativeHumanoidFrame(ikSolver);
                }
            }
        }

        private void RejectCurrentNativeHumanoidFrame(HumanoidIKSolverComponent? ikSolver)
        {
            ...
            ikSolver?.ResolveAnimationDrivenGoalFrame(false);
        }
        ...
        {
            HumanoidAvatarSolverSettings solverSettings = compiled.SolverSettings;
            EHumanoidAvatarBoneRole shoulderRole = isLeft ? EHumanoidAvatarBoneRole.LeftShoulder : EHumanoidAvatarBoneRole.RightShoulder;
            EHumanoidAvatarBoneRole upperArmRole = isLeft ? EHumanoidAvatarBoneRole.LeftUpperArm : EHumanoidAvatarBoneRole.RightUpperArm;
            EHumanoidAvatarBoneRole lowerArmRole = isLeft ? EHumanoidAvatarBoneRole.LeftLowerArm : EHumanoidAvatarBoneRole.RightLowerArm;
            EHumanoidAvatarBoneRole handRole = isLeft ? EHumanoidAvatarBoneRole.LeftHand : EHumanoidAvatarBoneRole.RightHand;
            EHumanoidAvatarBoneRole upperLegRole = isLeft ? EHumanoidAvatarBoneRole.LeftUpperLeg : EHumanoidAvatarBoneRole.RightUpperLeg;
            EHumanoidAvatarBoneRole lowerLegRole = isLeft ? EHumanoidAvatarBoneRole.LeftLowerLeg : EHumanoidAvatarBoneRole.RightLowerLeg;
            ...
            SceneNode? shoulderNode = compiled.GetNode(shoulderRole);
            SceneNode? upperArmNode = compiled.GetNode(upperArmRole);
            SceneNode? lowerArmNode = compiled.GetNode(lowerArmRole);
            SceneNode? handNode = compiled.GetNode(handRole);
            SceneNode? upperLegNode = compiled.GetNode(upperLegRole);
            SceneNode? lowerLegNode = compiled.GetNode(lowerLegRole);
            ...
            // Stretch channels behave like hinge flexion/extension. Unity's normalized
            // muscle range includes both negative extension and positive flexion, so both
            // sides of the configured range must reach the joint solver unchanged.
            ...
            float armTwistDeg = MapMuscleToDeg(armTwist, GetMuscleValue(muscleSnapshot, armTwist));
            float forearmTwistDeg = MapMuscleToDeg(forearmTwist, GetMuscleValue(muscleSnapshot, forearmTwist));
            float upperLegTwistDeg = MapMuscleToDeg(upperLegTwist, GetMuscleValue(muscleSnapshot, upperLegTwist));
            float lowerLegTwistDeg = MapMuscleToDeg(lowerLegTwist, GetMuscleValue(muscleSnapshot, lowerLegTwist));
            ...
            Vector3 shoulderTwistAxisWorld = GetBoneToChildAxisWorld(compiled, shoulderRole, upperArmRole, isLeft ? bodyLeft : -bodyLeft);
            Vector3 armTwistAxisWorld = GetBoneToChildAxisWorld(compiled, upperArmRole, lowerArmRole, isLeft ? bodyLeft : -bodyLeft);
            Vector3 forearmTwistAxisWorld = GetBoneToChildAxisWorld(compiled, lowerArmRole, handRole, isLeft ? bodyLeft : -bodyLeft);
            Vector3 upperLegTwistAxisWorld = GetBoneToChildAxisWorld(compiled, upperLegRole, lowerLegRole, -bodyUp);
            Vector3 lowerLegTwistAxisWorld = GetBoneToChildAxisWorld(compiled, lowerLegRole, footRole, -bodyUp);
            Vector3 footTwistAxisWorld = GetBoneToChildAxisWorld(compiled, footRole, toesRole, bodyForward);
            ...
            ApplyLimbBoneRotation(
                shoulderNode, DebugShoulderSigns,
                yawDeg: 0.0f,
            ...
            ApplyLimbBoneRotation(
                upperArmNode, DebugArmSigns,
                yawDeg: armTwistDeg * solverSettings.UpperArmTwist,
            ...
            ApplyLimbBoneRotation(
                lowerArmNode, DebugForearmSigns,
                yawDeg: MapMuscleToDeg(forearmTwist, GetMuscleValue(muscleSnapshot, forearmTwist)) * solverSettings.LowerArmTwist,
                pitchDeg: MapMuscleToDeg(forearmStretch, forearmStretchMuscle),
                rollDeg: 0.0f,
                twistAxisWorld: forearmTwistAxisWorld,
                pitchAxisWorld: armPitchAxisWorld,
                rollAxisWorld: armRollAxisWorld,
                inheritedTwistDeg: armTwistDeg * (1.0f - solverSettings.UpperArmTwist),
                inheritedTwistAxisWorld: armTwistAxisWorld);

            ApplyLimbBoneRotation(
                handNode, DebugWristSigns,
                yawDeg: 0.0f,
                ...
                pitchAxisWorld: armPitchAxisWorld,
                rollAxisWorld: armRollAxisWorld,
                inheritedTwistDeg: forearmTwistDeg * (1.0f - solverSettings.LowerArmTwist),
                inheritedTwistAxisWorld: forearmTwistAxisWorld);

            ApplyLimbBoneRotation(
                upperLegNode, DebugUpperLegSigns,
                yawDeg: upperLegTwistDeg * solverSettings.UpperLegTwist,
            ...
            ApplyLimbBoneRotation(
                lowerLegNode, DebugKneeSigns,
                yawDeg: lowerLegTwistDeg * solverSettings.LowerLegTwist,
                ...
                rollAxisWorld: legRollAxisWorld,
                preferAxisMapping: true,
                inheritedTwistDeg: upperLegTwistDeg * (1.0f - solverSettings.UpperLegTwist),
                inheritedTwistAxisWorld: upperLegTwistAxisWorld);

            ApplyLimbBoneRotation(
                footNode, DebugFootSigns,
                ...
                rollAxisWorld: legRollAxisWorld,
                preferAxisMapping: true,
                inheritedTwistDeg: lowerLegTwistDeg * (1.0f - solverSettings.LowerLegTwist),
                inheritedTwistAxisWorld: lowerLegTwistAxisWorld);

            ApplyLimbBoneRotation(
                toesNode, DebugToesSigns,
                yawDeg: 0.0f,
        ...
        private void ApplyFingerMuscles(
            CompiledHumanoidAvatarDefinition compiled,
            bool isLeft,
            ReadOnlySpan<float> muscleSnapshot)
        {
            ...
            EHumanoidValue spread, EHumanoidValue prox, EHumanoidValue mid, EHumanoidValue dist)
        {
            ...
            ApplyBindRelativeEulerDegrees(
                ...
                rollDeg: 0.0f,
                axisMapping: compiled.GetAxisMapping(proximalRole));

            ApplyBindRelativeEulerDegrees(
                compiled.GetNode(intermediateRole),
                yawDeg: 0.0f,
                pitchDeg: MapMuscleToDeg(mid, GetMuscleValue(muscleSnapshot, mid)),
                rollDeg: 0.0f,
                axisMapping: compiled.GetAxisMapping(intermediateRole));

            ApplyBindRelativeEulerDegrees(
                compiled.GetNode(distalRole),
                yawDeg: 0.0f,
                pitchDeg: MapMuscleToDeg(dist, GetMuscleValue(muscleSnapshot, dist)),
                rollDeg: 0.0f,
                axisMapping: compiled.GetAxisMapping(distalRole));
        }

        private BoneAxisMapping? GetBoneAxisMapping(SceneNode? node)
        {
            if (TryGetCompiledAvatarDefinition(out CompiledHumanoidAvatarDefinition compiled))
                return compiled.GetAxisMapping(node);

            return node?.Name is not null && Settings.TryGetBoneAxisMapping(node.Name, out var mapping)
                ? mapping
                : null;
        }

        private SceneNode? GetCompiledAvatarNode(EHumanoidAvatarBoneRole role)
            => TryGetCompiledAvatarDefinition(out CompiledHumanoidAvatarDefinition compiled)
                ? compiled.GetNode(role)
                : null;
        ...
        private void ApplyBodyBasisLimbRotation(
            ...
            float inheritedTwistDeg,
            Vector3? inheritedTwistAxisWorld)
            ...
            Vector3? inheritedTwistAxisWorld)
        {
            ...
            inheritedTwistDeg *= overrides.YawSign;
            ...
            Quaternion inheritedTwist = CreateInheritedTwistRotation(
                node,
                inheritedTwistDeg,
                inheritedTwistAxisWorld,
                overrides.SkipBlanketNegateYaw);
            Quaternion muscleRotation = CreateBindRelativeEulerRotationDegrees(yawDeg, pitchDeg, rollDeg, axisMapping);
            ApplyNeutralBindRelativeRotation(node, Quaternion.Normalize(inheritedTwist * muscleRotation));
        }
        ...
        private float MapMuscleToDeg(EHumanoidValue value, float muscle)
        {
            ...
            float m = Math.Clamp(muscle, -1.0f, 1.0f);
            CompiledHumanoidAvatarDefinition? compiled = TryGetCompiledAvatarDefinition(out var avatar)
                ? avatar
                : null;
            Vector2 range = compiled?.GetMuscleRange(value)
                ?? Settings.GetResolvedMuscleRotationDegRange(value);
        ...
        {
            if (TryGetCompiledAvatarDefinition(out CompiledHumanoidAvatarDefinition compiled)
                && compiled.BodyAxes.IsFiniteOrthonormal())
            {
                bindBodyLeft = -compiled.BodyAxes.Right;
                bindBodyUp = compiled.BodyAxes.Up;
                bindBodyForward = compiled.BodyAxes.Forward;
                return;
            }
        ...
        private void ApplyBindRelativeEulerDegrees(SceneNode? node, float yawDeg, float pitchDeg, float rollDeg, BoneAxisMapping? axisMapping)
        {
            ...
            ApplyNeutralBindRelativeRotation(node, CreateBindRelativeEulerRotationDegrees(yawDeg, pitchDeg, rollDeg, axisMapping));
        }

        private static Quaternion CreateBindRelativeEulerRotationDegrees(
            float yawDeg,
            float pitchDeg,
            float rollDeg,
            BoneAxisMapping? axisMapping)
        ...
        private static Vector3 GetBoneToChildAxisWorld(
            CompiledHumanoidAvatarDefinition compiled,
            EHumanoidAvatarBoneRole boneRole,
            EHumanoidAvatarBoneRole childRole,
            Vector3 fallbackWorldAxis)
        {
            if (compiled.GetNode(boneRole) is null || compiled.GetNode(childRole) is null)
                return fallbackWorldAxis;

            Vector3 from = compiled.NeutralWorldTransforms[(int)boneRole].Translation;
            Vector3 to = compiled.NeutralWorldTransforms[(int)childRole].Translation;
            ...
            Vector3? inheritedTwistAxisWorld)
        {
            ...
            ApplyBindRelativeSwingTwistWorldAxes(
                ...
                inheritedTwistDeg, inheritedTwistAxisWorld,
                overrides.SkipBlanketNegateYaw,
            ...
            bool skipBlanketNegateRoll = false)
        {
            ...
            Quaternion inheritedTwist = CreateInheritedTwistRotation(
                ...
                inheritedTwistDeg,
                inheritedTwistAxisWorld,
                skipBlanketNegateYaw);

            // Unity applies twist inherited from the proximal segment before this
            // bone's own swing and twist response.
            Quaternion q = inheritedTwist * leftRight * frontBack * twist;
            ApplyNeutralBindRelativeRotation(node, q);
        }

        private Quaternion CreateInheritedTwistRotation(
            SceneNode node,
            float inheritedTwistDeg,
            Vector3? inheritedTwistAxisWorld,
            bool skipBlanketNegateYaw)
        {
            if (!inheritedTwistAxisWorld.HasValue || MathF.Abs(inheritedTwistDeg) <= 0.000001f)
                return Quaternion.Identity;

            const float degToRad = MathF.PI / 180.0f;
            float handednessSign = skipBlanketNegateYaw ? 1.0f : -1.0f;
            Vector3 inheritedTwistLocal = TransformWorldAxisToBoneLocal(node, inheritedTwistAxisWorld.Value);
            return Quaternion.CreateFromAxisAngle(inheritedTwistLocal, handednessSign * inheritedTwistDeg * degToRad);
        }
        ...
        private bool _showAvatarAxes;
        /// <summary>Draws the finalized body-axis basis at the mapped hips.</summary>
        public bool ShowAvatarAxes
        {
            get => _showAvatarAxes;
            set => SetField(ref _showAvatarAxes, value);
        }
        ...
        private void RenderAvatarAxes()
        {
            if (Hips.Node?.Transform is null)
                return;

            HumanoidAvatarBodyAxes axes = TryGetCompiledAvatarDefinition(out CompiledHumanoidAvatarDefinition compiled)
                ? compiled.BodyAxes
                : AvatarDefinition.BodyAxes;
            if (!axes.IsFiniteOrthonormal())
                return;

            Vector3 origin = Hips.Node.Transform.WorldTranslation;
            float scale = MathF.Max(EstimateAnimatedMotionScale() * 0.2f, 0.05f);
            RuntimeAnimationHostServices.Current.RenderLine(origin, origin + axes.Right * scale, ColorF4.Red);
            RuntimeAnimationHostServices.Current.RenderLine(origin, origin + axes.Up * scale, ColorF4.Green);
            RuntimeAnimationHostServices.Current.RenderLine(origin, origin + axes.Forward * scale, ColorF4.Blue);
            RuntimeAnimationHostServices.Current.RenderPoint(origin + axes.Right * scale, ColorF4.Red);
            RuntimeAnimationHostServices.Current.RenderPoint(origin + axes.Up * scale, ColorF4.Green);
            RuntimeAnimationHostServices.Current.RenderPoint(origin + axes.Forward * scale, ColorF4.Blue);
        }
        ...
        {
            if (TryGetCompiledAvatarDefinition(out CompiledHumanoidAvatarDefinition compiled)
                && compiled.TryGetRole(node, out EHumanoidAvatarBoneRole role))
            {
                node.Transform.DeriveLocalMatrix(compiled.NeutralLocalTransforms[(int)role]);
                return;
            }
        ...
        {
            if (AvatarDefinition.SourceProvenance == EHumanoidAvatarSourceProvenance.Unknown)
            {
                AvatarDefinition.SourceProvenance = EHumanoidAvatarSourceProvenance.RuntimeAuthoredSkeleton;
                AvatarDefinition.SourceModelContentSha256 = string.Empty;
            }

            SetFromNodeCore();
        }

        private void SetFromNodeCore()
        {
            AutoMapAvatarSkeleton();
            RemoveAmbiguousAutomaticToeLeafMappings();
            RefreshBoneBindPosesFromCapturedHierarchy();
        ...
        /// <summary>
        /// Toe roles are optional. A terminal child named like a toe provides no
        /// structural evidence that distinguishes an anatomical toe joint from an
        /// authored end marker, so automatic mapping leaves that ambiguous case for
        /// author confirmation. Imported semantic and editor-corrected bindings are
        /// intentionally retained.
        /// </summary>
        private void RemoveAmbiguousAutomaticToeLeafMappings()
        {
            RemoveAmbiguousAutomaticToeLeafMapping(
        ...
        private void RemoveAmbiguousAutomaticToeLeafMapping(
            EHumanoidAvatarBoneRole toesRole,
            BoneDef foot,
            BoneDef toes)
        {
            SceneNode? toeNode = toes.Node;
            HumanoidAvatarRoleMappingEvidence? evidence = _autoMappingEvidence[(int)toesRole];
            if (toeNode is null
                || foot.Node is null
                || evidence?.Source != EHumanoidAvatarMappingSource.Automatic
                || toeNode.Transform.Children.Count != 0
                || AliasScore(toeNode.Name, "toe", "toes", "ball") <= 0.0f
                || !IsDirectChild(foot.Node, toeNode))
                return;

            toes.ClearNodeBinding();
            _autoMappingEvidence[(int)toesRole] = null;
        }

        private static bool IsDirectChild(SceneNode parent, SceneNode child)
        {
            foreach (TransformBase transform in parent.Transform.Children)
                if (ReferenceEquals(transform.SceneNode, child))
                    return true;
            return false;
        }

        /// <summary>
        /// Retained temporarily as implementation history for the pre-definition
        /// mapper. Production initialization no longer invokes name-first role
        /// discovery; aliases are only one weighted input to
        /// <see cref="AutoMapAvatarSkeleton"/>.
        /// </summary>
        private void LegacySetFromNodeNameMapper()
        {
        ...
        private void LogFirstPlaybackDiagnostic()
        {
            ...
            string bodyModel = TryGetCompiledAvatarDefinition(out CompiledHumanoidAvatarDefinition compiled)
                ? compiled.BodyDefinition.ModelId : "(unavailable)";

            // Cold, one-time capability disclosure: native constraint success
            // is not ratification of the Body model against external Unity data.
            Debug.Animation(
                $"[HumanoidComponent] First native playback on '{avatarName}': " +
                $"avatarRevision={AvatarDefinition.DefinitionRevision} " +
                $"BodyModel={bodyModel} IK={ikPolicy}. " +
                "Pose-dependent Body/Hips compensation is enabled; full Unity pose parity is not yet ratified.");
        }
        ...
        /// <summary>
        /// Resolves the avatar-space scale used by imported Unity humanoid Body
        /// curves. A Unity avatar profile supplies the exact humanScale multiplied
        /// by the model's measured units-per-meter ratio; unprofiled avatars retain
        /// the legacy bind-pose leg-length heuristic.
        /// </summary>
        public float EstimateAnimatedMotionScale()
        {
            if (TryGetCompiledAvatarDefinition(out CompiledHumanoidAvatarDefinition compiled)
                && float.IsFinite(compiled.HumanScale)
                && compiled.HumanScale > 0.0f
                && float.IsFinite(compiled.ModelUnitsPerMeter)
                && compiled.ModelUnitsPerMeter > 0.0f)
                return compiled.HumanScale * compiled.ModelUnitsPerMeter;
        ...
        private HumanoidImportedBodySample _currentImportedMappedBodySample = HumanoidImportedBodySample.Neutral;
        private HumanoidImportedBodySample _stagedImportedBodySample = HumanoidImportedBodySample.Neutral;
        ...
        private bool _isImportedBodySampleTransactionActive;
        private bool _transactionHasCanonicalImportedBodySample;
        private bool _hasPreviousProjectedRootPose;
        ...
        private ImportedHumanoidClipRootMotionSettings? _importedBodyProjectionSettings;
        ...
        private ImportedHumanoidRootMotionPolicy? _activeImportedBodyProjectionPolicy;
        ...
        private bool _loggedInvalidImportedBodyWeight;
        private bool _loggedRejectedImportedHumanoidFrame;

        /// <summary>
        /// The time-zero imported Unity RootT/RootQ sample used as this clip's canonical body reference.
        /// </summary>
        public HumanoidImportedBodySample CanonicalImportedBodySample => _canonicalImportedBodySample;

        /// <summary>
        /// The exact imported-mapped RootT/RootQ values committed for the current sample, before conversion to the avatar bind pose.
        /// </summary>
        public HumanoidImportedBodySample CurrentImportedMappedBodySample => _currentImportedMappedBodySample;

        /// <summary>
        /// The bind-relative Hips translation delta converted from the current imported body sample.
        /// </summary>
        public Vector3 CurrentConvertedBodyTranslationDelta => _currentConvertedBodyTranslationDelta;

        /// <summary>
        /// The bind-relative Hips rotation delta converted from the current imported body sample.
        /// </summary>
        public Quaternion CurrentConvertedBodyRotationDelta => _currentConvertedBodyRotationDelta;

        /// <summary>
        /// The model-root pose projected from the current Body sample. This component only
        /// publishes the value; an owning evaluator may explicitly consume or apply it.
        /// </summary>
        public HumanoidProjectedRootPose CurrentProjectedRootPose => _currentProjectedRootPose;

        /// <summary>
        /// The temporal change from the preceding projected pose owned by the same evaluator.
        /// The first sample after an owner or lifecycle change has no valid channels.
        /// </summary>
        public HumanoidRootMotionDelta CurrentRootMotionDelta => _currentRootMotionDelta;

        /// <summary>
        /// Invalidates the temporal projected-root baseline owned by an evaluator without changing
        /// its canonical Body reference or the currently composed skeletal pose.
        /// </summary>
        public void InvalidateProjectedRootMotionBaseline(object owner)
        {
        ...
        // Compatibility aliases retained for existing pose-audit consumers.
        public Vector3 CurrentRawBodyPosition => _currentImportedMappedBodySample.Position;
        public Quaternion CurrentRawBodyRotation => _currentImportedMappedBodySample.Rotation;
        public Quaternion CurrentNormalizedBodyRotation => NormalizeImportedBodyRotationPure(_currentImportedMappedBodySample.Rotation, Quaternion.Identity);

        public bool IsImportedBodySampleTransactionActive => _isImportedBodySampleTransactionActive;

        /// <summary>
        /// Captures mutable humanoid values that diagnostic evaluation is allowed to change.
        /// The caller is responsible for separately preserving the transform hierarchy.
        /// </summary>
        internal HumanoidDiagnosticState CaptureDiagnosticState()
        {
            var state = new HumanoidDiagnosticState
            {
                BodyFrameDiagnostic = _currentBodyFrameDiagnostic,
                LastNativeFrameAccepted = _lastNativeFrameAccepted,
            ...
            {
                foreach ((EHumanoidValue value, float amount) in _muscleValues)
                    state.MuscleValues[value] = amount;
                foreach ((EHumanoidValue value, float amount) in _rawHumanoidValues)
                    state.RawHumanoidValues[value] = amount;
                _importedTranslationDofValues.CopyTo(state.ImportedTranslationDofValues, 0);
                state.ImportedTranslationDofMask = _importedTranslationDofMask;
                state.HasInvalidImportedTranslationDof = _hasInvalidImportedTranslationDof;
            }

            foreach ((EHumanoidValue value, float amount) in Settings.CurrentValues)
                state.SettingsCurrentValues[value] = amount;

            return state;
        }

        /// <summary>
        /// Restores a state captured by <see cref="CaptureDiagnosticState"/> without re-applying a pose.
        /// </summary>
        internal void RestoreDiagnosticState(HumanoidDiagnosticState state)
        {
            ...
            {
                _muscleValues.Clear();
                foreach ((EHumanoidValue value, float amount) in state.MuscleValues)
                    _muscleValues[value] = amount;

                _rawHumanoidValues.Clear();
                foreach ((EHumanoidValue value, float amount) in state.RawHumanoidValues)
                    _rawHumanoidValues[value] = amount;

                state.ImportedTranslationDofValues.CopyTo(_importedTranslationDofValues, 0);
                _importedTranslationDofMask = state.ImportedTranslationDofMask;
                _hasInvalidImportedTranslationDof = state.HasInvalidImportedTranslationDof;
            }

            Settings.CurrentValues.Clear();
            foreach ((EHumanoidValue value, float amount) in state.SettingsCurrentValues)
                Settings.CurrentValues[value] = amount;

            _currentBodyFrameDiagnostic = state.BodyFrameDiagnostic;
            _lastNativeFrameAccepted = state.LastNativeFrameAccepted;
        ...
        /// <summary>
        /// Verifies exact restoration after a diagnostic evaluation transaction.
        /// </summary>
        internal bool DiagnosticStateMatches(HumanoidDiagnosticState state)
        {
            ArgumentNullException.ThrowIfNull(state);

            if (!_currentBodyFrameDiagnostic.Equals(state.BodyFrameDiagnostic)
                || _lastNativeFrameAccepted != state.LastNativeFrameAccepted
                || !_canonicalImportedBodySample.Equals(state.CanonicalImportedBodySample)
                || !_currentImportedMappedBodySample.Equals(state.CurrentImportedMappedBodySample)
                || !_stagedImportedBodySample.Equals(state.StagedImportedBodySample)
                || _currentConvertedBodyTranslationDelta != state.CurrentConvertedBodyTranslationDelta
                || _currentConvertedBodyRotationDelta != state.CurrentConvertedBodyRotationDelta
                || _currentProjectedRootPose != state.CurrentProjectedRootPose
                || _bodyAllocationProjectedRootPose != state.BodyAllocationProjectedRootPose
            ...
            lock (_muscleValuesLock)
            {
                if (!DictionaryContentsMatch(_muscleValues, state.MuscleValues)
                    || !DictionaryContentsMatch(_rawHumanoidValues, state.RawHumanoidValues)
                    || _importedTranslationDofMask != state.ImportedTranslationDofMask
                    || _hasInvalidImportedTranslationDof != state.HasInvalidImportedTranslationDof
                    || !_importedTranslationDofValues.AsSpan().SequenceEqual(state.ImportedTranslationDofValues))
                    return false;
            }

            return DictionaryContentsMatch(Settings.CurrentValues, state.SettingsCurrentValues);
        }

        private static bool DictionaryContentsMatch<TKey, TValue>(
            ...
            where TKey : notnull
        {
            ...
            {
                if (!current.TryGetValue(key, out TValue? currentValue) || !comparer.Equals(currentValue, value))
                    return false;
            }
        ...
        /// <summary>
        /// Starts staging scalar RootT/RootQ members for an evaluator-owned canonical reference.
        /// Direct clips pass their cached time-zero reference every evaluation; state machines may
        /// capture their own provisional reference after their first completed evaluation.
        /// </summary>
        public bool BeginImportedBodySampleTransaction(
            object owner,
            ...
            ReadOnlySpan<float> canonicalProjectionMuscles = default)
        {
            ...
            if (!TryValidateImportedBodyWeight(weight))
            {
                LogRejectedImportedHumanoidFrame("Body/root blend weight was non-finite or outside [0,1]");
                return false;
            }

            if (hasCanonicalSample && !IsValidImportedBodySample(canonicalSample))
            {
                LogRejectedImportedHumanoidFrame("canonical Body/root sample contained a non-finite position or invalid rotation");
                return false;
            }
            ...
            _importedBodySampleWeight = weight;
            ...
            _hasProjectionMuscleValueSnapshot = projectionSettings is not null
                && ImportedHumanoidRootMotionPolicy.TryCreate(
                    projectionSettings,
                    out ImportedHumanoidRootMotionPolicy projectionPolicy,
                    out _)
        ...
        /// <summary>
        /// Atomically applies the RootT/RootQ values staged by the current evaluator.
        /// </summary>
        private bool TryValidateActiveImportedBodySampleTransaction()
        {
            if (!_isImportedBodySampleTransactionActive)
                return true;

            if (TryValidateImportedBodySampleTransaction())
                return true;

            LogRejectedImportedHumanoidFrame(
                "staged Body/root sample contained a non-finite position, invalid rotation, or invalid weight");
            return false;
        }

        private bool CommitActiveImportedBodySampleTransaction()
        {
            object? owner = _importedBodySampleTransactionOwner;
            return !_isImportedBodySampleTransactionActive
                || owner is not null && CommitImportedBodySampleTransaction(owner);
        }

        private void CancelActiveImportedBodySampleTransaction()
        {
            object? owner = _importedBodySampleTransactionOwner;
            if (owner is not null)
                CancelImportedBodySampleTransaction(owner);
        }

        private bool TryValidateImportedBodySampleTransaction()
        {
            if (!TryValidateImportedBodyWeight(_importedBodySampleWeight))
                return false;

            if (_transactionHasCanonicalImportedBodySample
                && !IsValidImportedBodySample(_canonicalImportedBodySample))
                return false;

            return _stagedImportedBodySample.Channels == EHumanoidImportedBodySampleChannels.None
                || IsValidImportedBodySample(_stagedImportedBodySample);
        }

        private static bool TryValidateImportedBodyWeight(float weight)
            => float.IsFinite(weight) && weight >= 0.0f && weight <= 1.0f;

        private static bool IsValidImportedBodySample(HumanoidImportedBodySample sample)
        {
            EHumanoidImportedBodySampleChannels channels = sample.Channels;
            if ((channels & EHumanoidImportedBodySampleChannels.Position) != 0
                && !IsFinite(sample.Position))
                return false;

            return (channels & EHumanoidImportedBodySampleChannels.Rotation) == 0
                || TryNormalizeImportedBodyRotation(sample.Rotation, out _);
        }

        private void LogRejectedImportedHumanoidFrame(string reason)
        {
            if (_loggedRejectedImportedHumanoidFrame)
                return;

            _loggedRejectedImportedHumanoidFrame = true;
            Debug.Animation($"[RootMotion] Rejected imported humanoid frame: {reason}. The previous pose/root remains committed.");
        }

        public bool CommitImportedBodySampleTransaction(object owner)
        {
            if (!_isImportedBodySampleTransactionActive || !ReferenceEquals(_importedBodySampleTransactionOwner, owner))
                return false;
            ...
            _isImportedBodySampleTransactionActive = false;
            _importedBodySampleTransactionOwner = null;
            if (_stagedImportedBodySample.Channels == EHumanoidImportedBodySampleChannels.None)
            {
                _stagedImportedBodySample = HumanoidImportedBodySample.Neutral;
            ...
            if (!_transactionHasCanonicalImportedBodySample)
            {
                _canonicalImportedBodySample = _stagedImportedBodySample;
                _canonicalImportedBodySampleOwner = owner;
            }

            ApplyImportedBodySample(
                _stagedImportedBodySample,
        ...
        /// <summary>
        /// Discards a partially evaluated RootT/RootQ sample after an evaluation failure.
        /// </summary>
        public void CancelImportedBodySampleTransaction(object owner)
        {
            if (!_isImportedBodySampleTransactionActive || !ReferenceEquals(_importedBodySampleTransactionOwner, owner))
                return;
        ...
        /// <summary>
        /// Applies RootT as a complete one-shot body sample when no evaluator transaction is active.
        /// </summary>
        public void SetRootPosition(Vector3 position)
        {
            if (_isImportedBodySampleTransactionActive)
            {
                _stagedImportedBodySample.Position = position;
                _stagedImportedBodySample.Channels |= EHumanoidImportedBodySampleChannels.Position;
                return;
            }

            HumanoidImportedBodySample sample = _currentImportedMappedBodySample;
            sample.Position = position;
            sample.Channels |= EHumanoidImportedBodySampleChannels.Position;
            ApplyManualBodySample(sample);
        }

        public void SetRootPositionX(float x)
            => StageImportedBodyScalar(EHumanoidImportedBodySampleChannels.PositionX, x);

        public void SetRootPositionY(float y)
            => StageImportedBodyScalar(EHumanoidImportedBodySampleChannels.PositionY, y);

        public void SetRootPositionZ(float z)
            => StageImportedBodyScalar(EHumanoidImportedBodySampleChannels.PositionZ, z);
    ...
    public LimbSignOverrides DebugForearmSigns = LimbSignOverrides.Default;
        ...
        /// <summary>
        /// Applies RootQ as a complete one-shot body sample when no evaluator transaction is active.
        /// </summary>
        public void SetRootRotation(Quaternion rotation)
        {
            if (_isImportedBodySampleTransactionActive)
            {
                _stagedImportedBodySample.Rotation = rotation;
                _stagedImportedBodySample.Channels |= EHumanoidImportedBodySampleChannels.Rotation;
                return;
            }

            HumanoidImportedBodySample sample = _currentImportedMappedBodySample;
            sample.Rotation = rotation;
            sample.Channels |= EHumanoidImportedBodySampleChannels.Rotation;
            ApplyManualBodySample(sample);
        }

        public void SetRootRotationX(float x)
            => StageImportedBodyScalar(EHumanoidImportedBodySampleChannels.RotationX, x);

        public void SetRootRotationY(float y)
            => StageImportedBodyScalar(EHumanoidImportedBodySampleChannels.RotationY, y);

        public void SetRootRotationZ(float z)
            => StageImportedBodyScalar(EHumanoidImportedBodySampleChannels.RotationZ, z);

        public void SetRootRotationW(float w)
            => StageImportedBodyScalar(EHumanoidImportedBodySampleChannels.RotationW, w);

        /// <summary>
        /// Clears staged, current, and canonical imported body data. The surrounding reset path restores the Hips bind pose.
        /// </summary>
        public void ResetRootMotionBaseline()
        {
            ...
            _currentImportedMappedBodySample = HumanoidImportedBodySample.Neutral;
            _currentConvertedBodyTranslationDelta = Vector3.Zero;
        ...
        /// <summary>
        /// Discards a clip-owned canonical body reference without modifying the current skeletal pose.
        /// </summary>
        public void DiscardCanonicalImportedBodySample(object owner)
        {
            if (!ReferenceEquals(_canonicalImportedBodySampleOwner, owner))
                return;
        ...
        private void StageImportedBodyScalar(EHumanoidImportedBodySampleChannels channel, float value)
        {
            if (!_isImportedBodySampleTransactionActive)
            {
                if (!_loggedScalarImportedBodyChannelWithoutTransaction)
                {
                    _loggedScalarImportedBodyChannelWithoutTransaction = true;
                    Debug.Animation("[RootMotion] Ignored scalar RootT/RootQ channel outside an imported body sample transaction.");
                }
                return;
            }

            if ((channel & EHumanoidImportedBodySampleChannels.Position) != 0)
                _stagedImportedBodySample.SetPositionComponent(channel, value);
            else
                _stagedImportedBodySample.SetRotationComponent(channel, value);
        }

        private void ApplyImportedBodySample(
            HumanoidImportedBodySample sample,
            HumanoidImportedBodySample canonicalSample,
            float weight,
            object? owner,
            ImportedHumanoidClipRootMotionSettings? projectionSettings,
            HumanoidProjectedRootPose? projectionPrefix = null,
            HumanoidLoopPoseCorrection? loopPoseCorrection = null)
        {
            ...
            ImportedHumanoidRootMotionPolicy? policy = projectionSettings is not null
                && ImportedHumanoidRootMotionPolicy.TryCreate(
                    projectionSettings,
                    out ImportedHumanoidRootMotionPolicy compiledPolicy,
                    out _)
                        ? compiledPolicy
                        : null;
            if (!ReferenceEquals(_activeImportedBodyProjectionOwner, owner))
            ...
            UpdateProjectedRootMotion(
                ...
                policy.HasValue ? compiled.HumanScale * compiled.ModelUnitsPerMeter : weightedMotionScale,
                policy.HasValue ? 1.0f : weight,
                owner,
                projectionSettings,
                projectionPrefix);
            _pendingImportedBodyLoopPoseCorrection = loopPoseCorrection;

            // Native pose evaluation owns the only Hips commit, including
            // one-shot/manual Body channels without an imported root policy.
        }

        private void UpdateProjectedRootMotion(
            ...
            HumanoidProjectedRootPose? projectionPrefix)
        {
            ...
            HumanoidProjectedRootPose projectedPose = CalculateProjectedRootPose(
        ...
        internal HumanoidProjectedRootPose CalculateProjectedRootPose(
            HumanoidImportedBodySample currentSample,
            HumanoidImportedBodySample canonicalSample,
            float weight,
            ImportedHumanoidClipRootMotionSettings? settings,
            ReadOnlySpan<float> muscleValues)
        {
            Vector3 canonicalPosition = IsFinite(canonicalSample.Position)
                ? canonicalSample.Position
                : Vector3.Zero;
            Vector3 currentPosition = IsFinite(currentSample.Position)
                ? currentSample.Position
                : canonicalPosition;
            Quaternion canonicalRotation = NormalizeImportedBodyRotationPure(
                canonicalSample.Rotation,
                Quaternion.Identity);
            Quaternion currentRotation = NormalizeImportedBodyRotationPure(
            ...
            return CalculateProjectedRootPose(
                ...
                currentRotation,
                canonicalRotation,
                EstimateAnimatedMotionScale() * safeWeight,
                safeWeight,
                settings,
                muscleValues);
        }
            ...
            ImportedHumanoidRootMotionPolicy policy)
        {
            ...
            if (policy.Mirror)
            {
                currentImportedPosition = ImportedHumanoidMirrorOperator.MirrorPosition(currentImportedPosition);
                canonicalImportedPosition = ImportedHumanoidMirrorOperator.MirrorPosition(canonicalImportedPosition);
                currentImportedRotation = ImportedHumanoidMirrorOperator.MirrorRotation(currentImportedRotation);
                canonicalImportedRotation = ImportedHumanoidMirrorOperator.MirrorRotation(canonicalImportedRotation);
            }
        ...
        /// </summary>
        internal bool TryCalculateLoopEvaluation(
            ...
            out HumanoidProjectedRootPose projectedRootGenerator)
        {
            ...
            if (!IsValidImportedBodySample(sourceStart)
                || !IsValidImportedBodySample(sourceEnd)
                || !ImportedHumanoidRootMotionPolicy.TryCreate(settings, out ImportedHumanoidRootMotionPolicy policy, out _)
                || !TryGetCompiledAvatarDefinition(out CompiledHumanoidAvatarDefinition compiled))
                return false;

            bool needsEndpointPose = policy.LoopPose
                || (policy.LoopTime && !policy.BakePositionYIntoPose
                    && policy.PositionYBasis is EImportedHumanoidRootPositionYBasis.Feet);
            if (!needsEndpointPose)
            {
                projectedRootGenerator = CalculateProjectedRootPose(
                    sourceEnd, sourceStart, 1.0f, settings, ReadOnlySpan<float>.Empty);
                return IsFinite(projectedRootGenerator.Position) && IsFiniteNonZero(projectedRootGenerator.Rotation);
            }

            // Endpoint FK is required data for this policy. A failed solve must
            // reject the input cache, never substitute a Body-only approximation.
            return sourceStartMuscles.Length >= MuscleValueCount
        ...
        {
            requestedBodyPose = Matrix4x4.Identity;
            ...
            // Loop Pose corrects the requested Body frame, never an already
            // compensated Hips pose. Endpoint sidecars do not mutate the rig.
            requestedBodyPose = CalculateRequestedBodyFrame(compiled, policy);
            return HumanoidBodyFrameMath.IsRigid(requestedBodyPose)
                && _nativePoseWorkspace.TryAlignBody(compiled, requestedBodyPose);
        }
        ...
        /// <summary>
        /// Resolves Feet projection from scratch FK aligned to the requested Body.
        /// Only root bookkeeping changes here; the final requested frame removes
        /// the extracted channel before the single compensated skeleton commit.
        /// </summary>
        ...
        private Matrix4x4 CalculateImportedBodyAllocatedLocalPose(
            CompiledHumanoidAvatarDefinition compiled,
            Matrix4x4 hipsLocalPose,
            ImportedHumanoidRootMotionPolicy policy)
        {
            Matrix4x4 requestedBody = CalculateRequestedBodyFrame(compiled, policy);
            if (!Matrix4x4.Invert(_nativePoseWorkspace.ProvisionalBodyFrame, out Matrix4x4 inverseProvisional)
                || !Matrix4x4.Invert(_nativePoseWorkspace.HipsParentInModelRoot, out Matrix4x4 inverseParent))
                return default;
            Matrix4x4 compensation = inverseProvisional * requestedBody;
            return hipsLocalPose * _nativePoseWorkspace.HipsParentInModelRoot * compensation * inverseParent;
        }
            ...
            HumanoidLoopPoseCorrection? loopPoseCorrection)
        {
                ...
                && loopCorrection.Phase > 0.0f)
            {
                ...
                {
                    // Direct and state endpoint caches use unit-weight clip
                    // inputs. The complete corrected target is weighted once
                    // by their caller. Correct Body channels
                    // directly so a rotational seam does not orbit the model origin.
            ...
            float weight)
        {
            ...
            Vector3 canonicalMappedPosition = SanitizeImportedBodyPosition(
                ...
                Vector3.Zero);
            if (policy.Mirror)
                canonicalMappedPosition = ImportedHumanoidMirrorOperator.MirrorPosition(canonicalMappedPosition);
            ...
            if (policy.PositionXZBasis is EImportedHumanoidRootPositionXZBasis.CenterOfMass)
            {
            ...
            if (policy.PositionYBasis is EImportedHumanoidRootPositionYBasis.CenterOfMass)
            ...
            Quaternion canonicalMappedRotation = NormalizeImportedBodyRotationPure(
                ...
                Quaternion.Identity);
            if (policy.Mirror)
                canonicalMappedRotation = ImportedHumanoidMirrorOperator.MirrorRotation(canonicalMappedRotation);
            Quaternion canonicalOrientationAllocation = policy.OrientationBasis
                is EImportedHumanoidRootOrientationBasis.Body
                    ? Quaternion.Slerp(Quaternion.Identity, Quaternion.Inverse(ExtractYawRotation(canonicalMappedRotation)), weight)
                    : Quaternion.Identity;
            ...
            out float feetY)
        {
            ...
            Matrix4x4 projectedHipsLocal = CalculateImportedBodyAllocatedLocalPose(
                compiled,
                _nativePoseWorkspace.GetLocalMatrix(EHumanoidAvatarBoneRole.Hips),
                policy);
            Matrix4x4 projectedHipsInModelRoot = projectedHipsLocal
                * _nativePoseWorkspace.HipsParentInModelRoot;
            Matrix4x4 solvedHipsInModelRoot = _nativePoseWorkspace.GetModelRootMatrix(
                EHumanoidAvatarBoneRole.Hips);
            if (!Matrix4x4.Invert(solvedHipsInModelRoot, out Matrix4x4 inverseSolvedHips))
            {
                feetY = 0.0f;
                return false;
            }
            ...
            out float bottomY)
        {
            ...
            {
                Matrix4x4 footInHips = workspace.GetModelRootMatrix(footRole)
                    * inverseSolvedHips;
                Vector3 footOffset = footInHips.Translation;
            ...
            {
                Matrix4x4 toesInHips = workspace.GetModelRootMatrix(toesRole)
                    * inverseSolvedHips;
                Vector3 toesOffset = toesInHips.Translation;
        ...
        internal static HumanoidRootMotionDelta CalculateProjectedRootDelta(
            HumanoidProjectedRootPose previous,
            HumanoidProjectedRootPose current)
        {
            EHumanoidProjectedRootChannels channels = previous.Channels & current.Channels;
            Vector3 translation = Vector3.Zero;
            Quaternion rotation = Quaternion.Identity;

            if ((channels & EHumanoidProjectedRootChannels.PositionXZ) != 0)
            {
                translation.X = current.Position.X - previous.Position.X;
                translation.Z = current.Position.Z - previous.Position.Z;
            }

            if ((channels & EHumanoidProjectedRootChannels.PositionY) != 0)
                translation.Y = current.Position.Y - previous.Position.Y;

            if ((channels & EHumanoidProjectedRootChannels.RotationYaw) != 0)
            {
                rotation = Quaternion.Normalize(
                    Quaternion.Inverse(previous.Rotation) * current.Rotation);
            }

            return new HumanoidRootMotionDelta(translation, rotation, channels);
        }
        ...
        private static float GetEffectiveOrientationOffsetDegrees(ImportedHumanoidRootMotionPolicy policy)
        {
        ...
        private void ResetProjectedRootMotion()
        {
            ...
            _currentRootMotionDelta = HumanoidRootMotionDelta.Identity;
            _hasPreviousProjectedRootPose = false;
            _projectedRootMotionOwner = null;
        }

        private Vector3 SanitizeImportedBodyPosition(Vector3 position, Vector3 fallback)
        {
            if (float.IsFinite(position.X) && float.IsFinite(position.Y) && float.IsFinite(position.Z))
                return position;

            if (!_loggedInvalidImportedBodyPosition)
            {
                _loggedInvalidImportedBodyPosition = true;
                Debug.Animation("[RootMotion] Imported RootT sample was non-finite; using the canonical body reference.");
            }
            return fallback;
        }

        private Quaternion NormalizeImportedBodyRotation(Quaternion rotation, Quaternion fallback)
        {
            if (TryNormalizeImportedBodyRotation(rotation, out Quaternion normalizedRotation))
                return normalizedRotation;

            if (!_loggedInvalidImportedBodyRotation)
            {
                _loggedInvalidImportedBodyRotation = true;
                Debug.Animation("[RootMotion] Imported RootQ sample was non-finite or near zero; using the canonical body reference.");
            }

            return NormalizeImportedBodyRotationPure(fallback, Quaternion.Identity);
        }

        private static Quaternion NormalizeImportedBodyRotationPure(Quaternion rotation, Quaternion fallback)
            => TryNormalizeImportedBodyRotation(rotation, out Quaternion normalizedRotation)
                ? normalizedRotation
                : TryNormalizeImportedBodyRotation(fallback, out Quaternion normalizedFallback)
                    ? normalizedFallback
                    : Quaternion.Identity;

        private static bool TryNormalizeImportedBodyRotation(Quaternion rotation, out Quaternion normalizedRotation)
        {
            ...
            if (float.IsFinite(rotation.X) && float.IsFinite(rotation.Y) && float.IsFinite(rotation.Z) && float.IsFinite(rotation.W) &&
                float.IsFinite(lengthSquared) && lengthSquared > 0.00000001f)
            {
                normalizedRotation = Quaternion.Normalize(rotation);
                return true;
            }
        ...
        private float SanitizeImportedBodyWeight(float weight)
        {
            if (float.IsFinite(weight) && weight >= 0.0f && weight <= 1.0f)
                return weight;

            if (!_loggedInvalidImportedBodyWeight)
            {
                _loggedInvalidImportedBodyWeight = true;
                Debug.Animation("[RootMotion] Imported body sample weight was invalid or outside [0,1]; clamping it once for this humanoid.");
            }

            return float.IsFinite(weight) ? Math.Clamp(weight, 0.0f, 1.0f) : 1.0f;
        }
        ...
        private void ResetMappedTransformsToBindPose(bool includeEyesTarget)
        {
            if (TryGetCompiledAvatarDefinition(out CompiledHumanoidAvatarDefinition compiled))
            {
                for (int roleIndex = 0; roleIndex < CompiledHumanoidAvatarDefinition.RoleCount; roleIndex++)
                    if (compiled.Nodes[roleIndex] is SceneNode node)
                        ResetNodeToHumanoidBindPose(node);
