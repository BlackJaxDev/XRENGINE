...
internal static class UnityAnimationEventDispatcher
{
    ...
    public static int Dispatch(XRComponent owner, in UnityAnimationEventOccurrence occurrence)
    {
        int receiverCount = 0;
        foreach (XRComponent component in owner.SceneNode.Components)
        {
            if (component is IUnityAnimationEventReceiver typedReceiver)
            {
                typedReceiver.ReceiveUnityAnimationEvent(occurrence);
                receiverCount++;
                continue;
            }
    ...
    {
        if (parameterType == typeof(UnityAnimationEventOccurrence))
            return EArgumentKind.Occurrence;
        if (parameterType == typeof(UnityAnimationEvent))
            return EArgumentKind.Event;
        if (parameterType == typeof(UnityAssetReference))
    ...
    private static void Invoke(
        ...
        DispatchPlan plan,
        in UnityAnimationEventOccurrence occurrence)
    {
