...
/// </summary>
public sealed class UnityHumanoidBoneResponseProfile
{
    public string BoneName { get; set; } = string.Empty;
    public UnityHumanoidMuscleResponse[] Responses { get; set; } = [];

    public Quaternion Evaluate(ReadOnlySpan<float> muscles, float muscleInputScale)
    {
        Quaternion result = Quaternion.Identity;
        foreach (UnityHumanoidMuscleResponse response in Responses)
        {
