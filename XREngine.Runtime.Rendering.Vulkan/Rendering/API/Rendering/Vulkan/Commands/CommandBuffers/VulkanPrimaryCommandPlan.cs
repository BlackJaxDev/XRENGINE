...
internal sealed class VulkanPrimaryCommandPlan
{
    ...
    {
        foreach (var barrier in barrierPlanner.GetBarriersForPass(passIndex))
        ...
        foreach (var barrier in barrierPlanner.GetBufferBarriersForPass(passIndex))
            if (barrier.SrcQueueFamilyIndex != Silk.NET.Vulkan.Vk.QueueFamilyIgnored &&
                barrier.DstQueueFamilyIndex != Silk.NET.Vulkan.Vk.QueueFamilyIgnored &&
                barrier.SrcQueueFamilyIndex != barrier.DstQueueFamilyIndex)
                return true;

        foreach (var barrier in barrierPlanner.GetSwapchainBarriersForPass(passIndex))
            if (barrier.SrcQueueFamilyIndex != Silk.NET.Vulkan.Vk.QueueFamilyIgnored &&
                barrier.DstQueueFamilyIndex != Silk.NET.Vulkan.Vk.QueueFamilyIgnored &&
                barrier.SrcQueueFamilyIndex != barrier.DstQueueFamilyIndex)
                return true;
