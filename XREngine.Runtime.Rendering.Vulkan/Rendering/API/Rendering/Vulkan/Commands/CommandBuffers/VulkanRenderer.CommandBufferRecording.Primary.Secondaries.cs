...
namespace XREngine.Rendering.Vulkan
{
    public unsafe partial class VulkanRenderer
    {
        ...
        private bool TryExecuteIndirectCommandChainSecondaryRun(scoped ref PrimaryCommandBufferRecordingState recordingState, int startIndex, int runCount, int passIndex, IndirectDrawOp firstDraw)
        {
        ...
        private bool TryExecuteScheduledMeshCommandChainSecondaryRun(scoped ref PrimaryCommandBufferRecordingState recordingState, int startIndex, int runCount, int passIndex, MeshDrawOp firstDraw)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < runCount; i++)
                {
                    ...
                    if (!needsRecording)
                    {
                        ...
                        {
                            MeshDrawOp refreshDraw = (MeshDrawOp)recordingState.Ops[chain.SourceStartIndex + drawIndex];
                            ...
                            {
                                _lastReusableFrameDataRefreshFailureReason =
                                    $"scheduled chain op={chain.SourceStartIndex + drawIndex}/{recordingState.Ops.Length} mesh='{refreshDraw.Draw.Renderer.MeshRenderer.Mesh?.Name ?? "<unnamed mesh>"}' slot={refreshUniformSlot}: " +
        ...
        private bool TryExecuteMeshCommandChainSecondaryRun(scoped ref PrimaryCommandBufferRecordingState recordingState, int startIndex, int runCount, int passIndex, MeshDrawOp firstDraw)
        {
