...
namespace XREngine.Rendering.Vulkan
{
    public unsafe partial class VulkanRenderer
    {
        ...
        internal ulong GetBufferAllocationOffset(Buffer buffer)
            => ResourceRuntime.Buffers.GetAllocationOffset(buffer);
        ...
        internal bool TryMapBufferMemory(
            ...
            ulong length,
            out void* mappedPtr)
            => ResourceRuntime.Buffers.TryMap(
        ...
        internal (Buffer stagingBuffer, DeviceMemory stagingMemory) CreateReadbackBuffer(ulong bufferSize)
            => ReadbackOutputResources.CreateStagingBuffer(
                BackendObjectContext,
                bufferSize,
                "GenericReadback.Staging");
        ...
        internal void DestroyBufferRaw(Buffer? buffer, DeviceMemory? memory)
        {
            ...
            if (resolvedBuffer.Handle != 0)
                ResourceRuntime.Allocations.Staging.TryForget(resolvedBuffer, resolvedMemory);
        ...
        internal bool TryMapReadbackMemory(Buffer buffer, DeviceMemory memory, ulong offset, ulong length, out void* mappedPtr)
            => ResourceRuntime.Buffers.TryMapReadback(
                BackendObjectContext,
                buffer,
                memory,
                offset,
