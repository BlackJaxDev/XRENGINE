...
/// </summary>
internal sealed class VulkanUpscaleBridge : IVulkanUpscaleBridge
{
    ...
    public bool TryResolveCurrentFrame(out VulkanUpscaleBridgeFrameInfo frame)
    {
        if (!TryResolveCurrentFrameSlot(out VulkanUpscaleBridgeFrameSlot? slot) || slot is null)
        {
            frame = default;
            return false;
        }

        frame = new VulkanUpscaleBridgeFrameInfo(
            slot.SlotIndex,
            slot.SourceColorFrameBuffer,
            slot.SourceDepthFrameBuffer,
            slot.SourceMotionFrameBuffer,
            slot.ExposureFrameBuffer,
            slot.OutputColorFrameBuffer);
        return true;
    }
        ...
        global::XREngine.VulkanUpscaleBridgeCapabilitySnapshot snapshot)
    {
        ...
        try
        {
            ...
            {
                if (_viewport.Window?.Renderer is not IRuntimeRendererHost rendererHost
                    || !rendererHost.TryGetBackendCapability<IOpenGlVendorUpscaleBackendCapability>(out var renderer)
                    || renderer is null)
                {
                    DestroyInteropResources();
                    SidecarDeviceOwned = false;
        ...
        out string failureReason)
    {
        ...
        try
        {
            ...
            renderer.SignalExternalTextureSemaphore(slot.GlReadySemaphore, readyTextureIds);
            ...
            renderer.WaitExternalTextureSemaphore(slot.GlCompleteSemaphore, outputColorTextureId);
    ...
    public bool TryExecuteVendorUpscale(
        IOpenGlVendorUpscaleBackendCapability renderer,
        XRFrameBuffer sourceColorFbo,
        XRFrameBuffer sourceDepthFbo,
        XRFrameBuffer sourceMotionFbo,
        ...
        out string failureReason)
    {
        ...
        try
        {
            ...
            renderer.SignalExternalTextureSemaphore(slot.GlReadySemaphore, readyTextureIds[..readyCount]);
            slot.SourceColor.CurrentLayout = Silk.NET.Vulkan.ImageLayout.General;
            slot.SourceDepth.CurrentLayout = Silk.NET.Vulkan.ImageLayout.General;
            slot.SourceMotion.CurrentLayout = Silk.NET.Vulkan.ImageLayout.General;
            if (parameters.HasExposureTexture)
        ...
        out string reason)
    {
        ...
        if (_viewport.Window?.Renderer is not IRuntimeRendererHost rendererHost
            || !rendererHost.TryGetBackendCapability<IOpenGlVendorUpscaleBackendCapability>(out _))
        {
    ...
    private static bool TryGetGlTextureBinding(
        IOpenGlVendorUpscaleBackendCapability renderer,
        XRTexture texture,
        out uint bindingId)
        => renderer.TryResolveTextureBinding(texture, out bindingId);
}
