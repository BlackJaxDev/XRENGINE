...
internal unsafe class VkFrameBuffer(VulkanRenderer api, XRFrameBuffer data) : VkObject<XRFrameBuffer>(api, data)
{
    ...
    {
        if (!VulkanResourceBindingView.TryParse(resourceName.AsSpan(), out VulkanResourceBindingView binding) ||
            binding.Kind != EVulkanResourceBindingKind.FrameBuffer ||
            !binding.Name.Equals(frameBufferName.AsSpan(), StringComparison.OrdinalIgnoreCase) ||
            binding.Slot.IsWhiteSpace())
        {
            slot = default;
            return false;
        }
    ...
    private static ImageLayout ToVkImageLayout(
        ...
        => layout switch
        {
            RenderGraphImageLayout.ColorAttachment => AttachmentRoleClassifier.IsColorLike(signature.Role)
                ? ImageLayout.ColorAttachmentOptimal
                : ImageLayout.DepthStencilAttachmentOptimal,
            RenderGraphImageLayout.DepthStencilAttachment => AttachmentRoleClassifier.IsColorLike(signature.Role)
                ? ImageLayout.ColorAttachmentOptimal
                : ImageLayout.DepthStencilAttachmentOptimal,
            RenderGraphImageLayout.RenderingLocalRead => ImageLayout.RenderingLocalRead,
            RenderGraphImageLayout.ShaderReadOnly => AttachmentRoleClassifier.IsColorLike(signature.Role)
    ...
    private static ImageLayout ResolveAttachmentReferenceLayout(
        ...
        => AttachmentRoleClassifier.IsColorLike(signature.Role)
            ? ImageLayout.ColorAttachmentOptimal
    ...
    internal static bool UsesReadOnlyDepthStencil(FrameBufferAttachmentSignature[] signatures)
    {
        ...
        {
            if (AttachmentRoleClassifier.IsColorLike(signature.Role))
                continue;
    ...
    private static FrameBufferAttachmentSignature BuildAttachmentSignature(AttachmentSource source, AttachmentRole role, uint colorIndex)
    {
        ...
        ImageLayout finalLayout = AttachmentRoleClassifier.IsColorLike(role)
        ...
        ImageLayout referenceLayout = AttachmentRoleClassifier.IsColorLike(role)
            ? ImageLayout.ColorAttachmentOptimal
            : ImageLayout.DepthStencilAttachmentOptimal;
