...
internal sealed partial class VulkanFrameLoop
{
        ...
        out string? failure)
    {
        ...
        try
        {
            ...
            Result endResult = _commandRuntime.EndCommandBufferTracked(slot.CommandBuffer);
    ...
    internal void PollScreenshotReadbacks()
    {
        if (!RuntimeEngine.IsRenderThread)
        {
    ...
    internal ScreenshotReadbackStatus GetScreenshotReadbackStatus()
    {
        int submitted = 0;
        int cpuProcessing = 0;
        int abandoned = 0;
        ...
        out string? failure)
    {
        ...
        if (slot.Fence.Handle == 0)
        {
            ...
            _deviceContext.SetDebugObjectName(ObjectType.Fence, slot.Fence.Handle, $"ScreenshotReadback[{slotIndex}].Fence");
        }
        ...
        out string? failure)
    {
        bool ensured = ReadbackOutputResources.EnsureScreenshotResolveImage(
                ...
                out failure);
        if (created)
            _deviceContext.SetDebugObjectName(ObjectType.Image, slot.ResolveImage.Handle, $"ScreenshotReadback[{slotIndex}].Resolve");
        return ensured;
    }
    ...
    internal void DrainScreenshotReadbacksForShutdown()
    {
        if (_deviceLost)
        {
            OutputRuntime.Capture.FailPendingScreenshotReadbacksForDeviceLoss(
