...
internal sealed unsafe partial class VulkanFrameLoop
{
    ...
    private FrameOpContext LegacyPrepareResourcePlannerForFrameOps(FrameOp[] ops, ulong frameOpsSignature = 0)
    {
        if (ops.Length == 0)
        {
        ...
        FrameOpContext primary = VulkanFramePlanner.SelectPrimaryPlannerContext(ops);
        ...
        out ulong plannerRevision)
    {
        ...
        int currentPassMetadataRevision = VulkanFramePlanner.ComputePassMetadataRevisionStamp(fastPathKey.ActivePassMetadata);
        ...
        bool preserveActiveKeys = false)
    {
        ...
        preparedState.LastActiveFrameOpContext = VulkanFramePlanner.SelectPrimaryPlannerContext(ops, key);
        ...
        out string failureReason)
    {
        ...
        for (int keyIndex = 0; keyIndex < planKeyCount; keyIndex++)
        {
            ...
            FrameOpResourcePlannerPreparationToken preparationToken =
                BeginFrameOpResourcePlannerPreparation(partition);
            try
            {
                ...
                _ = PublishFrameOpResourcePlannerPreparationState(preparationToken);
            ...
            {
                EndFrameOpResourcePlannerPreparation(preparationToken);
            }
        }
        switchingState.ActiveKeys.Clear();
        for (int keyIndex = 0; keyIndex < planKeyCount; keyIndex++)
        ...
        in VulkanFrameOpPlannerStateKey key)
    {
        ...
        for (int index = 0; index < operations.Length; index++)
            if (VulkanFramePlanner.FrameOpContextMatchesPlannerStateKey(operations[index].Context, key))
                count++;
        ...
        for (int index = 0; index < operations.Length; index++)
        {
            ...
            if (VulkanFramePlanner.FrameOpContextMatchesPlannerStateKey(operation.Context, key))
    ...
    private void PruneFrameOpResourcePlannerStatesToCapacity(FrameOpResourcePlannerSwitchingState switchingState)
    {
        ...
        for (int i = 0; i < pruneCount; i++)
        {
            ...
            if (!IsAllocatorOwnedByFrameOpPlannerState(switchingState, state.ResourceAllocator))
            {
                ...
                if (ResourceAllocator.TryRetirePhysicalResources(BackendObjectContext))
    ...
    private void CollectFrameOpPlannerStateKeys(FrameOp[] ops, List<VulkanFrameOpPlannerStateKey> keys)
    {
        for (int i = 0; i < ops.Length; i++)
        {
            ...
            if (!VulkanFramePlanner.FrameOpContextHasPlannerResources(context))
                continue;

            VulkanFrameOpPlannerStateKey key = VulkanFramePlanner.BuildFrameOpPlannerStateKey(context);
        ...
        out VulkanFrameOpPlannerStateKey key)
    {
        ...
        for (int i = 0; i < ops.Length; i++)
        {
            ...
            VulkanFrameOpPlannerStateKey candidate = VulkanFramePlanner.BuildFrameOpPlannerStateKey(context);
            if (!found)
            {
                key = candidate;
                found = true;
        ...
        out VulkanFrameOpPlannerStateKey key)
    {
        ...
        {
            if (!VulkanFramePlanner.FrameOpContextMatchesPlannerStateKey(context, activeKey))
                continue;
    ...
    private string DescribeActiveFrameOpPlannerStateKeys(
        ...
        FramePlan? framePlan)
    {
        VulkanFrameOpPlannerStateKey requested = VulkanFramePlanner.BuildFrameOpPlannerStateKey(requestedContext);
        FrameOpResourcePlannerSwitchingState switchingState = ActiveFrameOpResourcePlannerSwitchingState;
    ...
    private void LegacyRetireResourcePlannerRuntimeStateAllocator(
        ...
        string reason)
    {
        ...
        RestoreResourcePlannerRuntimeState(state);
        _ = allocator.TryRetirePhysicalResources(BackendObjectContext);
    }

    private void LegacyRetireResourcePlannerRuntimeStateAllocators(
        in ResourcePlannerRuntimeState state,
        HashSet<VulkanResourceAllocator> retiredAllocators,
        string reason)
    {
        ...
        in FrameOpContext context)
    {
        ...
        if (!VulkanFramePlanner.FrameOpContextMatchesPlannerStateKeyIgnoringRegistry(lastContext, key))
            throw new InvalidOperationException($"Cached frame-op planner context does not match key {key}.");
        if (!VulkanFramePlanner.FrameOpContextMatchesPlannerStateKey(context, key))
    ...
    private static HashSet<int>? BuildActiveFrameOpPassSet(FrameOp[] ops, in VulkanFrameOpPlannerStateKey key)
    {
        ...
        foreach (FrameOp op in ops)
        {
            if (!VulkanFramePlanner.FrameOpMatchesPlannerStateKey(op, key))
                continue;
        ...
        in VulkanFrameOpPlannerStateKey plannerKey)
    {
        ...
        {
            var (DisplayWidth, DisplayHeight, InternalWidth, InternalHeight) = VulkanFramePlanner.ResolveExternalFrameOpResourceDimensions(
                externalExtent,
        ...
        if (_targetDriver is VulkanDesktopWsiTargetDriver { IsInteractiveResizeInProgress: true })
        ...
        if (!refreshExtents)
        {
            foreach (FrameOp op in ops)
            {
                if (filterByPlannerKey && !VulkanFramePlanner.FrameOpMatchesPlannerStateKey(op, plannerKey))
                    continue;
    ...
    private VulkanResourceExtentContext LegacyBuildResourceExtentContext(in FrameOpContext context)
    {
        if (TryResolveExternalSwapchainTargetExtent(out Extent2D externalExtent))
        {
            var (DisplayWidth, DisplayHeight, InternalWidth, InternalHeight) = VulkanFramePlanner.ResolveExternalFrameOpResourceDimensions(
