...
internal sealed unsafe partial class VulkanDescriptorManager
{
    ...
    /// </summary>
    internal bool SupportsDescriptorIndexing => DeviceContext.Capabilities.Supports(EVulkanDeviceCapability.DescriptorIndexing);
        ...
        out bool usesVariableDescriptorCount)
    {
        ...
        lock (_descriptorSetLayoutCacheLock)
        {
            ...
            ResourceRuntime.RegisterDescriptorSetLayout(layout, $"DescriptorLayoutCache.Set{setIndex}");
    ...
    /// </summary>
    internal void DestroyCachedDescriptorSetLayouts()
    {
        lock (_descriptorSetLayoutCacheLock)
        {
        ...
        out bool usesVariableDescriptorCount)
    {
        ...
        for (int i = 0; i < layoutBindings.Length; i++)
            if (layoutBindings[i].DescriptorType == DescriptorType.Sampler &&
                ResourceRuntime.Descriptors.TryGetCanonicalImmutableSampler(VulkanCanonicalSampler.LinearClamp, out _))
        ...
        try
        {
            if (immutableSamplers is not null &&
                ResourceRuntime.Descriptors.TryGetCanonicalImmutableSampler(VulkanCanonicalSampler.LinearClamp, out Sampler linearClampSampler))
            {
                int samplerOffset = 0;
                for (int i = 0; i < layoutBindings.Length; i++)
                {
            ...
            if (SupportsDescriptorIndexing && layoutBindings.Length > 0)
            {
                ...
                for (int i = 0; i < layoutBindings.Length; i++)
                {
                    ...
                    if (DeviceContext.MutableCapabilities._supportsDescriptorBindingPartiallyBound)
                ...
                fixed (DescriptorBindingFlags* bindingFlagsPtr = bindingFlags)
                {
                    ...
                    fixed (DescriptorSetLayoutBinding* bindingsPtr = layoutBindings)
                    {
                        ...
                        if (Api.CreateDescriptorSetLayout(DeviceContext.Device, ref layoutInfo, null, out layout) != Result.Success)
                            return false;
                    }
                }
