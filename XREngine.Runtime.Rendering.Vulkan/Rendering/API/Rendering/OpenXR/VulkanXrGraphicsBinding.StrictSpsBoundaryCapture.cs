...
internal sealed unsafe partial class VulkanXrGraphicsBinding
{
    ...
    private bool _strictSpsBoundaryMotionCadenceStarted;

    public OpenXrSmokeCaptureLedgerEntry[] GetStrictSpsBoundaryCaptureLedger()
        => Context.WithSmokeDiagnosticsLock<OpenXrSmokeCaptureLedgerEntry[]>(
            () => [.. _strictSpsBoundaryCaptureLedger]);

    private void ResetStrictSpsBoundaryCaptureDiagnostics()
    {
        Context.WithSmokeDiagnosticsLock(_strictSpsBoundaryCaptureLedger.Clear);
        _strictSpsBoundarySuccessfulFrameCount = 0;
        _strictSpsBoundaryCaptureCount = 0;
        _strictSpsBoundaryMotionCadenceStarted = false;
        Phase524bTemporalScenarioDiagnostics.ClearBoundaryCaptureMotion();
    }
        ...
        uint rightImageIndex)
    {
        ...
        // Boundary parity compares different processes. Start its deterministic
        // motion cadence only after imported textures have reached their desired
        // resident generations; otherwise motion 0 can compare a preview mip in
        // one process with the promoted texture in the other.
        if (!Phase524bCaptureReadinessDiagnostics.IsReady(out string readinessReason))
        {
            Debug.RenderingEvery(
                "Phase524b.StrictSpsBoundaryCaptureReadiness.Waiting",
                TimeSpan.FromSeconds(2),
                "[Phase524bCapture] Waiting for stable imported textures before strict-SPS boundary capture: {0}",
                readinessReason);
            return;
        }

        if (Phase524bTemporalScenarioDiagnostics.SequenceFrame <
            Phase524bTemporalScenarioDiagnostics.SequenceCompleteFrame)
        {
            return;
        }

        if (!_strictSpsBoundaryMotionCadenceStarted)
        {
            _strictSpsBoundaryMotionCadenceStarted = true;
            _strictSpsBoundarySuccessfulFrameCount = 0;
        ...
        Context.WithSmokeDiagnosticsLock(
            () => _strictSpsBoundaryCaptureLedger.AddRange(captures));
        _strictSpsBoundaryCaptureCount++;
        if (_strictSpsBoundaryCaptureCount < StrictSpsBoundaryMotionCaptureCount)
        {
            Phase524bTemporalScenarioDiagnostics.SetBoundaryCaptureMotionIndex(
                _strictSpsBoundaryCaptureCount);
        }
        else
        {
