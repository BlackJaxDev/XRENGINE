...
public static class ModelingDocumentToXRMeshConverter
{
    public static XRMesh Convert(ModelingMeshDocument document, ModelingDocumentToXRMeshOptions? options = null)
    {
        ...
        ConversionOrderingResult ordering = BuildOrderingResult(document, options.OrderingPolicy);
        int convertedVertexCount = ordering.NewToOldVertexMap?.Length ?? document.Positions.Count;

        if (convertedVertexCount > ushort.MaxValue)
        {
            throw new InvalidOperationException(
                $"The runtime-mesh converter supports up to {ushort.MaxValue} vertices, but received {convertedVertexCount}.");
        }
        ...
        {
            throw new InvalidOperationException(
                $"Converted mesh vertex count mismatch. Expected {convertedVertexCount}, got {mesh.VertexCount}.");
        }

        int[]? convertedIndices = mesh.GetIndices(EPrimitiveType.Triangles);
        if (convertedIndices is null || convertedIndices.Length != ordering.TriangleIndices.Count)
        {
            throw new InvalidOperationException(
                $"Converted triangle index count mismatch. Expected {ordering.TriangleIndices.Count}, got {convertedIndices?.Length ?? 0}.");
        }

        ApplyRuntimeMeshContract(mesh);
        return mesh;
    }

    private static void EnforceFallbackPolicy(ModelingMeshDocument document, ModelingDocumentToXRMeshOptions options)
    {
        if (options.SkinningBlendshapeFallbackPolicy != ModelingDocumentToXRMeshSkinningBlendshapeFallbackPolicy.Strict)
            return;
    ...
    internal static void ApplyRuntimeMeshContract(XRMesh mesh)
    {
        ArgumentNullException.ThrowIfNull(mesh);
    ...
    private static ConversionOrderingResult BuildOrderingResult(
        ModelingMeshDocument document,
        ModelingDocumentToXRMeshOrderingPolicy orderingPolicy)
    {
        return orderingPolicy switch
        {
            ModelingDocumentToXRMeshOrderingPolicy.PreserveDocumentOrder => new ConversionOrderingResult(document.TriangleIndices, null),
            ModelingDocumentToXRMeshOrderingPolicy.Canonicalized => BuildCanonicalizedOrderingResult(document),
            _ => throw new ArgumentOutOfRangeException(nameof(orderingPolicy), orderingPolicy, "Unknown conversion ordering policy.")
        };
    }

    private static ConversionOrderingResult BuildCanonicalizedOrderingResult(ModelingMeshDocument document)
    {
        if ((document.TriangleIndices.Count % 3) != 0)
            throw new InvalidOperationException("Triangle index count must be divisible by 3 for canonicalized conversion ordering.");
        ...
        return new ConversionOrderingResult(remappedIndices, newToOldVertexMap);
    }
    ...
    private readonly struct ConversionOrderingResult(IReadOnlyList<int> triangleIndices, int[]? newToOldVertexMap)
    {
    ...
    {
        StringBuilder builder = new("Modeling mesh validation failed during runtime-mesh conversion:");
