...
public enum RuntimeCookedBinaryTypeMarker : byte
{
    Null = 0,
    Boolean = 1,
    Byte = 2,
...
{
    public const string ReflectionWarningMessage = "Runtime cooked binary serialization relies on reflection and cannot be statically analyzed for trimming or AOT";
    ...
    public static IDisposable RegisterRuntimeFactory<T>(Func<T> factory)
    {
        ...
        return RegisterRuntimeFactory(typeof(T), () => factory());
    }

    public static IDisposable RegisterRuntimeFactory(Type type, Func<object?> factory)
    {
        ArgumentNullException.ThrowIfNull(type);
        ArgumentNullException.ThrowIfNull(factory);

        RuntimeFactoryEntry entry = new(Guid.NewGuid(), type, factory);
        lock (FactorySync)
        {
            if (RuntimeObjectFactories.ContainsKey(type))
                throw new InvalidOperationException($"A runtime cooked-binary factory for '{type.FullName}' is already installed.");
            RuntimeObjectFactories.Add(type, entry);
        }
    ...
    private sealed record RuntimeFactoryEntry(Guid RegistrationId, Type Type, Func<object?> Factory);

    private sealed class RuntimeFactoryLease(RuntimeFactoryEntry entry) : IDisposable
    {
        private RuntimeFactoryEntry? _entry = entry;

        public void Dispose()
        {
            RuntimeFactoryEntry? current = Interlocked.Exchange(ref _entry, null);
            if (current is null)
                return;

            lock (FactorySync)
            {
                if (RuntimeObjectFactories.TryGetValue(current.Type, out RuntimeFactoryEntry? registered)
                    && registered.RegistrationId == current.RegistrationId)
                {
                    RuntimeObjectFactories.Remove(current.Type);
                }
            }
        }
    }
