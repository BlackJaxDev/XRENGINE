    ...
    public class VPRC_GTAOPass : ViewportRenderCommand
    {
        ...
        protected override void Execute()
        {
            ...
            Debug.RenderingEvery(
                $"AO.GTAO.Execute.{RuntimeHelpers.GetHashCode(instance)}",
                TimeSpan.FromSeconds(1),
                "[AO][GTAO] Execute forceRebuild={0} size={1}x{2} stereo={3} normal={4} depth={5} final={6}",
                forceRebuild,
                width,
                height,
                Stereo,
        ...
        private void GTAOGen_SetUniforms(XRRenderProgram program)
        {
            ...
            Debug.RenderingEvery(
                $"AO.GTAO.GenUniforms.{RuntimeHelpers.GetHashCode(ActivePipelineInstance)}",
                TimeSpan.FromSeconds(1),
                "[AO][GTAO] Gen depthMode={0} region={1}x{2} stereoPass={3}",
                camera.DepthMode,
                region.Width,
                region.Height,
        ...
        private void SetBlurUniforms(XRRenderProgram program, Vector2 direction)
        {
            ...
            Debug.RenderingEvery(
                $"AO.GTAO.BlurUniforms.{RuntimeHelpers.GetHashCode(ActivePipelineInstance)}.{direction}",
                TimeSpan.FromSeconds(1),
                "[AO][GTAO] Blur direction=({0}, {1}) depthMode={2}",
                direction.X,
                direction.Y,
