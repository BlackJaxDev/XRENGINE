...
namespace XREngine.Components.Physics
{
    ...
    public class StaticRigidBodyComponent : PhysicsActorComponent
    {
        ...
        public ModelComponent? TargetModelComponent
        {
            ...
            set => _targetModelComponentRef = value is not null ? new WeakReference<ModelComponent>(value) : null;
        }
        ...
        private void TryScheduleAutoGeneratedConvexColliders()
        {
            ...
            var modelComponent = TargetModelComponent ?? GetSiblingComponent<ModelComponent>();
            if (modelComponent is null)
                return;

            if (HasCollisionMeshData(modelComponent))
            {
            ...
            Engine.AddAppThreadCoroutine(() =>
            {
                ...
                var pendingModelComponent = TargetModelComponent ?? GetSiblingComponent<ModelComponent>();
                if (pendingModelComponent is null)
                {
                    _autoConvexColliderGenerationQueued = false;
                    return true;
                }

                if (!HasCollisionMeshData(pendingModelComponent))
                    return false;
        ...
        private async Task GenerateAndAttachAutoGeneratedConvexCollidersAsync()
        {
            ...
            try
            {
                if (WorldAs<XREngine.Rendering.XRWorldInstance>()?.PhysicsScene is not PhysxScene)
                {
                    ...
                    Debug.LogWarning($"[StaticRigidBodyComponent] Auto-generated CoACD colliders currently require PhysX: {SceneNode?.Name ?? "<unnamed>"}");
                    return;
                }
                ...
                {
                    CompleteConvexHullGeneration("No convex hulls generated.");
                    Debug.LogWarning($"[StaticRigidBodyComponent] No CoACD hulls were generated for {SceneNode?.Name ?? "<unnamed>"}.");
                    return;
                }
                ...
                void AttachShapes()
                {
                    ...
                    {
                        _autoConvexColliderGenerationStarted = false;
                        CompleteConvexHullGeneration("No PhysX material was available for convex collider attachment.");
                        return;
                    }
                    ...
                    for (int i = 0; i < convexMeshes.Count; i++)
                    {
                        PhysxConvexMesh convexMesh = convexMeshes[i];
                            unsafe
                            {
                                var geometry = new IPhysicsGeometry.ConvexMesh(convexMesh.ConvexMeshPtr, Vector3.One, Quaternion.Identity, tightBounds: false);
            ...
            catch (Exception ex)
            {
                ...
                Debug.PhysicsException(ex, $"Failed to auto-generate convex colliders for {SceneNode?.Name ?? "<unnamed>"}.");
            }
        }
