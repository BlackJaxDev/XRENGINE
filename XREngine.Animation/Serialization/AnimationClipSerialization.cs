...
public sealed class AnimationClipYamlTypeConverter : IYamlTypeConverter
{
    ...
    private static AnimationClip? ResolveExternalReference(Guid id)
    {
        ...
        if (services.TryGetAssetById(id, out XRAsset? loadedAsset) && loadedAsset is AnimationClip loadedClip)
            return loadedClip;

        string? referenceAssetPath = services.CurrentDeserializationPath;
        if (!services.TryResolveAssetPathById(id, referenceAssetPath, out string? assetPath)
            || string.IsNullOrWhiteSpace(assetPath)
            || !File.Exists(assetPath))
            return null;

        if (services.TryDeferAssetLoad(assetPath, typeof(AnimationClip), out XRAsset? deferredAsset))
            return deferredAsset as AnimationClip;

        return services.LoadImmediate(assetPath, typeof(AnimationClip)) as AnimationClip;
    }
