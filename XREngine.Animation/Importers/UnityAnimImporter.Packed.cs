...
public static partial class AnimYamlImporter
{
        ...
        int clipSampleRate)
    {
        ...
        if (!TryReadPackedBindingChannels(
            ...
            out UnityAssetReference[] pptrMapping,
        ...
        out string diagnostic)
    {
        ...
        {
            pptrMapping = new UnityAssetReference[mappingSequence.Children.Count];
        ...
        for (int bindingIndex = 0; bindingIndex < genericBindings.Children.Count; bindingIndex++)
        {
            ...
            UnityAssetReference script = ReadAssetReference(GetMappingOrNull(binding, "script"));
            int componentCount = GetPackedBindingComponentCount(classId, attributeHash, isPPtr);
            for (int component = 0; component < componentCount; component++)
            {
                EUnityAnimationBindingValueKind valueKind = GetPackedBindingValueKind(
                    classId,
                    attributeHash,
                    componentCount,
                    isPPtr,
                ...
                bool requiresAdapter = RequiresPackedBindingAdapter(
                    ...
                    script,
                    attribute);
                UnityAnimationBindingDescriptor descriptor = new()
                {
    ...
    private static EUnityAnimationBindingValueKind GetPackedBindingValueKind(
        int classId,
        uint attributeHash,
        int componentCount,
        bool isPPtr,
    ...
    {
        if (isPPtr)
            return EUnityAnimationBindingValueKind.ObjectReference;
        if (isInt)
            return EUnityAnimationBindingValueKind.Integer;
        if (classId is 4 or 224)
        {
            return attributeHash switch
            {
                1 or 3 => EUnityAnimationBindingValueKind.Vector3,
                2 => EUnityAnimationBindingValueKind.Quaternion,
                4 => EUnityAnimationBindingValueKind.Euler,
                _ => EUnityAnimationBindingValueKind.Float,
            };
        }
        return componentCount switch
        {
            ...
            4 => EUnityAnimationBindingValueKind.Vector4,
            _ => EUnityAnimationBindingValueKind.Float,
        };
    }
    ...
    private static bool TryGetPackedHumanoidAttribute(uint attributeId, out string attribute)
    {
        ...
        if ((uint)muscleIndex < (uint)UnityHumanoidMuscleMap.OrderedMuscleEntries.Count)
        {
            attribute = UnityHumanoidMuscleMap.OrderedMuscleEntries[muscleIndex].CurveAttributeName;
            return true;
        }
    ...
    private static bool IsNativePackedHumanoidAttribute(string attribute)
        => TryMapRootMotionComponent(attribute, out _, out _)
            ...
            || UnityHumanoidMuscleMap.TryGetValue(attribute, out _);
    ...
    private static bool TryDecodeStreamedClip(
        ...
        UnityAssetReference[] pptrMapping,
        List<ScalarCurve> scalarDestination,
        List<ObjectCurve> objectDestination,
        out int curveCount,
        out string diagnostic)
    ...
    private static bool TryAppendPackedCurve(
        ...
        out string diagnostic)
    {
        UnityAnimationBindingDescriptor descriptor = binding.Descriptor;
        ...
        UnityAnimationImportManifestBuilder manifestBuilder)
    {
        ...
        for (int i = 0; i < curves.Count; i++)
        {
            ...
            UnityAnimationBindingDescriptor? descriptor = curve.BindingDescriptor;
            if (descriptor?.ValueKind != EUnityAnimationBindingValueKind.Quaternion
        ...
        UnityAssetReference[] mapping)
    {
        ...
        Dictionary<string, string> resolvedPaths = ResolveUnityGuidPaths(clipFilePath, requestedGuids);
