...
public sealed class ModelImportBackendRegistry
{
    ...
    /// <summary>
    /// Gets the process registry populated with the built-in ModelAssetPipeline backends.
    ...
    /// <summary>
    /// Registers a descriptor for the lifetime of the returned lease. The exact
    /// descriptor installed by this call is removed during teardown.
    /// </summary>
    public IDisposable Install(ModelImportBackendDescriptor descriptor)
    {
        Register(descriptor);
        return new RegistrationLease(this, descriptor);
    }
    ...
    private void Remove(ModelImportBackendDescriptor descriptor)
    {
        lock (_sync)
        {
            if (_descriptors.TryGetValue(descriptor.StableId, out ModelImportBackendDescriptor? current)
                && ReferenceEquals(current, descriptor))
            {
                _descriptors.Remove(descriptor.StableId);
            }
        }
    }

    private sealed class RegistrationLease(
        ModelImportBackendRegistry registry,
        ModelImportBackendDescriptor descriptor) : IDisposable
    {
        private ModelImportBackendRegistry? _registry = registry;

        public void Dispose()
            => Interlocked.Exchange(ref _registry, null)?.Remove(descriptor);
    }
}
