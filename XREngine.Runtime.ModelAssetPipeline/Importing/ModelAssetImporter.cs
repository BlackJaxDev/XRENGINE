...
{
    /// <summary>
    /// Imports external model assets through the selected FBX, glTF, or Assimp producer.
    /// Returns a SceneNode hierarchy populated with model components and outputs generated materials and meshes.
    /// </summary>
    [XRTypeRedirect("XREngine.ModelImporter")]
    public class ModelAssetImporter : IDisposable
    {
        ...
        public ModelAssetImporter(string path, Action? onCompleted, DelMaterialFactory? materialFactory)
        {
        ...
        public static EnumeratorJob ScheduleImportJob(
            string path,
            ModelImportSteps options,
            Action<ModelAssetImportResult>? onFinished = null,
            ...
            int layer = DefaultLayers.DynamicIndex)
        {
            ...
            var job = RuntimeModelImportServices.Current.Schedule(
                ImportRoutine,
                progress: onProgress,
                completed: null,
                error: ex =>
            ...
            foreach (string rawSearchPath in searchPaths)
            {
                ...
                catch (Exception ex)
                {
                    if (_invalidTextureSearchPathWarnings.TryAdd(rawSearchPath, true))
                        LogImportExpectedWarning(modelFilePath, $"[ModelAssetImporter] Ignoring invalid texture search path '{rawSearchPath}'. {ex.Message}");
                    continue;
                }
                ...
                if (!Directory.Exists(searchPath))
                {
                    if (_invalidTextureSearchPathWarnings.TryAdd(searchPath, true))
                        LogImportExpectedWarning(modelFilePath, $"[ModelAssetImporter] Texture search path does not exist: '{searchPath}'");
                    continue;
                }
                ...
                catch (Exception ex)
                {
                    if (_invalidTextureSearchPathWarnings.TryAdd($"{searchPath}|scan", true))
                        LogImportExpectedWarning(modelFilePath, $"[ModelAssetImporter] Failed to scan texture search path '{searchPath}'. {ex.Message}");
                }
            }
        ...
        private string ResolveTextureFilePath(string modelFilePath, string rawPath, out bool exists)
        {
            ...
            if (_missingTexturePathWarnings.TryAdd(candidate, true))
                LogImportExpectedWarning(modelFilePath, $"[ModelAssetImporter] Texture path not found: '{candidate}' (from '{rawPath}', model='{modelFilePath}')");
        ...
        public static SceneNode? Import(
            ...
            bool? batchSubmeshAddsDuringAsyncImport = null)
        {
            ModelAssetImporter? importer = null;
            int completed = 0;
            ...
            importer = new ModelAssetImporter(path, CompleteImport, materialFactory);
            try
            {
        ...
        private static ModelAssetImportResult ImportInternal(
            string path,
            PostProcessSteps options,
            SceneNode? parent,
            ...
            Action<ModelAssetImportResult>? onFinished,
            DelMaterialFactory? materialFactory,
        ...
        {
            ModelAssetImporter? importer = null;
            ModelAssetImportResult? result = null;
            ...
            {
                ModelAssetImportResult? resultToInvoke = null;
                ModelAssetImporter? importerToDispose = null;
                ...
                try
                {
                    if (resultToInvoke is ModelAssetImportResult completedResult)
            ...
            importer = new ModelAssetImporter(path, CompleteMeshProcessing, materialFactory);
            try
            {
                ...
                ModelAssetImportResult importResult = new(
                    node,
                    importer._materials,
                    importer._meshes,
        ...
        public static ShaderVar[] CreateDefaultForwardPlusUberShaderParameters(float bumpScale = 0.0f, bool usesHeightMap = false)
        {
            ...
            [
                ...
                new ShaderVector4(identitySt, "_ToonRamp_ST"),
                new ShaderVector2(Vector2.Zero, "_ToonRampPan"),
                new ShaderInt(0, "_ToonRampUV"),
                new ShaderVector4(identitySt, "_FirstShadeMap_ST"),
                new ShaderVector2(Vector2.Zero, "_FirstShadeMapPan"),
                new ShaderInt(0, "_FirstShadeMapUV"),
                new ShaderVector4(identitySt, "_SecondShadeMap_ST"),
                new ShaderVector2(Vector2.Zero, "_SecondShadeMapPan"),
                new ShaderInt(0, "_SecondShadeMapUV"),
                ...
                new ShaderVector4(identitySt, "_RimColorTexture_ST"),
                new ShaderVector2(Vector2.Zero, "_RimColorTexturePan"),
                new ShaderInt(0, "_RimColorTextureUV"),
                new ShaderInt(0, "_RimMaskUV"),
                ...
                new ShaderVector2(Vector2.Zero, "_DissolveNoiseTexturePan"),
                new ShaderInt(0, "_DissolveNoiseTextureUV"),
                new ShaderVector4(identitySt, "_DissolveDetailNoise_ST"),
                new ShaderVector2(Vector2.Zero, "_DissolveDetailNoisePan"),
                new ShaderInt(0, "_DissolveDetailNoiseUV"),
                new ShaderVector4(identitySt, "_DissolveMask_ST"),
                new ShaderVector2(Vector2.Zero, "_DissolveMaskPan"),
                new ShaderInt(0, "_DissolveMaskUV"),
                new ShaderVector4(identitySt, "_DissolveEdgeGradient_ST"),
                new ShaderVector4(identitySt, "_DissolveEdgeTexture_ST"),
                new ShaderVector2(Vector2.Zero, "_DissolveEdgeTexturePan"),
                new ShaderInt(0, "_DissolveEdgeTextureUV"),
                new ShaderFloat(1.0f, "_DissolveDetailStrength"),
                new ShaderFloat(0.0f, "_DissolveMaskInvert"),
                ...
                new ShaderVector4(identitySt, "_PBRSmoothnessMaps_ST"),
                new ShaderVector2(Vector2.Zero, "_PBRSmoothnessMapsPan"),
                new ShaderInt(0, "_PBRSmoothnessMapsUV"),
                new ShaderInt(0, "_PBRSmoothnessMapsChannel"),
                new ShaderFloat(0.0f, "_PBRSmoothnessMapInvert"),
            ...
            Matrix4x4? rootTransformMatrix = null)
        {
            ...
            try
            {
                ...
                LogImportDiagnostic(
                    SourceFilePath,
                    "[ModelAssetImporter.Import] Resolved backends for '{0}': requested={1}, hostPreference={2}, candidates=[{3}], candidateHash={4}, resolverVersion={5}.",
                    SourceFilePath,
                ...
                for (int candidateIndex = 0; candidateIndex < backendResolution.Candidates.Count; candidateIndex++)
                {
                    ...
                    if (candidate.StableId.Equals(ModelImportBackendIds.NativeFbx, StringComparison.Ordinal))
                    {
                        try
                        {
                            ...
                            LogImportDiagnostic(
                                SourceFilePath,
                                "[ModelAssetImporter.Import] Using native FBX backend '{0}' for '{1}'.",
                                candidate,
                                SourceFilePath);
                    ...
                    if (!candidate.StableId.Equals(ModelImportBackendIds.Assimp, StringComparison.Ordinal))
                    {
                        NotSupportedException unsupportedBackend = new(
                            $"ModelAssetImporter has no execution adapter for registered backend '{candidate.StableId}'.");
                        if (!hasFallback)
                            throw unsupportedBackend;

                        LogImportWarning(
                            SourceFilePath,
                            $"[ModelAssetImporter.Import] {unsupportedBackend.Message} Trying '{fallbackId}'.");
                        continue;
                    }

                    LogImportDiagnostic(
                        SourceFilePath,
                        "[ModelAssetImporter.Import] Using Assimp backend '{0}' for '{1}'.",
                        candidate,
                        SourceFilePath);
                    ...
                    AScene scene;
                    using (runtimeServices.StartProfileScope($"Assimp ImportFile: {SourceFilePath} with options: {effectiveImportOptions.ImportSteps}"))
                    {
                        assimpProgress?.Invoke(0.02f);
                        RuntimeEngine.Rendering.Stats.GpuMeshlets.RecordMeshletSourceParserEntry();
                        scene = _assimp.ImportFile(SourceFilePath, effectiveImportOptions.ImportSteps.ToAssimp());
                        assimpProgress?.Invoke(0.12f);
                    }

                    if (scene is null || scene.SceneFlags == SceneFlags.Incomplete || scene.RootNode is null)
                    {
                        LogImportWarning(SourceFilePath, $"[ModelAssetImporter.Import] Assimp returned null/incomplete scene for '{SourceFilePath}'. scene={scene != null}, flags={scene?.SceneFlags}, rootNode={scene?.RootNode != null}");
                        _onCompleted?.Invoke();
                        return null;
                    }
            ...
            bool? batchSubmeshAddsDuringAsyncImport)
        {
            ...
            resolved.LegacyPostProcessSteps = options.ToModelImportSteps();
            return resolved;
        }
        ...
        {
            using var t = RuntimeModelImportServices.Current.StartProfileScope("Processing meshes on job thread");
            ...
            Action? beforeCompleted = null)
        {
            ...
            int availableWorkers = Math.Max(1, RuntimeModelImportServices.Current.WorkerCount - 2);
            ...
            void TryFlushAndComplete()
            {
                ...
                RuntimeModelImportServices.Current.EnqueueAppThread(() =>
                {
                    try
                    {
                        ...
                        if (Volatile.Read(ref faulted) != 0 || Volatile.Read(ref canceled) != 0)
                            LogImportWarning(SourceFilePath, $"[ModelAssetImporter] Mesh processing completed with partial failures for '{SourceFilePath}'. faulted={faulted}, canceled={canceled}");
                ...
                }, "ModelAssetImporter.FinalizeAsyncMeshProcessing");
            }
            ...
            void ScheduleMeshBatch(int batchIndex)
            {
                ...
                RuntimeModelImportServices.Current.Schedule(
                    BatchRoutine,
                    completed: () =>
                    {
                        TryFinalize();
        ...
        {
            using var t = RuntimeModelImportServices.Current.StartProfileScope("Normalizing node scales");
            ...
            bool batchSubmeshAddsDuringAsyncImport)
        {
            ...
            IEnumerable MeshRoutine(int meshIndex)
            {
                ...
                if (pending != null)
                {
                    ...
                    if (!batchSubmeshAddsDuringAsyncImport)
                        RuntimeModelImportServices.Current.EnqueueAppThread(FlushReadySubMeshes, "ModelAssetImporter.FlushReadySubMeshes");
                }
                else
                {
        ...
        private unsafe (XRMesh mesh, XRMaterial material) ProcessSubMesh(
            ...
            CancellationToken cancellationToken)
        {
            using var t = RuntimeModelImportServices.Current.StartProfileScope($"Processing submesh for {mesh.Name}");

            cancellationToken.ThrowIfCancellationRequested();

            XRMesh xrMesh = AssimpMeshFactory.Create(mesh, _nodeCache, dataTransform);
            cancellationToken.ThrowIfCancellationRequested();
        ...
        {
            using var t = RuntimeModelImportServices.Current.StartProfileScope($"Processing material for {meshName}");
