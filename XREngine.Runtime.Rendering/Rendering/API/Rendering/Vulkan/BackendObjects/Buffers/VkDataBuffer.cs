...
namespace XREngine.Rendering.Vulkan
{
    public unsafe partial class VulkanRenderer
    {
        ...
        /// <summary>
        /// Tracks buffer handles created by this renderer so teardown paths can skip
        /// duplicate stale destroys before validation sees an invalid handle.
        /// </summary>
        private readonly System.Collections.Concurrent.ConcurrentDictionary<ulong, byte> _liveBufferHandles = new();
        ...
        public class VkDataBuffer(VulkanRenderer renderer, XRDataBuffer buffer) : VkObject<XRDataBuffer>(renderer, buffer), IApiDataBuffer
        {
                    ...
                    if (_vkBuffer.HasValue || _vkMemory.HasValue)
                    {
                        ...
                        else
                        {
                            ...
                            if (_vkMemory.HasValue)
                                Renderer.FreeUntrackedBufferMemory(_vkMemory.Value, "VkDataBuffer.PushData.PartialState");
                        }
                        _vkBuffer = null;
                        _vkMemory = null;
            ...
            protected override void DeleteObjectInternal()
            {
                ...
                else
                {
                    ...
                    if (_vkBuffer.HasValue)
                        Renderer.DestroyBufferRaw(_vkBuffer.Value, null);
                    if (_vkMemory.HasValue)
                        Renderer.FreeUntrackedBufferMemory(_vkMemory.Value, "VkDataBuffer.DeleteObjectInternal.PartialState");
                }
            ...
            out bool freedMemory)
        {
            ...
            if (_bufferAllocations.TryRemove(buffer.Handle, out VulkanMemoryAllocation allocation))
            {
            ...
            if (_legacyBufferAllocations.TryRemove(buffer.Handle, out VulkanMemoryAllocation legacyAllocation))
            {
