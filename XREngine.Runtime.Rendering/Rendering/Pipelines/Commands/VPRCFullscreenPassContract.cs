...
internal static class VPRCFullscreenPassContract
{
        ...
        bool stereo)
    {
        ...
        int destinationWidth = checked((int)destination.Width);
        int destinationHeight = checked((int)destination.Height);
        if (area.X != 0 ||
            area.Y != 0 ||
        ...
        uint attachmentLayers = ValidateDestinationAttachments(passName, destination, stereo);
    ...
    private static uint ValidateDestinationAttachments(string passName, XRFrameBuffer destination, bool stereo)
    {
        ...
        for (int i = 0; i < targets.Length; i++)
        {
            ...
            ValidateAttachmentExtent(passName, destination, target, mipLevel);
        ...
        int mipLevel)
    {
        ...
        if (width != destination.Width || height != destination.Height)
        {
            throw new InvalidOperationException(
                ...
                $"AttachmentMip={mipLevel} Extent={width}x{height} FBO={destination.Width}x{destination.Height}.");
        }
    }
