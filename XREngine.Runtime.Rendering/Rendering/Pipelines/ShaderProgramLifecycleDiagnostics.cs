...
namespace XREngine.Rendering
{
    ...
    /// <para/>
    /// Counters are aggregated across all OpenGL render-program instances and
    /// emitted as a single <c>[ShaderProgramSummary]</c> log line via
    /// <see cref="LogSummary"/>, typically during renderer cleanup.
    /// <para/>
    ...
    public static class ShaderProgramLifecycleDiagnostics
    {
        ...
        /// plus optional asynchronous binary-upload totals when supplied.
        /// Safe to call any time; intended for shutdown.
        /// </summary>
        public static void LogSummary(ShaderProgramBinaryUploadSummary? binaryUploadSummary = null)
        {
            ...
            long uploadCompleted = binaryUploadSummary?.CompletedCount ?? 0L;
            long uploadFailed = binaryUploadSummary?.FailedCount ?? 0L;
            long uploadBackpressure = binaryUploadSummary?.BackpressureCount ?? 0L;
            long uploadCoalesced = binaryUploadSummary?.CoalescedCount ?? 0L;
