...
namespace XREngine.Rendering
{
    ...
    public partial class FontGlyphSet : XRAsset
    {
        ...
        private static XRFontImportOptions ResolveImportOptions(string filePath)
        {
            try
            {
                return RuntimeEngine.Assets.GetOrCreateThirdPartyImportOptions(filePath, typeof(FontGlyphSet)) as XRFontImportOptions ?? new XRFontImportOptions();
            }
            catch
            {
        ...
        {
            string path = RuntimeEngine.Assets.ResolveEngineAssetPath(
                RuntimeEngine.Rendering.Constants.EngineFontsCommonFolderName,
                folderName,
                fontName);
            ...
            FontGlyphSet font = RuntimeEngine.Assets.Load<FontGlyphSet>(path)
            ...
            if (ShouldForceMsdfReload(font, path) && ForcedReloadAttemptedPaths.TryAdd(path, 0))
            {
                ...
                FontGlyphSet? reloaded = RuntimeEngine.Assets.Load<FontGlyphSet>(path, JobPriority.Highest, true);
        ...
        {
            bool logTiming = RuntimeEngine.StartingUp || RuntimeEngine.StartupPresentationEnabled;
            ...
            string? cacheAssetPath = RuntimeEngine.Assets.TryResolveThirdPartyCachePath(path, typeof(FontGlyphSet), importProfileKey, out string resolvedCacheAssetPath)
            ...
            FontGlyphSet font = RuntimeEngine.Assets.Load3rdPartyVariantWithCache<FontGlyphSet>(path, importOptions, importProfileKey, JobPriority.Highest, bypassJobThread: true)
            ...
            if (TryGetLayoutResourceIssue(font, out string? layoutIssue))
            {
                ...
                font = RuntimeEngine.Assets.Load3rdPartyVariantWithCache<FontGlyphSet>(path, importOptions, importProfileKey, JobPriority.Highest, bypassJobThread: true)
        ...
        {
            string engineAssetsPath = Path.GetFullPath(RuntimeEngine.Assets.EngineAssetsPath);
            ...
            string? cacheRoot = RuntimeEngine.Assets.GameCachePath;
            if (string.IsNullOrWhiteSpace(cacheRoot))
            {
                string? projectRoot = Path.GetDirectoryName(Path.GetFullPath(RuntimeEngine.Assets.GameAssetsPath));
        ...
        private static void EvictLoadedAsset(FontGlyphSet font, string path)
            => RuntimeEngine.Assets.Evict(font, path);
        ...
        public static async Task<FontGlyphSet> LoadEngineFontAsync(string folderName, string fontName)
            => await RuntimeEngine.Assets.LoadEngineAssetAsync<FontGlyphSet>(
                RuntimeEngine.Rendering.Constants.EngineFontsCommonFolderName,
                folderName,
                fontName);
