    ...
    public static partial class RuntimeEngine
    {
        public static partial class Rendering
        {
            ...
            public static class Debug
            {
                ...
                static Debug()
                {
                    RuntimeRenderingHostServices.Scheduling.SubscribePreUpdateFrame(PreUpdate);
                    RuntimeRenderingHostServices.Scheduling.SubscribeViewportSwapBuffers(SwapBuffers);
                }
                ...
                private sealed class DebugLineOverlayState
                {
                    ...
                    public void AddBox(ReadOnlySpan<Vector3> points, ColorF4 color, float lineWidth)
                    {
                        ...
                        bool isRenderThread = RuntimeEngine.IsRenderThread;
                ...
                {
                    using var sample = RuntimeRenderingHostServices.Profiling.StartProfileScope("Rendering.Debug.SwapBuffers");
                    LastDebugDrawComponentCallbackCount =
                        Interlocked.Exchange(ref _debugDrawComponentCallbacks, 0);

                    if (RuntimeRenderingHostServices.FrameTiming.IsShuttingDown)
                    {
                        ClearQueues();
                        ClearDebugShapeBags();
                    ...
                    RuntimeDebugShapePopulationMode mode =
                        RuntimeRenderingHostServices.FrameTiming.DebugShapePopulationMode;

                    try
                    {
                        switch (mode)
                        {
                            case RuntimeDebugShapePopulationMode.Tasks:
                                PopulateDebugScene(_debug3D);
                                PopulateDebugScene(_debug2D);
                                break;
                            case RuntimeDebugShapePopulationMode.ParallelInvoke:
                                PopulateDebugScene(_debug3D);
                                PopulateDebugScene(_debug2D);
                                break;
                            case RuntimeDebugShapePopulationMode.Sequential:
                            default:
                                PopulateDebugScene(_debug3D);
                                PopulateDebugScene(_debug2D);
                                break;
                    ...
                    catch (OperationCanceledException) when (RuntimeRenderingHostServices.FrameTiming.IsShuttingDown)
                    {
                        ClearQueues();
                    }
                    catch (AggregateException ex) when (RuntimeRenderingHostServices.FrameTiming.IsShuttingDown && IsCancellationOnly(ex))
                    {
                        ClearQueues();
                    }
                }
                ...
                {
                    float pointSize = RuntimeRenderingHostServices.FrameTiming.DebugPointSize;
                    float lineWidth = RuntimeRenderingHostServices.FrameTiming.DebugLineWidth;
                    _debug3D.Visualizer.PointSize = pointSize;
                    _debug3D.Visualizer.LineWidth = lineWidth;
                    _debug3D.DepthTestedVisualizer.PointSize = pointSize;
                    _debug3D.DepthTestedVisualizer.LineWidth = lineWidth;
                    _debug2D.Visualizer.PointSize = pointSize;
                    _debug2D.Visualizer.LineWidth = lineWidth;
                    _debug2D.DepthTestedVisualizer.PointSize = pointSize;
                    _debug2D.DepthTestedVisualizer.LineWidth = lineWidth;
                }
                ...
                {
                    if (!RuntimeEngine.Rendering.State.IsMainPass)
                        return;

                    if (RuntimeEngine.Rendering.State.RenderingCamera is { } camera &&
                    ...
                    if (!depthTested && !scene.Texts.IsEmpty)
                    {
                        ...
                        for (int i = 0; i < hashes.Length; i++)
                        {
                            ...
                            float nowTime = RuntimeRenderingHostServices.FrameTiming.ElapsedTime;
                            float lastTime = text.lastUpdatedTime;
                            if (nowTime - lastTime > RuntimeRenderingHostServices.FrameTiming.DebugTextMaxLifespan &&
                                scene.Texts.TryRemove(hash, out (UIText text, float lastUpdatedTime) item))
                ...
                private static EDebugPrimitiveSceneKind ResolveDebugPrimitiveSceneKind()
                    => RuntimeEngine.IsRenderThread &&
                        RuntimeEngine.Rendering.State.RenderingScene is VisualScene2D
                ...
                public static void RenderQuad(
                    ...
                    => RenderQuad(
                        centerTranslation,
                        Rotator.FromQuaternion(rotation),
                        extents,
                        solid,
                        color,
                        depthTested);
                ...
                private static void UpdateDebugText(DebugPrimitiveSceneState scene, Vector3 worldPosition, string text, ColorF4 color, float scale = DefaultDebugTextScale)
                {
                    ...
                    scene.Texts.AddOrUpdate(hash, _ =>
                    {
                        ...
                        t.Scale = scale;
                        return (t, RuntimeRenderingHostServices.FrameTiming.ElapsedTime);
                    }, (_, pair) =>
                    {
                        var t = pair.text;
                        t.Text = text;
