...
namespace XREngine
{
    public static partial class RuntimeEngine
    {
        public static partial class Rendering
        {
            ...
            private static readonly Dictionary<XRViewport, IVulkanUpscaleBridge> _vulkanUpscaleBridges = new(System.Collections.Generic.ReferenceEqualityComparer.Instance);
            ...
            internal static IVulkanUpscaleBridge? GetVulkanUpscaleBridge(XRViewport? viewport)
            {
                if (viewport is null)
                    return null;

                lock (_vulkanUpscaleBridgeRegistrySync)
                    return _vulkanUpscaleBridges.TryGetValue(viewport, out IVulkanUpscaleBridge? bridge)
                        ? bridge
                        : null;
            }
            ...
            public static EVulkanUpscaleBridgeState PrepareVulkanUpscaleBridgeForFrame(XRViewport? viewport, XRRenderPipelineInstance? pipeline)
            {
                ...
                lock (_vulkanUpscaleBridgeRegistrySync)
                {
                    if (!_vulkanUpscaleBridges.TryGetValue(viewport, out bridge!))
                    {
                        ...
                        bridge = service.CreateBridge(viewport);
            ...
            {
                IVulkanUpscaleBridge[] bridges;
                lock (_vulkanUpscaleBridgeRegistrySync)
                    bridges = [.. _vulkanUpscaleBridges.Values];

                foreach (IVulkanUpscaleBridge bridge in bridges)
            ...
            internal static void ReleaseAllVulkanUpscaleBridges(string reason)
            {
                KeyValuePair<XRViewport, IVulkanUpscaleBridge>[] bridges;
                lock (_vulkanUpscaleBridgeRegistrySync)
                {
                ...
                foreach (KeyValuePair<XRViewport, IVulkanUpscaleBridge> bridge in bridges)
                ...
                IOpenGlExternalInteropBackendCapability renderer)
            {
                ...
                IRuntimeVendorUpscaleService? service = RuntimeVendorUpscaleService.Current;
                VulkanUpscaleBridgeProbeSnapshot probe = service?.ProbeBridge(
                    State.OpenGLVendor,
                    State.OpenGLRendererName) ?? default;
            ...
            public static string DescribeVulkanUpscaleBridgeUnavailability(XRViewport? viewport, bool hdrRequested)
            {
                ...
                if (GetVulkanUpscaleBridge(viewport) is IVulkanUpscaleBridge bridge)
                {
            ...
            private static int CountWindowViewports(XRWindow? window)
            {
                ...
                int count = 0;
                foreach (var _ in RuntimeEngine.EnumerateActiveViewports(window))
                    count++;
