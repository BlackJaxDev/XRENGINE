...
namespace XREngine.Components
{
    ...
    public class CameraComponent : XRComponent
    {
        ...
        public IRuntimeInputControllablePawn? GetUsingPawn()
        {
            var player = GetUsingLocalPlayer();
            if (player?.ControlledPawnComponent is IRuntimeInputControllablePawn pawn && ReferenceEquals(pawn.RuntimeCameraComponent, this))
                return pawn;

            if (SceneNode is not null)
                foreach (var component in SceneNode.Components)
                    if (component is IRuntimeInputControllablePawn siblingPawn && ReferenceEquals(siblingPawn.RuntimeCameraComponent, this))
                        return siblingPawn;
        ...
        private IRuntimeScreenSpaceUserInterface? _userInterface;
        /// <summary>
        ...
        public IRuntimeScreenSpaceUserInterface? UserInterface
        {
            get => _userInterface;
        ...
        /// </summary>
        /// <returns></returns>
        public IRuntimeScreenSpaceUserInterface? GetUserInterfaceOverlay()
        {
            ...
            if (SceneNode is not null)
                foreach (var component in SceneNode.Components)
                    if (component is IRuntimeScreenSpaceUserInterface ui)
                        return ui;
                ...
                switch (propName)
                {
                    ...
                    case nameof(UserInterface):
                        if (UserInterface is { IsScreenSpace: false } && _camera.IsValueCreated)
                            UserInterface.ClearCameraSpaceCamera(Camera);
                        break;
                }
            }
            return change;
        ...
        private void ViewportResized(XRViewport viewport)
        {
            ...
            {
                UserInterface.ResizeScreenSpace(viewport.Region.Size);
            }
        ...
        private void CameraResized(XRCameraParameters parameters)
        {
            ...
            ResizeCameraSpaceUserInterface(Camera, parameters);
        }

        private void ResizeCameraSpaceUserInterface(XRCamera camera, XRCameraParameters parameters)
        {
            if (UserInterface is { IsScreenSpace: false } ui)
                ui.ResizeCameraSpace(camera, parameters);
        }
        ...
        //    if (WorldAs<XREngine.Rendering.IRuntimeRenderWorld>()?.VisualScene?.RenderablesTree is not I3DRenderTree tree)
        //        return bounces;
