...
{
    /// <summary>
    /// Re-publishes the authored mesh bounds with conservative Poiyomi vertex
    ...
    /// </summary>
    internal void RefreshPoiyomiVertexCullingBounds()
    {
        ...
        RenderInfo.LocalCullingVolume = ExpandPoiyomiLocalBounds(_bindPoseBounds, material);
        PublishRenderCommandCullingVolume();
    }

    private AABB ExpandPoiyomiLocalBounds(in AABB bounds, XRMaterial? material)
    {
        if (material?.Parameter<ShaderFloat>("_PoiVertexEffectsEnabled")?.Value is not > 0.0001f)
            return ExpandPoiyomiOutlineBounds(bounds, material);

        Vector3 halfExtents = bounds.HalfExtents;
        Vector3 padding = GetAbsolute(material, "_PoiConservativeBounds");
        float uniformPadding = GetAbsoluteW(material, "_PoiConservativeBounds");
        ...
        padding.X += GetAbsoluteX(material, "_PoiVertexGlitch");
        padding += new Vector3(GetAbsoluteW(material, "_PoiUzumore"));
        padding += new Vector3(GetAbsoluteZ(material, "_PoiNaturalEquation"));
        padding += new Vector3(GetAbsoluteW(material, "_PoiDepthBulge"));

        Vector4 vertexColorPosition = GetVector4(material, "_PoiVertexColorPosition");
        ...
        bool rotates =
            ...
            GetAbsoluteFloat(material, "_PoiLookAtWeight") > 0.0001f;
        if (rotates)
        {
        ...
        return ExpandPoiyomiOutlineBounds(expanded, material);
    }

    private AABB ExpandPoiyomiWorldBounds(in AABB bounds, in Matrix4x4 localToWorld)
    {
        AABB expandedLocalBounds = ExpandPoiyomiLocalBounds(_bindPoseBounds, CurrentLODRenderer?.Material);
    ...
    private static AABB ExpandPoiyomiOutlineBounds(in AABB bounds, XRMaterial? material)
    {
