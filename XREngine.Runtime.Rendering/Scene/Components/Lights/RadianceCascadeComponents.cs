...
namespace XREngine.Components.Lights
{
    ...
    public class RadianceCascadeComponent : XRComponent
    {
        ...
        {
            private static readonly Dictionary<IRuntimeRenderWorld, List<RadianceCascadeComponent>> s_perWorld = [];
            private static readonly object s_lock = new();

            public static void Register(IRuntimeRenderWorld world, RadianceCascadeComponent component)
            {
                lock (s_lock)
                {
            ...
            public static void Unregister(IRuntimeRenderWorld world, RadianceCascadeComponent component)
            {
                lock (s_lock)
                {
            ...
            public static bool TryGetFirstActive(IRuntimeRenderWorld world, out RadianceCascadeComponent? component)
            {
                lock (s_lock)
                {
                    if (s_perWorld.TryGetValue(world, out List<RadianceCascadeComponent>? list))
