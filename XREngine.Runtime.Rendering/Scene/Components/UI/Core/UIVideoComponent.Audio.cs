...
namespace XREngine.Rendering.UI
{
    ...
    public partial class UIVideoComponent
    {
        ...
        private bool SubmitDecodedAudioFrame(DecodedAudioFrame frame, IAudioStreamingComponent? audioSource, out bool queueFull)
        {
            ...
            if (!_audioStartupDiagLogged)
            {
                ...
                Debug.UI($"[AV Audio Diag] First frame reached submission: " +
                          ...
                          $"listeners={audioSource.ActiveListenerCount} " +
            ...
            if (audioSource.ActiveListenerCount == 0)
            {
        ...
        {
            IAudioPlaybackSource? candidate = audioSource?.PrimaryPlaybackSource;
            ...
            if (_primaryAudioSource is not null)
                _primaryAudioSource.StreamingBufferProcessed -= OnPrimaryBufferProcessed;
        ...
        /// </summary>
        private void OnPrimaryBufferProcessed()
        {
        ...
        {
            audioSource.StopAndRewindAllSources();
#if false
        ...
        {
            return audioSource?.MinimumPlayableBufferCount ?? 0;
#if false
        ...
        private void SuppressAutoPlayOnAudioSources(IAudioStreamingComponent? audioSource)
        {
            ...
            if (nowTicks - _lastListenerDiagnosticTicks >= TimeSpan.TicksPerSecond * 5)
            {
                ...
                int count = audioSource.ActiveListenerCount;
                bool anyPlaying = audioSource.AnySourcePlaying;
                int buffersQueued = audioSource.PrimaryPlaybackSource?.BuffersQueued ?? 0;
                int buffersProcessed = audioSource.PrimaryPlaybackSource?.BuffersProcessed ?? 0;
#if false
            ...
            audioSource.SetAutoPlayOnQueue(false);
        }
