...
namespace XREngine.Rendering.UI
{
    ...
    public partial class UIVideoComponent
    {
                                         ...
                                         out int queuedAudioBuffers)
        {
            ...
            if (audioSource is not null)
            {
                bool anyRewound = audioSource.RewindStoppedEmptySources();
#if false
            ...
            bool hasListeners = audioSource is not null && audioSource.ActiveListenerCount > 0;
            bool anyPlaying = audioSource?.AnySourcePlaying ?? false;
            ...
            if (GetEstimatedQueuedAudioDurationTicks(audioSource) >= MinAudioQueuedDurationBeforePlayTicks &&
                ...
                audioSource is not null && audioSource.ActiveListenerCount > 0)
            {
                bool startedPlaying = audioSource.PlayStoppedSources();
#if false
                foreach (var source in audioSource.ActiveListeners.Values)
                {
                    if (!source.IsPlaying)
            ...
            {
                anyPlaying = audioSource.AnySourcePlaying;
#if false
                foreach (var source in audioSource.ActiveListeners.Values)
        ...
        private void DrainStreamingFramesOnMainThread(IMediaStreamSession session)
        {
            ...
            bool hasAudioPipeline = audioSource is not null && audioSource.ActiveListenerCount > 0;
