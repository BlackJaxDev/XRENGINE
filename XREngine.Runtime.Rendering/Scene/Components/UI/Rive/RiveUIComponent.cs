...
public class RiveUIComponent : UIInteractableComponent
{
    ...
    {
        _deltaLock.Enter();
        _renderDeltaAccumulator += RuntimeEngine.SmoothedDelta;
        _deltaLock.Exit();
    }
    ...
    private void RenderFBO()
    {
        ...
        RuntimeEngine.Rendering.State.ClearColor(ColorF4.Transparent);
        RuntimeEngine.Rendering.State.ClearByBoundFBO(true, true, true);
    ...
    {
        int sampleCount = capability.GetCurrentSurfaceSampleCount();
