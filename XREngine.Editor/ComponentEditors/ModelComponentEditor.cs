...
public sealed class ModelComponentEditor : IXRComponentEditor
{
    ...
    private sealed class MeshOptimizerEditorState
    {
    ...
    private static void DrawModelOverview(ModelComponent modelComponent)
    {
        ...
        ImGui.InputTextWithHint("##SubmeshFilter", "Filter by submesh, mesh, or material...", ref panelState.Filter, 128u);
        if (!string.IsNullOrWhiteSpace(panelState.Filter))
        {
            ImGui.SameLine();
            if (ImGui.SmallButton("Clear##SubmeshFilter"))
    ...
    private static bool SubmeshMatchesFilter(SubMesh subMesh, int index, string? filter, bool hideEmptySubmeshes = false)
    {
        ...
        if (index.ToString(CultureInfo.InvariantCulture).Contains(needle, StringComparison.OrdinalIgnoreCase))
            return true;

        if (!string.IsNullOrWhiteSpace(subMesh.Name) && subMesh.Name.Contains(needle, StringComparison.OrdinalIgnoreCase))
            return true;

        foreach (SubMeshLOD lod in subMesh.LODs)
        {
            if (!string.IsNullOrWhiteSpace(lod.Mesh?.Name) && lod.Mesh.Name.Contains(needle, StringComparison.OrdinalIgnoreCase))
                return true;

            if (!string.IsNullOrWhiteSpace(lod.Material?.Name) && lod.Material.Name.Contains(needle, StringComparison.OrdinalIgnoreCase))
                return true;
        }
    ...
    private static string BuildSubmeshHeaderLabel(int index, SubMesh subMesh, RenderableMesh? runtimeMesh)
    {
        ...
        string lodLabel = $"{subMesh.LODs.Count} LOD{(subMesh.LODs.Count == 1 ? string.Empty : "s")}";
        int activeLodIndex = GetActiveLodIndex(runtimeMesh);
        return activeLodIndex >= 0
            ? $"#{index} {submeshName} | {lodLabel} | Active LOD {activeLodIndex}##Submesh{index}"
            : $"#{index} {submeshName} | {lodLabel}##Submesh{index}";
    }

    private static int GetActiveLodIndex(RenderableMesh? runtimeMesh)
    {
        if (runtimeMesh?.CurrentLOD is null)
            return -1;

        int lodIndex = 0;
        for (var node = runtimeMesh.LODs.First; node is not null; node = node.Next, lodIndex++)
        {
            if (ReferenceEquals(node, runtimeMesh.CurrentLOD))
                return lodIndex;
        }
    ...
    private static void DrawSubmeshOverview(
        int submeshIndex,
        SubMesh subMesh,
    ...
    {
        const ImGuiTableFlags overviewTableFlags = ImGuiTableFlags.SizingStretchProp | ImGuiTableFlags.NoSavedSettings;
        if (!ImGui.BeginTable($"SubmeshOverview_{submeshIndex}", 2, overviewTableFlags))
            return;

        ImGui.TableSetupColumn("Label", ImGuiTableColumnFlags.WidthFixed, 110.0f);
        ...
        ImGui.TableSetColumnIndex(1);
        DrawSubmeshBoundsSummary(subMesh);

        ImGui.TableNextRow();
        ImGui.TableSetColumnIndex(0);
        ImGui.TextDisabled("Runtime");
        ImGui.TableSetColumnIndex(1);
        DrawSubmeshRuntimeSummary(runtimeMesh, lodEntries);

        ImGui.EndTable();
    }

    private static void DrawSubmeshBoundsSummary(SubMesh subMesh)
    {
        var bounds = subMesh.Bounds;
        Vector3 size = bounds.Max - bounds.Min;
        ImGui.TextUnformatted($"Size {size.X:F2} x {size.Y:F2} x {size.Z:F2}");
        if (ImGui.IsItemHovered())
        {
            string tooltip = $"Min: ({bounds.Min.X:F2}, {bounds.Min.Y:F2}, {bounds.Min.Z:F2})\n"
                + $"Max: ({bounds.Max.X:F2}, {bounds.Max.Y:F2}, {bounds.Max.Z:F2})";
            if (subMesh.CullingBounds is { } cullingBounds)
                tooltip += $"\nCustom Culling Size: {cullingBounds.Size.X:F2} x {cullingBounds.Size.Y:F2} x {cullingBounds.Size.Z:F2}";
            ImGui.SetTooltip(tooltip);
        }

        if (subMesh.CullingBounds is { } customBounds)
        {
            ImGui.SameLine();
            ImGui.TextDisabled($"Custom culling {customBounds.Size.X:F2} x {customBounds.Size.Y:F2} x {customBounds.Size.Z:F2}");
        }
    }

    private static void DrawSubmeshRuntimeSummary(
        RenderableMesh? runtimeMesh,
        IReadOnlyList<(int Index, SubMeshLOD Lod, LinkedListNode<RenderableMesh.RenderableLOD>? RuntimeNode)> lodEntries)
    {
        if (runtimeMesh is null)
        {
            ImGui.TextDisabled("Runtime mesh not initialised.");
            return;
        }

        int activeLodIndex = GetActiveLodIndex(runtimeMesh);
        string activeLodLabel = activeLodIndex >= 0 ? $"Active LOD {activeLodIndex}" : "No active LOD";
        ImGui.TextUnformatted($"{activeLodLabel}, {FormatRenderCommandLabel(runtimeMesh)}");

        var activeEntry = activeLodIndex >= 0 && activeLodIndex < lodEntries.Count ? lodEntries[activeLodIndex] : default;
        var renderer = activeEntry.RuntimeNode?.Value?.Renderer ?? runtimeMesh.CurrentLODRenderer;
        if (renderer is not null)
        {
            ImGui.SameLine();
            ImGui.TextDisabled($"Async {(renderer.GenerateAsync ? "On" : "Off")}");
        }
        ...
        SubmeshEditorState editorState)
    {
        ...
        foreach (var entry in lodEntries)
        {
            ...
            DrawMaterialSummaryCell(submeshIndex, entry.Index, lod.Material, runtimeLod?.Renderer?.Material);
    ...
    private static void DrawMaterialSummaryCell(int submeshIndex, int lodIndex, XRMaterialBase? assetMaterial, XRMaterialBase? runtimeMaterial)
    {
        ...
        if (assetMaterial is not null)
        {
            ...
            int textureCount = assetMaterial.Textures?.Count ?? 0;
            ImGui.TextUnformatted($"Asset: {FormatAssetLabel(assetMaterial.Name, assetMaterial)}");
            ImGui.TextDisabled($"{parameterCount} param(s), {textureCount} tex");
        }

        if (runtimeMaterial is not null && !ReferenceEquals(runtimeMaterial, assetMaterial))
        {
            int runtimeParameterCount = runtimeMaterial.Parameters?.Length ?? 0;
            int runtimeTextureCount = runtimeMaterial.Textures?.Count ?? 0;
            ImGui.TextDisabled($"Runtime: {FormatAssetLabel(runtimeMaterial.Name, runtimeMaterial)}");
            ImGui.TextDisabled($"{runtimeParameterCount} param(s), {runtimeTextureCount} tex");
        }

        DrawMaterialTexturePreviewButton(submeshIndex, lodIndex, assetMaterial, runtimeMaterial);
    }

    private static void DrawMaterialTexturePreviewButton(int submeshIndex, int lodIndex, XRMaterialBase? assetMaterial, XRMaterialBase? runtimeMaterial)
    {
        XRMaterialBase? effectiveMaterial = runtimeMaterial ?? assetMaterial;
        if (effectiveMaterial is null)
            return;
    ...
    private static void DrawStatusSummaryCell(int entryIndex, bool isActive, RenderableMesh? runtimeMesh, LinkedListNode<RenderableMesh.RenderableLOD>? runtimeNode)
    {
        if (runtimeMesh is null)
        {
        ...
        ImGui.TextUnformatted(isActive ? "Active" : "Available");
        ImGui.TextDisabled($"Async {(renderer.GenerateAsync ? "On" : "Off")}");
    ...
    private static void DrawSubmeshMaterialControls(SubMesh subMesh)
    {
        ...
        ImGui.SeparatorText("Material Linking");
    ...
    private static bool DrawMeshletGenerationSettings(MeshletGenerationSettings settings)
    {
        ...
        if (ImGui.Checkbox("Enable Meshlets", ref enabled))
        {
            settings.Enabled = enabled;
            changed = true;
        }

        MeshletBuildMode buildMode = settings.BuildMode;
        if (DrawEnumCombo("Build Mode", ref buildMode))
        {
            settings.BuildMode = buildMode;
            changed = true;
        }

        int maxVertices = (int)settings.MaxVertices;
        if (ImGui.InputInt("Max Vertices", ref maxVertices))
        {
            settings.MaxVertices = (uint)Math.Max(1, maxVertices);
            changed = true;
        }

        int minTriangles = (int)settings.MinTriangles;
        if (ImGui.InputInt("Min Triangles", ref minTriangles))
        {
            settings.MinTriangles = (uint)Math.Max(1, minTriangles);
            changed = true;
        }

        int maxTriangles = (int)settings.MaxTriangles;
        if (ImGui.InputInt("Max Triangles", ref maxTriangles))
        {
            settings.MaxTriangles = (uint)Math.Max(1, maxTriangles);
            changed = true;
        }

        float coneWeight = settings.ConeWeight;
        if (ImGui.DragFloat("Cone Weight", ref coneWeight, 0.01f, 0.0f, 1.0f, "%.2f"))
        {
            settings.ConeWeight = coneWeight;
            changed = true;
        }

        float splitFactor = settings.SplitFactor;
        if (ImGui.DragFloat("Split Factor", ref splitFactor, 0.05f, 0.0f, 32.0f, "%.2f"))
        {
            settings.SplitFactor = splitFactor;
            changed = true;
        }

        float fillWeight = settings.FillWeight;
        if (ImGui.DragFloat("Fill Weight", ref fillWeight, 0.05f, 0.0f, 8.0f, "%.2f"))
        {
            settings.FillWeight = fillWeight;
            changed = true;
        }

        bool optimizeMeshlets = settings.OptimizeMeshlets;
        if (ImGui.Checkbox("Optimize Meshlets", ref optimizeMeshlets))
        {
            settings.OptimizeMeshlets = optimizeMeshlets;
            changed = true;
        }

        int optimizeLevel = settings.OptimizeLevel;
        if (ImGui.SliderInt("Optimize Level", ref optimizeLevel, 0, 9))
        {
            settings.OptimizeLevel = optimizeLevel;
            changed = true;
        }

        bool computeBounds = settings.ComputeBounds;
        if (ImGui.Checkbox("Compute Bounds", ref computeBounds))
        {
            settings.ComputeBounds = computeBounds;
            changed = true;
        }

        bool encodeMeshlets = settings.EncodeMeshlets;
        if (ImGui.Checkbox("Encode Meshlets", ref encodeMeshlets))
        {
            settings.EncodeMeshlets = encodeMeshlets;
            changed = true;
        }

        using (new ImGuiDisabledScope(!settings.EncodeMeshlets))
        {
            bool encodeVertexReferences = settings.EncodeVertexReferences;
            if (ImGui.Checkbox("Encode Vertex References", ref encodeVertexReferences))
            {
                settings.EncodeVertexReferences = encodeVertexReferences;
                changed = true;
            }
        }
    ...
    private static bool DrawMeshLodGenerationSettings(MeshLodGenerationSettings settings)
    {
        bool changed = false;

        bool enabled = settings.Enabled;
        if (ImGui.Checkbox("Enable Auto LODs", ref enabled))
        {
            settings.Enabled = enabled;
            changed = true;
        }

        MeshOptimizerLodMode mode = settings.Mode;
        if (DrawEnumCombo("Simplify Mode", ref mode))
        {
            settings.Mode = mode;
            changed = true;
        }

        int additionalLodCount = settings.AdditionalLodCount;
        if (ImGui.InputInt("Additional LOD Count", ref additionalLodCount))
        {
            settings.AdditionalLodCount = additionalLodCount;
            changed = true;
        }

        float firstLodIndexRatio = settings.FirstLodIndexRatio;
        if (ImGui.SliderFloat("First LOD Index Ratio", ref firstLodIndexRatio, 0.0f, 1.0f, "%.3f"))
        {
            settings.FirstLodIndexRatio = firstLodIndexRatio;
            changed = true;
        }

        float lodRatioScale = settings.LodRatioScale;
        if (ImGui.SliderFloat("LOD Ratio Scale", ref lodRatioScale, 0.0f, 1.0f, "%.3f"))
        {
            settings.LodRatioScale = lodRatioScale;
            changed = true;
        }

        float targetError = settings.TargetError;
        if (ImGui.InputFloat("Target Error", ref targetError, 0.0f, 0.0f, "%.6f"))
        {
            settings.TargetError = targetError;
            changed = true;
        }

        float firstLodDistance = settings.FirstLodDistance;
        if (ImGui.InputFloat("First LOD Distance", ref firstLodDistance, 0.0f, 0.0f, "%.2f"))
        {
            settings.FirstLodDistance = firstLodDistance;
            changed = true;
        }

        float lodDistanceScale = settings.LodDistanceScale;
        if (ImGui.InputFloat("LOD Distance Scale", ref lodDistanceScale, 0.0f, 0.0f, "%.2f"))
        {
            settings.LodDistanceScale = lodDistanceScale;
            changed = true;
        }

        bool reusePrevious = settings.ReusePreviousLodAsSource;
        if (ImGui.Checkbox("Reuse Previous LOD As Source", ref reusePrevious))
        {
            settings.ReusePreviousLodAsSource = reusePrevious;
            changed = true;
        }

        MeshOptimizerSimplifyOptions options = settings.Options;
        if (DrawMeshOptimizerFlags(ref options))
        {
            settings.Options = options;
            changed = true;
        }

        bool useNormals = settings.UseNormals;
        float normalWeight = settings.NormalWeight;
        if (DrawWeightedAttributeToggle("Use Normals", ref useNormals, ref normalWeight))
        {
            settings.UseNormals = useNormals;
            settings.NormalWeight = normalWeight;
            changed = true;
        }

        bool useTangents = settings.UseTangents;
        float tangentWeight = settings.TangentWeight;
        if (DrawWeightedAttributeToggle("Use Tangents", ref useTangents, ref tangentWeight))
        {
            settings.UseTangents = useTangents;
            settings.TangentWeight = tangentWeight;
            changed = true;
        }

        bool useTexCoords = settings.UseTexCoords;
        float texCoordWeight = settings.TexCoordWeight;
        if (DrawWeightedAttributeToggle("Use TexCoords", ref useTexCoords, ref texCoordWeight))
        {
            settings.UseTexCoords = useTexCoords;
            settings.TexCoordWeight = texCoordWeight;
            changed = true;
        }

        bool useColors = settings.UseColors;
        float colorWeight = settings.ColorWeight;
        if (DrawWeightedAttributeToggle("Use Colors", ref useColors, ref colorWeight))
        {
            settings.UseColors = useColors;
            settings.ColorWeight = colorWeight;
            changed = true;
        }

        bool protectAttributeSeams = settings.ProtectAttributeSeams;
        if (ImGui.Checkbox("Protect Attribute Seams", ref protectAttributeSeams))
        {
            settings.ProtectAttributeSeams = protectAttributeSeams;
            changed = true;
        }

        bool prioritizeBorderVertices = settings.PrioritizeBorderVertices;
        if (ImGui.Checkbox("Prioritize Border Vertices", ref prioritizeBorderVertices))
        {
            settings.PrioritizeBorderVertices = prioritizeBorderVertices;
            changed = true;
        }

        bool lockWeightedVertices = settings.LockWeightedVertices;
        if (ImGui.Checkbox("Lock Weighted Vertices", ref lockWeightedVertices))
        {
            settings.LockWeightedVertices = lockWeightedVertices;
            changed = true;
        }
    ...
    private static bool DrawWeightedAttributeToggle(string label, ref bool enabled, ref float weight)
    {
        bool changed = false;
        ...
        {
            ImGui.SetNextItemWidth(MathF.Min(220.0f, ImGui.GetContentRegionAvail().X));
            if (ImGui.DragFloat($"{label} Weight", ref weight, 0.01f, 0.0f, 16.0f, "%.2f"))
                changed = true;
        }
    ...
    private static bool DrawMeshOptimizerFlags(ref MeshOptimizerSimplifyOptions options)
    {
        bool changed = false;

        foreach (MeshOptimizerSimplifyOptions value in Enum.GetValues<MeshOptimizerSimplifyOptions>())
        {
            if (value == MeshOptimizerSimplifyOptions.None)
                continue;

            bool enabled = options.HasFlag(value);
            if (ImGui.Checkbox(value.ToString(), ref enabled))
            {
                options = enabled ? options | value : options & ~value;
                changed = true;
            }
        }
    ...
    private static bool DrawEnumCombo<TEnum>(string label, ref TEnum value) where TEnum : struct, Enum
    {
        ...
        if (ImGui.BeginCombo(label, preview))
        {
            ...
            {
                bool selected = EqualityComparer<TEnum>.Default.Equals(option, value);
