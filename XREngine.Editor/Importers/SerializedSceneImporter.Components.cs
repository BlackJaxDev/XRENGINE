...
internal static partial class SerializedSceneImporter
{
    ...
    private static void AttachSupportedComponents(ParsedSourceFile parsed, ImportedHierarchy hierarchy, ImportState state)
    {
        ...
        ImportState state)
    {
        ...
        foreach (AddedComponentDelta addedComponent in prefabInstance.AddedComponents)
        {
            if (!TryResolveTargetNode(prefabHierarchy, addedComponent.TargetCorrespondingSourceObject.FileId, out SceneNode? resolvedTargetNode) ||
                resolvedTargetNode is not SceneNode targetNode ||
                !ownerFile.ComponentsByFileId.TryGetValue(addedComponent.AddedObject.FileId, out ParsedSourceComponent? componentDefinition))
            {
                continue;
            }
        ...
        ImportState state)
    {
        ...
        XRMaterial? material = ResolveSerializedMaterial(materialReference, state);
        if (material is null)
        {
    ...
    private static void ApplyAuthoredBounds(
        ...
        Vector3? sourceExtent,
        ImportState state)
    {
        if (component.Model is null)
            return;

        if (sourceCenter is Vector3 center &&
            sourceExtent is Vector3 extent &&
            IsFinite(center) &&
            IsFinite(extent) &&
            extent.X > 0.0f &&
            extent.Y > 0.0f &&
    ...
    private static XRComponent? AttachSpecificComponent(
        ...
        ParsedSourceComponent componentDefinition,
        ImportState state)
    ...
    private static Model? ResolveModelFromMeshReference(
        ...
        IReadOnlyList<SourceReference> materialReferences,
        ParsedSkinnedMeshRenderer? skinnedMeshRenderer,
        ImportState state)
    {
        ...
        if (string.Equals(Path.GetExtension(assetPath), ".asset", StringComparison.OrdinalIgnoreCase))
            return LoadSerializedSourceMeshModel(assetPath, node, hierarchy, materialReferences, skinnedMeshRenderer, state);
        ...
        out Model? model)
    {
        ...
        LogSourceToonMeshDiagnostics(material, mesh);
    ...
    private static Model? LoadSerializedSourceMeshModel(
        string assetPath,
        SceneNode node,
        ImportedHierarchy hierarchy,
        IReadOnlyList<SourceReference> materialReferences,
        ...
        ImportState state)
    {
        if (LoadSourceDocumentMapping(assetPath, "Mesh") is not YamlMappingNode meshMapping)
            return null;

        string meshName = GetScalarString(meshMapping, "m_Name") ?? Path.GetFileNameWithoutExtension(assetPath);
        List<Vertex> vertices = ParseSerializedSourceMeshVertices(meshMapping);
        List<SerializedSourceSubMesh> subMeshes = ParseSerializedSourceSubMeshes(meshMapping);
        ...
        {
            SerializedSourceSubMesh sourceSubMesh = subMeshes[subMeshIndex];
            List<ushort>? triangleIndices = ExtractTriangleIndices(indexBuffer, sourceSubMesh.FirstByte, sourceSubMesh.IndexCount, sourceSubMesh.BaseVertex, use32BitIndices);
            ...
            LogSourceToonMeshDiagnostics(material, xrMesh);
            var lod = new SubMeshLOD(material, xrMesh, 0.0f);
            var subMesh = new SubMesh(lod)
            {
                Name = sourceSubMesh.Name ?? $"{meshName}_{subMeshIndex}",
        ...
        ImportState state)
    {
        ...
        for (int subMeshIndex = 0; subMeshIndex < sourceModel.Meshes.Count; subMeshIndex++)
        {
            ...
            foreach (SubMeshLOD sourceLod in sourceSubMesh.LODs)
            {
                ...
                LogSourceToonMeshDiagnostics(material, clonedMesh);
                var clonedLod = new SubMeshLOD(material, clonedMesh, sourceLod.MaxVisibleDistance)
                {
                    ...
                    MinProjectedScreenRadiusPixels = sourceLod.MinProjectedScreenRadiusPixels,
        ...
        XRMaterial? fallbackMaterial)
    {
        ...
        {
            SourceReference reference = materialReferences[Math.Min(subMeshIndex, materialReferences.Count - 1)];
            XRMaterial? resolved = ResolveSerializedMaterial(reference, state);
            if (resolved is not null)
                return resolved;
        }

        return fallbackMaterial ?? CreateFallbackSerializedMaterial("UnityImporterDefaultMaterial", ColorF4.White);
    }

    private static XRMaterial? ResolveSerializedMaterial(SourceReference materialReference, ImportState state)
    {
        ...
        if (string.Equals(Path.GetExtension(assetPath), ".mat", StringComparison.OrdinalIgnoreCase))
            return LoadSerializedMaterial(assetPath, state);

        AssetManager? assets = Engine.Assets;
        return assets?.Load<XRMaterial>(assetPath);
    }

    private static void LogSourceToonMeshDiagnostics(XRMaterial material, XRMesh? mesh)
    {
        if (mesh is null)
            return;

        foreach (MaterialConversionDiagnostic diagnostic in
                 SerializedMaterialImporter.ValidateMeshCompatibility(material, mesh.TexCoordCount))
        {
            Debug.LogWarning(diagnostic.ToString());
        }
    }

    private static XRMaterial? LoadSerializedMaterial(string materialPath, ImportState state)
    {
        try
        {
            return state.Context.GetOrAddCached(
                ...
                () =>
                {
                    SerializedMaterialImportResult result = SerializedMaterialImporter.ImportWithReport(materialPath, state.Context);
    ...
    private static XRMaterial CreateFallbackSerializedMaterial(string materialName, ColorF4 baseColor)
        => new()
        {
            Name = materialName,
    ...
    private static XRTexture2D? ResolveMaterialTexture(YamlMappingNode materialMapping, ImportState state)
    {
        ...
        foreach (string propertyName in new[] { "_BaseMap", "_MainTex" })
        {
            foreach (YamlNode item in texEnvsNode.Children)
            {
                ...
                SourceReference textureReference = ParseReference(GetNode(texEnv, "m_Texture"));
    ...
    private static YamlMappingNode? LoadSourceDocumentMapping(string assetPath, string documentType)
    {
    ...
    private static List<Vertex> ParseSerializedSourceMeshVertices(YamlMappingNode meshMapping)
    {
        ...
        SourceVertexChannel[] channels = channelsNode.Children
            .OfType<YamlMappingNode>()
            .Select(ParseSourceVertexChannel)
            .ToArray();
        ...
        for (int vertexIndex = 0; vertexIndex < vertexCount; vertexIndex++)
        {
            ...
            {
                SourceVertexChannel channel = channels[channelIndex];
    ...
    private static List<SerializedSourceSubMesh> ParseSerializedSourceSubMeshes(YamlMappingNode meshMapping)
    {
        if (GetNode(meshMapping, "m_SubMeshes") is not YamlSequenceNode subMeshesNode)
            return [];

        var subMeshes = new List<SerializedSourceSubMesh>(subMeshesNode.Children.Count);
        int index = 0;
        foreach (YamlNode child in subMeshesNode.Children)
        {
            ...
            subMeshes.Add(new SerializedSourceSubMesh(
    ...
    private static SourceVertexChannel ParseSourceVertexChannel(YamlMappingNode channelMapping)
        => new(
    ...
    private static Dictionary<int, int> ComputeStreamStrides(IEnumerable<SourceVertexChannel> channels)
    {
        var streamStrides = new Dictionary<int, int>();
        foreach (SourceVertexChannel channel in channels)
        {
    ...
    private static Vector2 ReadVector2(byte[] rawVertexData, int offset, SourceVertexChannel channel)
        => new(
            ReadVertexChannelComponent(rawVertexData, offset, channel, 0),
            ReadVertexChannelComponent(rawVertexData, offset, channel, 1));

    private static Vector3 ReadVector3(byte[] rawVertexData, int offset, SourceVertexChannel channel)
        => new(
            ReadVertexChannelComponent(rawVertexData, offset, channel, 0),
            ReadVertexChannelComponent(rawVertexData, offset, channel, 1),
            ReadVertexChannelComponent(rawVertexData, offset, channel, 2));

    private static Vector4 ReadVector4(byte[] rawVertexData, int offset, SourceVertexChannel channel)
        => new(
            ReadVertexChannelComponent(rawVertexData, offset, channel, 0),
            ReadVertexChannelComponent(rawVertexData, offset, channel, 1),
            ReadVertexChannelComponent(rawVertexData, offset, channel, 2),
    ...
    private static float ReadVertexChannelComponent(byte[] rawVertexData, int offset, SourceVertexChannel channel, int componentIndex)
    {
    ...
    private static Vector3 ConvertDirection(Vector3 sourceDirection)
        => new(sourceDirection.X, sourceDirection.Y, -sourceDirection.Z);
    ...
    private sealed record SourceVertexChannel(int Stream, int Offset, int Format, int Dimension);

    private sealed record SerializedSourceSubMesh(int FirstByte, int IndexCount, int BaseVertex, string? Name);
}
