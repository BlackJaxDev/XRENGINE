...
/// </summary>
public static class SerializedModelImporterDocumentParser
{
    public static SerializedModelImporterDocument ParseForModel(string modelPath)
        => ParseFile(modelPath + ".meta");

    public static SerializedModelImporterDocument ParseFile(string metaPath)
    {
        ...
        return new SerializedModelImporterDocument
        {
    ...
    private static IReadOnlyList<SerializedModelSkeletonTransform> ParseSkeletonTransforms(YamlNode? node)
    {
        ...
        var transforms = new List<SerializedModelSkeletonTransform>(sequence.Children.Count);
        foreach (YamlMappingNode entry in sequence.Children.OfType<YamlMappingNode>())
        {
            ...
            transforms.Add(new SerializedModelSkeletonTransform
            {
                Name = name,
    ...
    private static IReadOnlyList<SourceExternalMaterialRemap> ParseExternalObjects(YamlNode? node)
    {
        if (node is not YamlSequenceNode sequence)
            return [];

        var remaps = new List<SourceExternalMaterialRemap>(sequence.Children.Count);
        foreach (YamlMappingNode entry in sequence.Children.OfType<YamlMappingNode>())
        {
            ...
            SourceAssetReference reference = ParseReference(target);
            if (string.IsNullOrWhiteSpace(name) || !reference.HasExternalGuid)
                continue;

            remaps.Add(new SourceExternalMaterialRemap
            {
    ...
    private static SourceAssetReference ParseReference(YamlMappingNode mapping)
    {
        ...
        int? type = int.TryParse(GetString(mapping, "type"), NumberStyles.Integer, CultureInfo.InvariantCulture, out int parsedType)
            ? parsedType
            : null;
        return new SourceAssetReference(fileId, GetString(mapping, "guid"), type);
    }
