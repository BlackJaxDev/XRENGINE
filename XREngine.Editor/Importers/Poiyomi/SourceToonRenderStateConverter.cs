...
/// </summary>
public static class SourceToonRenderStateConverter
{
    ...
    public static SourceToonRenderStateConversion Convert(
        ...
        ICollection<MaterialConversionDiagnostic> diagnostics)
    {
        ...
        SourceToonRenderPreset preset = ResolvePreset(document, diagnostics);
        ...
        return new SourceToonRenderStateConversion
        {
            Preset = preset,
    ...
    private static StencilTestFace CreateStencilFace(
        ...
        => new()
        {
            ...
            BothPassOp = MapStencilOperation(document, prefix + "PassOp", ToSourceStencilOperation(commonPass), diagnostics),
            BothFailOp = MapStencilOperation(document, prefix + "FailOp", ToSourceStencilOperation(commonFail), diagnostics),
            StencilPassDepthFailOp = MapStencilOperation(document, prefix + "ZFailOp", ToSourceStencilOperation(commonDepthFail), diagnostics),
        };

    private static SourceToonRenderPreset ResolvePreset(
        SerializedMaterialDocument document,
        ICollection<MaterialConversionDiagnostic> diagnostics)
    {
        int fallback = document.CustomRenderQueue >= TransparentQueue ? 3 : 0;
        int raw = GetInt(document, "_Mode", fallback);
        if (Enum.IsDefined((SourceToonRenderPreset)raw))
            return (SourceToonRenderPreset)raw;

        ReportOutOfRange("_Mode", raw, SourceToonRenderPreset.Opaque, diagnostics);
        return SourceToonRenderPreset.Opaque;
    }
    ...
    private static int ToSourceStencilOperation(EStencilOp operation)
        => operation switch
        {
            EStencilOp.Keep => 0,
            EStencilOp.Zero => 1,
    ...
    private static ETransparencyMode ResolveTransparencyMode(SourceToonRenderPreset preset, bool alphaToCoverage)
    {
        if (alphaToCoverage)
        ...
        {
            SourceToonRenderPreset.Opaque => ETransparencyMode.Opaque,
            SourceToonRenderPreset.Cutout or SourceToonRenderPreset.TransClipping => ETransparencyMode.Masked,
            SourceToonRenderPreset.Transparent => ETransparencyMode.PremultipliedAlpha,
            SourceToonRenderPreset.Additive => ETransparencyMode.Additive,
            _ => ETransparencyMode.AlphaBlend,
        };
    }

    private static int ResolveRenderPass(SourceToonRenderPreset preset, bool alphaToCoverage)
    {
        if (alphaToCoverage || preset is SourceToonRenderPreset.Cutout or SourceToonRenderPreset.TransClipping)
            return (int)EDefaultRenderPass.MaskedForward;
        if (preset == SourceToonRenderPreset.Opaque)
            return (int)EDefaultRenderPass.OpaqueForward;
        return (int)EDefaultRenderPass.TransparentForward;
    }

    private static bool RequiresBlending(SourceToonRenderPreset preset)
        => preset is not SourceToonRenderPreset.Opaque and not SourceToonRenderPreset.Cutout;

    private static bool PresetWritesDepth(SourceToonRenderPreset preset)
        => preset is SourceToonRenderPreset.Opaque or SourceToonRenderPreset.Cutout or SourceToonRenderPreset.TransClipping;

    private static int GetPresetQueue(SourceToonRenderPreset preset)
        => preset switch
        {
            SourceToonRenderPreset.Opaque => OpaqueQueue,
            SourceToonRenderPreset.Cutout => CutoutQueue,
            SourceToonRenderPreset.TransClipping => TransClippingQueue,
            _ => TransparentQueue,
        };

    private static float GetPresetCutoff(SourceToonRenderPreset preset)
        => preset switch
        {
    ...
    private static (int SrcRgb, int DstRgb, int SrcAlpha, int DstAlpha) GetPresetBlend(SourceToonRenderPreset preset)
        => preset switch
        {
            SourceToonRenderPreset.TransClipping or SourceToonRenderPreset.Fade => (5, 10, 1, 1),
            SourceToonRenderPreset.Transparent => (1, 10, 1, 1),
            SourceToonRenderPreset.Additive => (1, 1, 1, 1),
            SourceToonRenderPreset.SoftAdditive => (4, 1, 1, 1),
            SourceToonRenderPreset.Multiplicative => (2, 0, 1, 1),
            SourceToonRenderPreset.Multiplicative2X => (2, 3, 1, 1),
            _ => (1, 0, 1, 1),
        };

    private static bool IsSourcePassEnabled(SerializedMaterialDocument document, string passName)
        => !document.DisabledShaderPasses.Contains(passName);

    private static bool GetBool(SerializedMaterialDocument document, string name, bool fallback)
        => document.TryGetFloat(name, out float value) ? value > 0.5f : fallback;

    private static int GetInt(SerializedMaterialDocument document, string name, int fallback)
        => document.TryGetInt(name, out int value) ? value : fallback;

    private static float GetFloat(SerializedMaterialDocument document, string name, float fallback)
        => document.TryGetFloat(name, out float value) ? value : fallback;

    private static ulong ComputePositionOpacityStateHash(SerializedMaterialDocument document, float cutoff)
    {
