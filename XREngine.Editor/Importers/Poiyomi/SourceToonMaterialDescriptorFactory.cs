...
public static class SourceToonMaterialDescriptorFactory
{
        ...
        ICollection<MaterialConversionDiagnostic>? diagnostics = null)
    {
        ...
        Dictionary<string, SourceToonPropertyBinding> bindings = new(StringComparer.Ordinal);
        Dictionary<string, SourceToonTextureDescriptor> textures = new(StringComparer.Ordinal);
        ...
        foreach ((string sourceName, SerializedTexturePropertyDocument texture) in document.Textures)
        {
            SourceToonPropertyBinding binding = ResolveBinding(sourceName, document, renameSuffix);
            bindings[sourceName] = binding;

            SourceResolvedAsset resolved = resolver.Resolve(texture.TextureReference);
            SerializedTextureImportDocument? importSettings = resolved.AssetPath is null
                ? null
                : SerializedTextureImportDocumentParser.ParseFile(resolved.AssetPath);
            var descriptor = new SourceToonTextureDescriptor
            {
        ...
        return new SourceToonMaterialDescriptor
        {
            Name = document.Name,
            Version = match.IsDowngradeSource
                ? SourceToon93Catalog.Version
                : match.Version ?? SourceToon93Catalog.Version,
    ...
    private static void Normalize<T>(
        ...
        Dictionary<string, SourceToonPropertyBinding> bindings,
        SerializedMaterialDocument document,
        string renameSuffix)
    {
    ...
    private static SourceToonPropertyBinding ResolveBinding(
        string sourceName,
        SerializedMaterialDocument document,
        string renameSuffix)
    {
        ...
        return new SourceToonPropertyBinding
        {
    ...
    private static string ResolveRenameSuffix(SerializedMaterialDocument document)
    {
