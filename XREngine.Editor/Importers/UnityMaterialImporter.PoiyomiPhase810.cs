...
{
    private static void ApplyPoiyomiPhase810(
        XRMaterial material,
        ...
        List<string> warnings)
    {
        ...
        bool specialEffects = HasPhase8Effects(document);
        bool vertexEffects = HasPhase9VertexEffects(document);
        ...
        MapPhase8OutlineAndDissolve(material, document);
        MapPhase8SpecialEffects(material, document);
        MapPhase9VertexEffects(material, document);
        BindPhase810Textures(material, document, resolver, diagnostics, warnings);
    ...
    private static bool HasPhase8Effects(UnityMaterialDocument document)
    ...
    private static bool HasPhase9VertexEffects(UnityMaterialDocument document)
        => HasAnyPositive(
            document,
    ...
    private static void MapPhase8OutlineAndDissolve(XRMaterial material, UnityMaterialDocument document)
    {
    ...
    private static void MapPhase8SpecialEffects(XRMaterial material, UnityMaterialDocument document)
    {
    ...
    private static void MapPhase9VertexEffects(XRMaterial material, UnityMaterialDocument document)
    {
        if (!HasPhase9VertexEffects(document))
            return;
    ...
    private static void BindPhase810Textures(
        XRMaterial material,
        UnityMaterialDocument document,
        UnityAssetResolver resolver,
        ICollection<MaterialConversionDiagnostic> diagnostics,
