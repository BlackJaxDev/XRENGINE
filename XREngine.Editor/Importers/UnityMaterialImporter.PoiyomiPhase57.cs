...
{
    private static void ApplyPoiyomiPhase57(
        XRMaterial material,
        ...
        List<string> warnings)
    {
        ...
        material.SetUberFeatureEnabled("poiyomi-masks-themes", HasPhase57MaskOrTheme(document));
        material.SetUberFeatureEnabled("poiyomi-lighting-parity", true);
        material.SetUberFeatureEnabled("poiyomi-pbr-parity", HasPhase57Pbr(document));
        material.SetUberFeatureEnabled("poiyomi-matcap-rim-slots", HasPhase57MatcapOrRim(document));
        ...
        BindExactPhase57Parameters(material, document);
        BindPhase57CompositeParameters(material, document);
        BindPhase57Textures(material, document, resolver, diagnostics, warnings);
        WarnForDeferredPhase57Adapters(document, warnings);
    }

    private static bool HasPhase57MaskOrTheme(UnityMaterialDocument document)
    ...
    private static bool HasPhase57Pbr(UnityMaterialDocument document)
        => HasAnyPositive(
            document,
            "_MochieBRDF",
            "_AnisotropyEnabled",
    ...
    private static bool HasPhase57MatcapOrRim(UnityMaterialDocument document)
        => HasAnyPositive(
            document,
            "_MatcapEnable",
            "_Matcap2Enable",
    ...
    private static void BindExactPhase57Parameters(XRMaterial material, UnityMaterialDocument document)
    {
    ...
    private static void BindPhase57CompositeParameters(XRMaterial material, UnityMaterialDocument document)
    {
    ...
    private static void BindPhase57Textures(
        XRMaterial material,
        UnityMaterialDocument document,
        UnityAssetResolver resolver,
        ICollection<MaterialConversionDiagnostic> diagnostics,
    ...
    private static void WarnForDeferredPhase57Adapters(UnityMaterialDocument document, List<string> warnings)
    {
