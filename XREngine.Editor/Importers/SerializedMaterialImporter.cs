...
public sealed class SerializedMaterialImportResult
{
    public XRMaterial? Material { get; init; }
    public bool IsSourceToon { get; init; }
    public bool IsSourceToonFeatureLoss { get; init; }
    public bool IsLilToon { get; init; }
    public string? ShaderPath { get; init; }
    public SerializedMaterialDocument? SourceDocument { get; init; }
    public SourceResolvedAsset? ShaderAsset { get; init; }
    public SourceToonMaterialDescriptor? SourceToonDescriptor { get; init; }
    public SourceToonShaderMatchResult? SourceToonShaderMatch { get; init; }
    public string[] Warnings { get; init; } = [];
    public MaterialConversionDiagnostic[] Diagnostics { get; init; } = [];
    public MaterialConversionReport? ConversionReport { get; init; }
}

public static partial class SerializedMaterialImporter
{
    private static readonly ConcurrentDictionary<string, XRTexture2D> DefaultUberSamplerTextures = new(StringComparer.Ordinal);
    private static readonly ConditionalWeakTable<XRMaterial, SerializedMaterialDocument> SourceToonDocuments = new();
    ...
    public static MaterialConversionDiagnostic[] ValidateMeshCompatibility(XRMaterial material, uint availableUvChannels)
        => SourceToonDocuments.TryGetValue(material, out SerializedMaterialDocument? document)
            ? SourceToonUvUsage.Validate(document, availableUvChannels)
            : [];


    public static SerializedMaterialImportResult ImportWithReport(string materialPath, string? projectRoot = null)
    {
        ...
        string resolvedProjectRoot = projectRoot ??
            SourceProjectLocator.Locate(normalizedPath).ProjectRoot;
        return ImportWithReportCore(normalizedPath, new SourceAssetResolver(resolvedProjectRoot), context: null);
    }

    /// <summary>
    /// Imports a Unity material through the resolver and caches owned by an active project import.
    /// </summary>
    public static SerializedMaterialImportResult ImportWithReport(
        string materialPath,
    ...
    private static SerializedMaterialImportResult ImportWithReportCore(
        ...
        SourceProjectImportContext? context)
    {
        ...
        SerializedMaterialDocument document;
        try
        {
            document = SerializedMaterialDocumentParser.ParseFile(normalizedPath);
        }
        catch (Exception ex) when (ex is IOException or UnauthorizedAccessException or InvalidDataException)
        {
            warnings.Add($"Could not parse Unity material '{normalizedPath}'. {ex.Message}");
            return new SerializedMaterialImportResult
            {
                Warnings = [.. warnings],
        ...
        SourceResolvedAsset shaderAsset = resolver.Resolve(document.Shader);
        SourceToonShaderMatchResult sourceToonMatch = MatchSourceToon93Material(document, resolver, out string? shaderPath);
        diagnostics.AddRange(sourceToonMatch.Diagnostics);
        SerializedMaterialDocument conversionDocument = sourceToonMatch.IsDowngradeSource
            ? SourceToonFeatureLossNormalizer.Normalize(document, diagnostics)
            : document;
        SourceToonMaterialDescriptor? sourceToonDescriptor = sourceToonMatch.IsAccepted || sourceToonMatch.IsDowngradeSource
            ? SourceToonMaterialDescriptorFactory.Create(conversionDocument, resolver, sourceToonMatch, diagnostics)
            : null;
        bool isSourceToonConversion = sourceToonMatch.IsAccepted || sourceToonMatch.IsDowngradeSource;
        foreach (MaterialConversionDiagnostic diagnostic in diagnostics)
            warnings.Add(diagnostic.ToString());
        bool isSourceToon = sourceToonMatch.IsAccepted;
        bool isSourceToonFeatureLoss = sourceToonMatch.IsDowngradeSource;
        bool isLilToon = false;
        if (!isSourceToonConversion && !sourceToonMatch.IsSourceToonFamily)
            isLilToon = IsLilToonMaterial(document, resolver, out shaderPath);

        try
        {
            XRMaterial material = isSourceToonConversion
                ? ConvertSourceToonToUberMaterial(
                    conversionDocument,
                    resolver,
                    warnings,
                    diagnostics,
                    isSourceToonFeatureLoss)
                : isLilToon
                    ? ConvertLilToonToUberMaterial(document, resolver, warnings, shaderPath)
                    : ConvertGenericSerializedMaterial(document, resolver, warnings);

            material.OriginalPath = normalizedPath;
            EMaterialConversionOutcome outcome = isSourceToonFeatureLoss
                ? EMaterialConversionOutcome.ConvertedToSourceToon
                : EMaterialConversionOutcome.Converted;
            MaterialConversionReport conversionReport = MaterialConversionReportBuilder.Create(
                normalizedPath,
                shaderPath,
                material,
                sourceToonDescriptor,
                warnings,
                diagnostics,
                outcome,
                sourceToonMatch);
            ...
            {
                Material = material,
                IsSourceToon = isSourceToon,
                IsSourceToonFeatureLoss = isSourceToonFeatureLoss,
                IsLilToon = isLilToon,
                ShaderPath = shaderPath,
                SourceDocument = document,
                ShaderAsset = shaderAsset,
                SourceToonDescriptor = sourceToonDescriptor,
                SourceToonShaderMatch = sourceToonMatch,
                Warnings = [.. warnings],
                Diagnostics = [.. diagnostics],
                ConversionReport = conversionReport,
            };
        }
        catch (Exception ex) when (isSourceToonConversion || isLilToon)
        {
            string shaderFamily = isLilToon
                ? "lilToon"
                : isSourceToonFeatureLoss ? "Poiyomi Pro downgrade" : "Poiyomi Toon";
            ...
            try
            {
                XRMaterial material = ConvertGenericSerializedMaterial(document, resolver, warnings);
                material.OriginalPath = normalizedPath;
                MaterialConversionReport conversionReport = MaterialConversionReportBuilder.Create(
                    normalizedPath,
                    shaderPath,
                    ...
                    sourceToonMatch);
                MaterialConversionReportRegistry.Instance.Set(material, conversionReport);
                context?.MarkOutcome(normalizedPath, SourceImportConversionOutcome.Failed);
                CopyDiagnosticsToContext(context, normalizedPath, diagnostics);
                return new SerializedMaterialImportResult
                {
                    Material = material,
        ...
        IEnumerable<MaterialConversionDiagnostic> diagnostics)
    {
        ...
        foreach (MaterialConversionDiagnostic diagnostic in diagnostics)
        {
            context.AddDiagnostic(
                ...
                {
                    MaterialConversionDiagnosticSeverity.Info => SourceImportDiagnosticSeverity.Info,
                    MaterialConversionDiagnosticSeverity.Warning => SourceImportDiagnosticSeverity.Warning,
                    _ => SourceImportDiagnosticSeverity.Error,
                },
                diagnostic.Code.StartsWith("POIPRO", StringComparison.Ordinal)
                    ? SourceImportDiagnosticCategory.MaterialDowngrade
                    : SourceImportDiagnosticCategory.TextureImport,
                diagnostic.Message,
                materialPath,
    ...
    private static XRMaterial ConvertSourceToonToUberMaterial(
        SerializedMaterialDocument document,
        SourceAssetResolver resolver,
        List<string> warnings,
        List<MaterialConversionDiagnostic> diagnostics,
        bool isSourceToonFeatureLoss)
    {
        ...
        XRMaterial material = new()
        {
            Name = document.Name,
            Textures = [.. textures],
            Parameters = ModelAssetImporter.CreateDefaultForwardPlusUberShaderParameters(bumpScale),
        ...
        ApplySourceToonTextureTransform(material, document, toonRampSource, "_ToonRamp", "_ToonRampUVSelector", diagnostics);
        ApplySourceToonTextureTransform(material, document, firstShadeSource, "_FirstShadeMap", null, diagnostics);
        ApplySourceToonTextureTransform(material, document, secondShadeSource, "_SecondShadeMap", null, diagnostics);
        ...
        ApplySourceToonTextureTransform(material, document, rimMaskSource, "_RimMask", null, diagnostics);
        ApplySourceToonTextureTransform(material, document, rimColorSource, "_RimColorTexture", null, diagnostics);
        ApplyTextureTransform(material, document, specularSource, "_SpecularMap");
        ApplySourceToonTextureTransform(material, document, metallicSource, "_PBRMetallicMaps", null, diagnostics);
        ApplySourceToonTextureTransform(material, document, smoothnessSource, "_PBRSmoothnessMaps", null, diagnostics);
        ...
        ApplySourceToonTextureTransform(material, document, dissolveNoiseSource, "_DissolveNoiseTexture", null, diagnostics);
        ApplySourceToonTextureTransform(material, document, dissolveDetailSource, "_DissolveDetailNoise", null, diagnostics);
        ApplySourceToonTextureTransform(material, document, dissolveMaskSource, "_DissolveMask", null, diagnostics);
        ApplyTextureTransform(material, document, dissolveGradientSource, "_DissolveEdgeGradient");
        ApplySourceToonTextureTransform(material, document, dissolveEdgeSource, "_DissolveEdgeTexture", null, diagnostics);
        ApplyTextureTransform(material, document, parallaxSource, "_ParallaxMap");

        ApplySourceToonScalarAndColorProperties(material, document, diagnostics);
        ApplySourceToonPackedPbrProperties(material, document, metallicSource, smoothnessSource, diagnostics);
        ApplySourceToonFeatureState(material, document,
            alphaSource,
            colorAdjustSource,
            shadowColorSource,
            toonRampSource,
        ...
        ApplySourceToonSurfaceFeatures(material, document, resolver, diagnostics, warnings);
        ApplySourceToonEffectsAndIntegrations(material, document, resolver, diagnostics, warnings);
        ApplySourceToonRenderState(material, document, diagnostics);
        ...
        if (!isSourceToonFeatureLoss)
            material.UseRuntimeUberPropertyBindings();

        SourceToonDocuments.Remove(material);
        SourceToonDocuments.Add(material, document);
        return material;
    }
    ...
    private static XRMaterial ConvertGenericSerializedMaterial(
        SerializedMaterialDocument document,
        SourceAssetResolver resolver,
        List<string> warnings)
    {
    ...
    private static void ApplySourceToonScalarAndColorProperties(
        XRMaterial material,
        SerializedMaterialDocument document,
        ICollection<MaterialConversionDiagnostic> diagnostics)
    {
        MapVector4(document, material, "_Color", "_Color");
        SetInt(material, "_ColorThemeIndex", SourceToonEnumMapper.Identity(document, "_ColorThemeIndex", 0, 0, 12, diagnostics));
        ...
        SetInt(material, "_MainAlphaMaskMode", SourceToonEnumMapper.AlphaMaskMode(document, diagnostics));
        ...
        SetInt(material, "_MainHueShiftColorSpace", SourceToonEnumMapper.Identity(document, "_MainHueShiftColorSpace", 0, 0, 1, diagnostics));
        MapFloat(document, material, "_MainHueShiftReplace", "_MainHueShiftReplace");

        SetInt(material, "_LightingMode", SourceToonEnumMapper.LightingMode(document, diagnostics));
        SetInt(material, "_LightingColorMode", SourceToonEnumMapper.Identity(document, "_LightingColorMode", 0, 0, 3, diagnostics));
        SetInt(material, "_LightingMapMode", SourceToonEnumMapper.Identity(document, "_LightingMapMode", 0, 0, 3, diagnostics));
        SetInt(material, "_LightingDirectionMode", SourceToonEnumMapper.Identity(document, "_LightingDirectionMode", 0, 0, 3, diagnostics));
        ...
        SetInt(material, "_EmissionColorThemeIndex", SourceToonEnumMapper.Identity(document, "_EmissionColorThemeIndex", 0, 0, 12, diagnostics));
        ...
        SetInt(material, "_MatcapUVMode", SourceToonEnumMapper.MatcapUvMode(document, diagnostics));
        ...
        SetInt(material, "_MatcapMaskChannel", SourceToonEnumMapper.TextureChannel(document, "_MatcapMaskChannel", 0, diagnostics));
        ...
        SetInt(material, "_RimStyle", SourceToonEnumMapper.RimStyle(document, diagnostics));
        MapFloat(document, material, "_RimBlendStrength", "_RimBlendStrength", "_RimMainStrength");
        SetInt(material, "_RimBlendMode", SourceToonEnumMapper.RimBlendMode(document, diagnostics));
        SetInt(material, "_RimMaskChannel", SourceToonEnumMapper.TextureChannel(document, "_RimMaskChannel", 0, diagnostics));
        ...
        SetInt(material, "_SpecularType", SourceToonEnumMapper.SpecularType(document, diagnostics));
        ...
        SetFloat(material, "_DissolveType", SourceToonEnumMapper.DissolveMode(document, diagnostics));
        ...
        SetFloat(material, "_ParallaxMode", SourceToonEnumMapper.ParallaxMode(document, diagnostics));
        ...
        SetFloat(material, "_ParallaxMapChannel", SourceToonEnumMapper.TextureChannel(
            document,
    ...
    private static void ApplySourceToonPackedPbrProperties(
        XRMaterial material,
        SerializedMaterialDocument document,
        ...
        ICollection<MaterialConversionDiagnostic> diagnostics)
    {
        ...
        {
            metallicChannel = SourceToonEnumMapper.TextureChannel(
                document,
        ...
        {
            smoothnessChannel = SourceToonEnumMapper.TextureChannel(
                document,
    ...
    private static void ApplyLilToonScalarAndColorProperties(XRMaterial material, SerializedMaterialDocument document)
    {
    ...
    private static void ApplyLilToonColorAdjustmentProperties(XRMaterial material, SerializedMaterialDocument document)
    {
    ...
    private static bool TryGetLilToonHsvgAdjustments(SerializedMaterialDocument document, out float saturation, out float brightness)
    {
        saturation = 0.0f;
        brightness = 0.0f;
    ...
    private static void ApplySourceToonFeatureState(
        XRMaterial material,
        SerializedMaterialDocument document,
        string? alphaSource,
    ...
    {
        bool hasNormalMap =
            document.Textures.TryGetValue("_BumpMap", out SerializedTexturePropertyDocument? bumpMap) &&
        ...
        material.SetUberFeatureEnabled("stylized-shading",
            SourceToonFeatureStateResolver.IsEnabled(
                document,
                stylizedEvidence,
                ["_ShadingEnabled"],
                ["VIGNETTE_MASKED"]));
        ...
        ReportUnsupportedSourceToonFeatures(document, outlineMaskSource, diagnostics, warnings);
    }

    private static void ReportUnsupportedSourceToonFeatures(
        SerializedMaterialDocument document,
        string? outlineMaskSource,
        ICollection<MaterialConversionDiagnostic> diagnostics,
        ICollection<string> warnings)
    ...
    private static void ApplySourceToonRenderState(
        XRMaterial material,
        SerializedMaterialDocument document,
        ICollection<MaterialConversionDiagnostic> diagnostics)
    {
        SourceToonRenderStateConversion conversion = SourceToonRenderStateConverter.Convert(document, diagnostics);
        ...
        material.AlphaCutoff = document.TryGetFloat("_Cutoff", out float authoredCutoff)
            ...
            {
                SourceToonRenderPreset.Cutout => 0.5f,
                SourceToonRenderPreset.TransClipping => 0.01f,
                SourceToonRenderPreset.Fade => 0.002f,
                _ => 0.0f,
            };
    ...
    private static void ApplyLilToonRenderState(XRMaterial material, SerializedMaterialDocument document, string? shaderPath)
    {
    ...
    private static ETransparencyMode ResolveLilToonTransparencyMode(SerializedMaterialDocument document, string? shaderPath)
    {
    ...
    private static XRTexture2D? ResolveUberTexture(
        ...
        string samplerName)
    {
        if (!document.Textures.TryGetValue(sourceProperty, out SerializedTexturePropertyDocument? textureProperty) ||
        ...
        if (string.IsNullOrWhiteSpace(texturePath) || !File.Exists(texturePath))
        {
            ...
            if (resolver.ImportContext is not null)
                throw new SourceVisualImportException(message);
        ...
        try
        {
            texture = resolver.ImportContext is null
                ? ModelAssetImporter.GetOrCreateUberSamplerTexture(texturePath, samplerName)
                : ModelAssetImporter.GetOrCreateDeferredUberSamplerTexture(texturePath, samplerName);
        ...
        SerializedTextureImportDocument? importSettings = SerializedTextureImportDocumentParser.ParseFile(texturePath);
        resolver.ImportContext?.Progress?.Invoke(
            0.12f,
            $"Parsed texture settings {Path.GetFileName(texturePath)}");
        if (importSettings is not null)
            ApplySerializedTextureImportSettings(texture, importSettings, samplerName);
        resolver.ImportContext?.MarkOutcome(texturePath, SourceImportConversionOutcome.Converted);
        return texture;
    }
        ...
        params string[] sourceProperties)
    {
        foreach (string sourceProperty in sourceProperties)
        {
            ...
            if (resolver.ImportContext is not null)
            {
                XRTexture2D deferred = ModelAssetImporter.GetOrCreateDeferredUberSamplerTexture(
                    texturePath,
                    sourceProperty);
                SerializedTextureImportDocument? importSettings = SerializedTextureImportDocumentParser.ParseFile(texturePath);
                if (importSettings is not null)
                    ApplySerializedTextureImportSettings(deferred, importSettings, sourceProperty);
            ...
            return ModelAssetImporter.GetOrCreateUberSamplerTexture(texturePath, sourceProperty);
        }

        return null;
    }

    private static void ValidateRequiredSerializedTexture(
        string texturePath,
        SourceProjectImportContext? context)
    {
        if (context is null)
    ...
    private static void ApplySerializedTextureImportSettings(
        XRTexture2D texture,
        SerializedTextureImportDocument settings,
        string samplerName)
    {
        ...
        texture.UWrap = MapSourceWrapMode(settings.WrapU);
        texture.VWrap = MapSourceWrapMode(settings.WrapV);
    ...
    private static ETexWrapMode MapSourceWrapMode(int wrapMode)
        => wrapMode switch
        {
    ...
    private static void ApplySourceToonTextureTransform(
        XRMaterial material,
        SerializedMaterialDocument document,
        string? sourceProperty,
        string destinationSampler,
        ...
        ICollection<MaterialConversionDiagnostic> diagnostics)
    {
        ...
        int uvChannel = SourceToonEnumMapper.UvChannel(document, selector, 0, diagnostics);
        SetInt(material, $"{destinationSampler}UV", uvChannel);
    }

    private static void ApplyTextureTransform(XRMaterial material, SerializedMaterialDocument document, string? sourceProperty, string destinationSampler)
    {
        if (string.IsNullOrWhiteSpace(sourceProperty))
            return;

        if (document.Textures.TryGetValue(sourceProperty, out SerializedTexturePropertyDocument? textureProperty) && textureProperty is not null)
    ...
    private static SourceToonShaderMatchResult MatchSourceToon93Material(
        SerializedMaterialDocument document,
        SourceAssetResolver resolver,
        out string? shaderPath)
    {
        ...
        return SourceToonShaderMatcher.Match(new SourceToonShaderMatchInput
        {
            ShaderPath = shaderPath,
    ...
    private static bool IsLilToonMaterial(SerializedMaterialDocument document, SourceAssetResolver resolver, out string? shaderPath)
    {
        shaderPath = resolver.Resolve(document.Shader.Guid);
    ...
    private static void MapFloat(SerializedMaterialDocument document, XRMaterial material, string destination, params string[] sources)
    {
    ...
    private static void MapInt(SerializedMaterialDocument document, XRMaterial material, string destination, params string[] sources)
    {
        foreach (string source in sources)
        {
    ...
    private static void MapVector2(SerializedMaterialDocument document, XRMaterial material, string destination, params string[] sources)
    {
        foreach (string source in sources)
        {
    ...
    private static void MapVector3(SerializedMaterialDocument document, XRMaterial material, string destination, params string[] sources)
    {
        foreach (string source in sources)
        {
            if (!document.TryGetVector(source, out Vector4 value))
    ...
    private static void MapVector4(SerializedMaterialDocument document, XRMaterial material, string destination, params string[] sources)
    {
        foreach (string source in sources)
        {
            if (!document.TryGetVector(source, out Vector4 value))
    ...
    private static string ResolveSourceProjectRoot(string sourcePath)
    {
