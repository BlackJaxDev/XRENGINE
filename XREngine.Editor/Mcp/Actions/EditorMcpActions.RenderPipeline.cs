...
namespace XREngine.Editor.Mcp
{
    public sealed partial class EditorMcpActions
    {
        ...
        [XRMcp(Name = "capture_openxr_eye_preview_texture", Permission = McpPermissionLevel.ReadOnly)]
        [Description("Capture the latest OpenXR preview copy for the left or right eye and report pixel statistics.")]
        ...
        {
            bool leftEye;
            if (string.Equals(eye, "left", StringComparison.OrdinalIgnoreCase) ||
                string.Equals(eye, "l", StringComparison.OrdinalIgnoreCase) ||
                string.Equals(eye, "0", StringComparison.OrdinalIgnoreCase))
            {
                leftEye = true;
            }
            else if (string.Equals(eye, "right", StringComparison.OrdinalIgnoreCase) ||
                string.Equals(eye, "r", StringComparison.OrdinalIgnoreCase) ||
                string.Equals(eye, "1", StringComparison.OrdinalIgnoreCase))
            {
                leftEye = false;
            }
            else
            {
                return new McpToolResponse("eye must be 'left' or 'right'.", isError: true);
            }
            ...
            {
                return new McpToolResponse($"Failed to capture OpenXR {safeEyeName} preview texture: {ex.Message}", isError: true);
            }
        }
        ...
        private static PipelineTextureCaptureResult CaptureTexture(
            AbstractRenderer renderer,
            XRTexture texture,
            string path,
            int mipLevel,
