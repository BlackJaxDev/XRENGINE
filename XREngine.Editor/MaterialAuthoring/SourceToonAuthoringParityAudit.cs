...
public sealed record SourceToonAuthoringAuditEntry(
    string Id,
    string Kind,
...
/// </summary>
public static class SourceToonAuthoringParityAudit
{
    private static readonly Lazy<IReadOnlyList<SourceToonAuthoringAuditEntry>> Entries =
        new(Build, LazyThreadSafetyMode.ExecutionAndPublication);

    public static IReadOnlyList<SourceToonAuthoringAuditEntry> All => Entries.Value;

    public static IReadOnlyList<SourceToonAuthoringAuditEntry> Unclassified
    ...
    private static IReadOnlyList<SourceToonAuthoringAuditEntry> Build()
    {
        using Stream stream = SourceToon93Catalog.OpenCatalog();
        using JsonDocument document = JsonDocument.Parse(stream);
        List<SourceToonAuthoringAuditEntry> entries = [];
...
public sealed record SourceToonRemoteFacilityAuditEntry(
    ERemoteAuthoringFacility Facility,
    bool Reachable,
    EMaterialWorkflowClassification Classification,
    string NativeBehavior);

public static class SourceToonRemoteFacilityAudit
{
    public static IReadOnlyList<SourceToonRemoteFacilityAuditEntry> Entries { get; } =
    [
