...
public static partial class EditorUnitTests
{
    ...
    {
        private sealed record UnityPrefabStartupImportResult(
            SceneNode RootNode,
            int ModelCount,
            int MaterialCount,
        ...
        /// </summary>
        private static void ScheduleUnityPrefabImport(
            string prefabPath,
            Settings.ModelImportSettings settings,
            Action<UnityPrefabStartupImportResult> onFinished,
            Action<Exception> onError)
        {
            UnityPrefabStartupImportResult? result = null;
            ...
            {
                string? projectRoot = ResolveUnityProjectRoot(settings.UnityProjectRoot);
                Debug.Meshes(
                ...
                UnityPrefabConversionResult conversion = UnitySceneImporter.ImportPrefabWithManifest(
                    prefabPath,
                ...
                ValidateUnityPrefabCompletion(manifest, settings, prefabPath);

                (int modelCount, int materialCount, int poiyomiCount, int downgradeCount) =
                    ValidateUnityPrefabMaterials(convertedRoot, prefabPath);
                SceneNode importRoot = ApplyUnityPrefabImportTransform(convertedRoot, settings);
                ...
                result = new UnityPrefabStartupImportResult(
                    importRoot,
                    modelCount,
                    materialCount,
                    poiyomiCount,
            ...
            try
            {
                _ = Engine.Jobs.Schedule(
                    ...
                    completed: () =>
                    {
                        if (result is null)
                        {
                            onError(new UnityVisualImportException(
        ...
        /// </summary>
        private static List<ModelComponent> SuspendUnityPrefabModelPublication(
            SceneNode rootNode)
        {
        ...
        private static void StageUnityPrefabModelPublication(
            SceneNode rootNode,
        ...
        private static void ValidateUnityPrefabCompletion(
            UnityPrefabImportManifest manifest,
            Settings.ModelImportSettings settings,
            string prefabPath)
        {
            if (settings.Kind is not UnitTestModelImportKind.Animated ||
                manifest.CompletionTier == UnityImportCompletionTier.VisualAndAvatarBehavior)
            {
                return;
            }
            ...
            [
                .. manifest.Diagnostics
                    .Where(static diagnostic => diagnostic.Severity == UnityImportDiagnosticSeverity.Error)
            ...
            throw new UnityVisualImportException(
                ...
                $"ModelsToImport Kind=Animated requires {UnityImportCompletionTier.VisualAndAvatarBehavior}. {details}");
        }

        private static string? ResolveUnityProjectRoot(string? rawPath)
        {
        ...
        private static SceneNode ApplyUnityPrefabImportTransform(
        ...
        private static (int ModelCount, int MaterialCount, int PoiyomiCount, int DowngradeCount)
            ValidateUnityPrefabMaterials(SceneNode rootNode, string prefabPath)
        {
            ...
            foreach (XRMaterial material in materials)
            {
                ...
                if (unsupportedVersion)
                {
                    throw new UnityVisualImportException(
                        ...
                        $"{PoiyomiToon93Catalog.VersionText}.");
                }
                ...
                if (report.Outcome == EMaterialConversionOutcome.DowngradedToPoiyomiToon)
                    downgradeCount++;
            }

            return (models.Count, materials.Count, poiyomiCount, downgradeCount);
        }
    }
}
