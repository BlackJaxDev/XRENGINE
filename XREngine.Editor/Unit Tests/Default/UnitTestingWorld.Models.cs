...
public static partial class EditorUnitTests
{
    public static class Models
    {
        ...
        private static bool HasPostImportFlag(Settings.ModelImportSettings model, ModelPostImportFlags flag)
            => (model.PostImportFlags & flag) == flag;
        ...
        private static string[] ResolveTextureLoadDirSearchPaths()
        {
            ...
            foreach (string rawPath in Toggles.TextureLoadDirSearchPaths)
            {
                ...
                {
                    Debug.MeshesWarning($"[UnitTestingWorld] Ignoring invalid texture search path '{rawPath}'. {ex.Message}");
                    continue;
                }

                if (!Directory.Exists(resolvedPath))
                {
                    Debug.MeshesWarning($"[UnitTestingWorld] Texture search path does not exist: '{resolvedPath}' (raw='{rawPath}')");
                    continue;
                }
        ...
        private static void AttachAutoGeneratedStaticColliders(SceneNode rootNode, bool combineIntoSingleStaticRigidBodyComponent)
        {
            if (combineIntoSingleStaticRigidBodyComponent)
            {
                AttachCombinedAutoGeneratedStaticCollider(rootNode);
                return;
            }

            AttachAutoGeneratedStaticCollidersPerModelComponent(rootNode);
        }

        private static void AttachAutoGeneratedStaticCollidersPerModelComponent(SceneNode rootNode)
        {
            int colliderCount = 0;
        ...
        private static void AttachCombinedAutoGeneratedStaticCollider(SceneNode rootNode)
        {
            List<ModelComponent> targetComponents = [];
            rootNode.IterateComponents<ModelComponent>(component =>
            {
                if (component.Model is not null || component.Meshes.Count > 0)
                    targetComponents.Add(component);
            }, true);

            if (targetComponents.Count == 0)
            {
                Debug.MeshesWarning($"[StaticModel] Could not enable combined CoACD collider generation because '{rootNode.Name}' has no imported model components.");
                return;
            }

            StaticRigidBodyComponent? staticBody = rootNode.AddComponent<StaticRigidBodyComponent>();
            if (staticBody is null)
                return;

            staticBody.TargetModelComponents = targetComponents;
            staticBody.AutoGenerateConvexCollidersFromSiblingModel = true;
        ...
        public static void ImportModels(string desktopDir, SceneNode rootNode, SceneNode characterParentNode, Action? onAllModelsImported = null)
        {
            ...
            if (Engine.StartingUp)
                Debug.Meshes($"[UnitTestingWorld] Scheduling {modelsToSchedule.Count} startup model import(s) across app-thread dispatches.");
        ...
        private static void WireLightProbeSpawnerToImportedModels(SceneNode rootNode, SceneNode[] importedStaticRoots)
        {
            ...
            {
                Debug.Lighting("[LightProbeGrid] No LightProbeGridSpawnerComponent found � skipping model-bounds wiring.");
                return;
            }
            ...
            if (models.Count == 0)
            {
                Debug.Lighting("[LightProbeGrid] No ModelComponents found under imported static models � skipping model-bounds wiring.");
                return;
            }
