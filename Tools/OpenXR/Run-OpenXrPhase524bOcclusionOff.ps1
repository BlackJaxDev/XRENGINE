...
try {
    ...
    if (-not [string]::IsNullOrWhiteSpace($EditorDll)) { $runnerArguments.EditorDll = $EditorDll }
    if ($NoBuild) { $runnerArguments.NoBuild = $true }
    if ($StartService) { $runnerArguments.StartService = $true }
    if ($StartService) {
        $runnerArguments.SimulatedHmdPoseMode = "stationary"
        $runnerArguments.RequireOwnedService = $true
    }
