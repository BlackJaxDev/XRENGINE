...
$serviceScript = Join-Path $PSScriptRoot "Start-MonadoService.ps1"
...
$matrixServiceStarted = $false
$matrixServiceMarker = Join-Path $RunRoot "mcp-output\monado-service-marker.json"
...
try {
    if ($StartService) {
        $serviceResult = & $serviceScript `
            ...
            -MarkerPath $matrixServiceMarker `
            -LogDirectory (Join-Path $RunRoot "logs\monado-service")
        $serviceResult | ConvertTo-Json -Depth 6 |
            Set-Content -LiteralPath (Join-Path $reports "monado-service-start.json") -Encoding UTF8
        if (-not $serviceResult.OwnedByRunner -or
            [string]$serviceResult.SimulatedHmdPoseMode -cne "stationary") {
            throw "Strict SPS failure-matrix validation requires a runner-owned stationary Monado service."
        }
        $matrixServiceStarted = $true
    }
    ...
    if ($matrixServiceStarted) {
        try {
            & $serviceScript -MarkerPath $matrixServiceMarker -Stop |
                ConvertTo-Json -Depth 6 |
                Set-Content -LiteralPath (Join-Path $reports "monado-service-stop.json") -Encoding UTF8
        }
        catch {
            Write-Warning "Failed to stop strict failure-matrix Monado service: $($_.Exception.Message)"
        }
    }
