...
$temporalScenarioDefinitions = @(
    ...
    [pscustomobject]@{ scenario = "Disocclusion"; sample = "DisocclusionRevealed"; velocityOracle = "Zero"; start = 48; end = 50; convergence = $true; disocclusionBaseline = $false; disocclusionResult = $true },
...
$desktopRecoveryStarts = ($desktopOcclusion | Measure-Object -Property recoveryStarts -Sum).Sum
$desktopRecoveryCompletions = ($desktopOcclusion | Measure-Object -Property recoveryCompletions -Sum).Sum
...
$vrRecoveryStarts = ($vrOcclusion | Measure-Object -Property recoveryStarts -Sum).Sum
$vrRecoveryCompletions = ($vrOcclusion | Measure-Object -Property recoveryCompletions -Sum).Sum
...
if ([string]::IsNullOrWhiteSpace($OcclusionOffSummaryPath)) {
    ...
    if (-not (Test-Path -LiteralPath $OcclusionOffSummaryPath -PathType Leaf)) {
        ...
        if ([string]$occlusionOffSummary.occlusionCullingModeEffective -cne "Disabled" -or
            ...
            [string]$occlusionOffSummary.foveationModeEffective -cne [string]$summary.foveationModeEffective -or
...
for ($motionIndex = 1; $motionIndex -lt $captureMotionSampleCount; $motionIndex++) {
    ...
    if ([int]$motionEntries[0].width -ne $desktopExpectedWidth -or
        ...
        [int]$motionEntries[0].topBandMagentaPixelCount -le 0 -or
...
$maxBloomRelativeEnergyDelta = 0.02
$maxBloomCentroidDistance = 0.005
    ...
    foreach ($viewKind in @("LeftEye", "RightEye")) {
        foreach ($boundary in @(
            ...
            if ($png.signature -cne "89504E470D0A1A0A" -or
                ...
                [int]$boundaryEntry.topBandMagentaPixelCount -le 0 -or
                ($retainedCohortStart -ne [DateTimeOffset]::MinValue -and
...
for ($layerIndex = 0; $layerIndex -lt 2; $layerIndex++) {
    $occluded = @($temporalScenarioCaptureLedger | Where-Object {
        ...
        [string]$_.stage -ceq "13b_PreTsrHistoryColor" -and [int]$_.layerIndex -eq $layerIndex
    })
...
if (-not $SkipSubNativeCompanion -and $failures.Count -eq 0) {
    ...
    $offArguments = @{
        ...
        TsrResolutionScale = $SubNativeTsrResolutionScale
        TimeoutSeconds = $TimeoutSeconds
        RunRoot = $subNativeOffRunRoot
        NoBuild = $true
    }
    if (-not [string]::IsNullOrWhiteSpace($RuntimeJson)) { $offArguments.RuntimeJson = $RuntimeJson }
    if ($StartService) { $offArguments.StartService = $true }
    try {
        $LASTEXITCODE = 0
        & $offRunner @offArguments
