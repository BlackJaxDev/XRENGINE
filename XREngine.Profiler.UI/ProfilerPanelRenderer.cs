...
public sealed class ProfilerPanelRenderer(IProfilerDataSource source)
{
    ...
    public void DrawRenderStatsPanel(ref bool open, bool allowClose = true, Action? drawHeader = null)
    {
        ...
        ImGui.Text($"  Command Chain Workers: queued {loop.CommandChainWorkerQueuedChains:N0} | started/completed {loop.CommandChainWorkersStarted:N0}/{loop.CommandChainWorkersCompleted:N0} | peak {loop.CommandChainPeakConcurrentWorkers:N0} | serial/reused/conflicts {loop.CommandChainSeriallyRecorded:N0}/{loop.CommandChainWorkerReused:N0}/{loop.CommandChainWorkerConflicts:N0} | failures/timeouts {loop.CommandChainWorkerFailures:N0}/{loop.CommandChainWorkerWaitTimeouts:N0}");
        ImGui.Text($"  Command Chain Timing: queue {loop.CommandChainWorkerQueueDelayMs:F3} ms | record {loop.CommandChainWorkerRecordMs:F3} ms | active {loop.CommandChainWorkerActiveSpanMs:F3} ms | overlap {loop.CommandChainWorkerOverlapMs:F3} ms | merge {loop.CommandChainWorkerMergeMs:F3} ms | wait {loop.RenderThreadWaitForChainWorkersMs:F3} ms | primary reused/recorded {loop.PrimaryCommandBuffersReused:N0}/{loop.PrimaryCommandBuffersRecorded:N0}");
