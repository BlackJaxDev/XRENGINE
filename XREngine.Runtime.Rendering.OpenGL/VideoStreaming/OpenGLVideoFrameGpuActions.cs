...
/// upload queue.
/// </summary>
public sealed class OpenGLVideoFrameGpuActions : IVideoFrameGpuActions
{
    private const int PboCount = 2;
    private readonly IOpenGLVideoFrameUploadContext _context;
    ...
    public OpenGLVideoFrameGpuActions(IOpenGLVideoFrameUploadContext context)
    ...
    public bool UploadVideoFrame(DecodedVideoFrame frame, object? targetTexture, out string? error)
    {
        ...
        IOpenGLVideoFrameTextureHandle? glTex = _context.ResolveTexture(xrTexture);
        if (glTex is null)
        {
