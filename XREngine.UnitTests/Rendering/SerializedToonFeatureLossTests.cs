...
[TestFixture]
[NonParallelizable]
public sealed class SerializedToonFeatureLossTests
{
        ...
        bool grabPass)
    {
        ...
        SourceToonShaderMatchResult match = SourceToonShaderMatcher.Match(
            new SourceToonShaderMatchInput
            {
                ShaderPath = path,
                ShaderSource =
        ...
        SourceToonShaderVersion.TryParse(version, out SourceToonShaderVersion expectedVersion)
            .ShouldBeTrue();
        match.Kind.ShouldBe(SourceToonShaderMatchKind.SourceToonFeatureLossSource);
        match.SourceFamily.ShouldBe(SourceToonShaderFamily.Pro);
        match.Version.ShouldBe(expectedVersion);
        match.IsSourceToonFamily.ShouldBeTrue();
    ...
    public void Importer_NormalizesCommonSurfaceAndDropsActiveProOnlyGroups()
    {
        ...
        result.IsSourceToonFeatureLoss.ShouldBeTrue();
        result.IsSourceToon.ShouldBeFalse();
        result.SourceToonShaderMatch.ShouldNotBeNull().IsLocked.ShouldBeTrue();
        result.SourceToonShaderMatch.Version.ShouldBe(new SourceToonShaderVersion(9, 3, 11));
        result.ConversionReport.ShouldNotBeNull().Outcome
            .ShouldBe(EMaterialConversionOutcome.ConvertedToSourceToon);
    ...
    public void Matcher_DoesNotTreatSupportedProximityPropertiesAsProEvidence()
    {
        ...
        match.SourceFamily.ShouldBe(SourceToonShaderFamily.Toon);
    }
