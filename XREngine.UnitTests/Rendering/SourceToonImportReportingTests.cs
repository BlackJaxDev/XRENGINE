...
[TestFixture]
[NonParallelizable]
public sealed class SourceToonImportReportingTests
{
    ...
    public void ImportReport_IsDeterministicAndDescribesSourceParity()
    {
        ...
        report.SourceShaderVersion.ShouldBe(SourceToon93Catalog.VersionText);
    ...
    public void Reconvert_PreservesSeparatedOverrides_AndResetRestoresImportedValue()
    {
        ...
        SerializedMaterialAsset asset = new();
        ...
        MaterialReimportWorkflow.Reconvert(asset, out SerializedMaterialImportResult preserved).ShouldBeTrue();
    ...
    public async Task BatchAudit_IsSortedAggregatedAndMachineReadable()
    {
        ...
        SerializedMaterialBatchReport first =
            await SerializedMaterialBatchConversionService.AuditProjectAsync(projectRoot);
        SerializedMaterialBatchReport secondRun =
            await SerializedMaterialBatchConversionService.AuditProjectAsync(projectRoot);
        ...
        float emissionStrength)
    {
        ...
        File.WriteAllText(
            shaderPath + ".meta",
            $"fileFormatVersion: 2\nguid: {SourceToon93Catalog.ShaderGuid}\n");
...
Material:
  serializedVersion: 8
  m_Name: {{name}}
  m_Shader: {fileID: 4800000, guid: {{SourceToon93Catalog.ShaderGuid}}, type: 3}
