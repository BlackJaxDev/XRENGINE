...
public sealed class StereoPostProcessContractTests
{
    ...
    public void StereoBloomPass_UsesDestinationMipRegionsAndTwoLayerViews()
    {
        ...
        contract.ShouldContain("BoundingRectangle? destinationRegion = null");
        contract.ShouldContain("BoundingRectangle expectedRegion = destinationRegion ??");
        contract.ShouldContain("int destinationWidth = expectedRegion.Width;");
        contract.ShouldContain("int destinationHeight = expectedRegion.Height;");
        contract.ShouldContain("width != destinationWidth || height != destinationHeight");
    }
    ...
    public void StereoFullscreenPasses_UseDestinationExtentAndValidateLocalRasterMapping()
    {
        ...
        string postProcessing = ReadWorkspaceFile(
    ...
    public void Phase524bMonoReference_RendersEachEyeThroughTheMonoTsrEntryPoint()
    {
        ...
        frameBuffers.ReplaceLineEndings("\n").ShouldContain("1u,\n            format,\n            array: false");
        ...
        CountOccurrences(chain, "x.ApplyUniforms = TsrMonoReferenceFBO_SettingUniforms").ShouldBe(2);
        chain.ShouldContain("AppendDiagnosticTextureCapture(tsrUpscale, \"13c_MonoTsrReference\"");
        CountOccurrences(frameBuffers, "TsrMonoReferenceFBO_SettingUniforms").ShouldBe(2);
        postProcessing.ShouldContain("private void TsrMonoReferenceFBO_SettingUniforms(XRRenderProgram program)");
        postProcessing.ShouldContain("=> TsrResolveFBO_SettingUniforms(program);");
