...
[TestFixture]
public sealed class SourceToonCatalogTests
{
    [Test]
    public void EmbeddedCatalog_AccountsForPinnedShaderAndActiveUiSurface()
    {
        ...
        source.GetProperty("shaderGuid").GetString().ShouldBe(SourceToon93Catalog.ShaderGuid);
        source.GetProperty("shaderBlob").GetString().ShouldBe(SourceToon93Catalog.ShaderBlob);
        source.GetProperty("shaderSha256").GetString().ShouldBe(SourceToon93Catalog.ShaderSha256);
        source.GetProperty("thryEditorTree").GetString().ShouldBe(SourceToon93Catalog.ThryEditorTree);
    ...
    {
        SourceToonShaderMatchResult exact = SourceToonShaderMatcher.Match(new SourceToonShaderMatchInput
        {
        ...
        exact.Kind.ShouldBe(SourceToonShaderMatchKind.ExactUnlockedSource);
        exact.Version.ShouldBe(SourceToon93Catalog.Version);
        exact.Diagnostics.ShouldBeEmpty();

        SourceToonShaderMatchResult unknown = SourceToonShaderMatcher.Match(new SourceToonShaderMatchInput
        {
        ...
        unknown.IsSourceToonFamily.ShouldBeTrue();
        unknown.Kind.ShouldBe(SourceToonShaderMatchKind.UnsupportedVersion);
    ...
    {
        SourceToonShaderMatchResult tagged = SourceToonShaderMatcher.Match(new SourceToonShaderMatchInput
        {
        ...
        tagged.Kind.ShouldBe(SourceToonShaderMatchKind.ExactLockedSource);
        ...
        SourceToonShaderMatchResult signatureOnly = SourceToonShaderMatcher.Match(new SourceToonShaderMatchInput
        {
            ShaderPath = "Assets/Materials/OptimizedShaders/Body/Locked.shader",
        ...
        signatureOnly.Kind.ShouldBe(SourceToonShaderMatchKind.LockedPropertySignature);
    ...
    {
        SourceToon93Catalog.GetMaterialName(
                @"Assets\Avatar Materials\Body!.mat",
                "ABCDEF0123456789abcdef0123456789")
            .ShouldBe("Body.poiyomi-9_3_64.abcdef01.uber");
        SourceToon93Catalog.GetPassVariantName("Body Material", "outline inverse hull", 0x1234UL)
            .ShouldBe("Body-Material.outline-inverse-hull.0000000000001234");
        SourceToon93Catalog.GetPreservedMetadataName("Body Material")
            .ShouldBe("Body-Material.poiyomi-source.json");
        SourceToon93Catalog.GetAnimationBindingName("Body Material", "_Rim Width")
