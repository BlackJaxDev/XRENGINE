...
[TestFixture]
[NonParallelizable]
public sealed class SourceToonConversionBaselineTests
{
    ...
    [Test]
    public void Importer_MapsIndependentBaselineSamplerFamiliesAndPackedChannels()
    {
        (string projectRoot, string materialPath) = CreateProject("baseline-mappings");
    ...
    [Test]
    public void Importer_PropagatesSourceSamplerMetadataAndUsesSemanticDefaults()
    {
        (string projectRoot, string materialPath) = CreateProject("baseline-metadata");
    ...
    {
        (string projectRoot, string materialPath) = CreateProject("baseline-disabled");
    ...
    {
        (string projectRoot, string materialPath) = CreateProject("baseline-diagnostics");
        ...
        result.ConversionReport.ShouldNotBeNull().DiagnosticGroups
        ...
        result.ConversionReport.PreservedInactiveValues.ShouldContain(value =>
            value.SourceProperty == "_SssScale" && value.SerializedValue == "0.5");
        material.IsUberFeatureEnabled("outline", defaultEnabled: false).ShouldBeTrue();
        result.ConversionReport.ShouldNotBeNull().Features.ShouldContain(feature =>
            feature.FeatureId == "outline" &&
            feature.RuntimeEnabled &&
            feature.Parity == EMaterialFeatureParity.NativeEquivalent);

        SerializedMaterialImporter.ValidateMeshCompatibility(material, 2)
            .ShouldContain(diagnostic =>
                diagnostic.Code == MaterialConversionDiagnosticCodes.RequestedUvChannelMissing &&
                diagnostic.SourceProperty == "UV3");
        SerializedMaterialImporter.ValidateMeshCompatibility(material, 4).ShouldBeEmpty();
    }
    ...
    {
        (string projectRoot, string materialPath) = CreateProject($"baseline-mode-{sourceMode}");
        (string Property, string Guid, string Path) main =
            CreateTexture(projectRoot, "_MainTex", 50 + sourceMode);
        WriteMaterial(materialPath, [main], floats: [("_Mode", sourceMode)]);

        SerializedMaterialImporter.ImportWithReport(materialPath, projectRoot)
    ...
    private static (string ProjectRoot, string MaterialPath) CreateProject(string suffix)
    {
        ...
        File.WriteAllText(shaderPath, $"Shader \".poiyomi/Poiyomi Toon\" {{ // Poiyomi {SourceToon93Catalog.VersionText} }}");
        WriteMeta(shaderPath, SourceToon93Catalog.ShaderGuid, srgb: true);
        ...
        IReadOnlyList<(string Property, float Value)>? floats = null)
    {
        ...
        yaml.AppendLine("  m_Name: BaselineFixture");
        yaml.AppendLine($"  m_Shader: {{fileID: 4800000, guid: {SourceToon93Catalog.ShaderGuid}, type: 3}}");
