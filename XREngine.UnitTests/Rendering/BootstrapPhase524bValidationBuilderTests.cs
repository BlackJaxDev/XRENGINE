...
public sealed class BootstrapPhase524bValidationBuilderTests
{
    ...
    public void HeadsetRelativeMovingSentinelTrajectory_IsPeriodicVisibleAndAlwaysMoving()
    {
        ...
        for (uint tick = 0u; tick < 120u; tick++)
        {
            ...
            translation.X.ShouldBeInRange(-0.5701f, -0.3299f);
            translation.Y.ShouldBeInRange(-0.3601f, -0.2399f);
            translation.Z.ShouldBe(-1.25f, 1.0e-6f);

            // Conservative normalized view bounds keep the complete 0.28 m box
            ...
            Vector3.Distance(next, translation).ShouldBeGreaterThan(0.003f);
        }
    }
