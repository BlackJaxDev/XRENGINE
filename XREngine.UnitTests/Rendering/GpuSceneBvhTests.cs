...
public class GpuSceneBvhTests
{
    ...
    /// <summary>
    /// Mock item for octree testing that implements IOctreeItem interface.
    /// </summary>
    private sealed class MockOctreeItem : IOctreeItem
    {
        public MockOctreeItem(AABB bounds, bool shouldRender = true)
        {
            LocalCullingVolume = bounds;
            ShouldRender = shouldRender;
        }

        public MockOctreeItem(Vector3 center, float radius, bool shouldRender = true)
        {
            var halfExtent = new Vector3(radius);
            LocalCullingVolume = new AABB(center - halfExtent, center + halfExtent);
            ShouldRender = shouldRender;
        }
    ...
    #region Octree / BVH Integration Tests

    [Test]
    public void OctreeGpu_Add_SingleItem_MarksTreeDirty()
    {
        // Arrange
        var bounds = new AABB(Vector3.Zero, new Vector3(100f));
        var octree = new OctreeGPU<MockOctreeItem>(bounds);
        var item = new MockOctreeItem(Vector3.Zero, 1f);

        // Act
        octree.Add(item);

        // Assert - tree should have one item tracked
        octree.ShouldNotBeNull();
    }

    [Test]
    public void OctreeGpu_AddRange_MultipleItems_AllItemsTracked()
    {
        // Arrange
        var bounds = new AABB(Vector3.Zero, new Vector3(100f));
        var octree = new OctreeGPU<MockOctreeItem>(bounds);
        var items = new[]
        {
            new MockOctreeItem(new Vector3(-10, 0, 0), 1f),
            new MockOctreeItem(new Vector3(0, 0, 0), 1f),
            new MockOctreeItem(new Vector3(10, 0, 0), 1f),
            new MockOctreeItem(new Vector3(0, 10, 0), 1f),
            new MockOctreeItem(new Vector3(0, -10, 0), 1f)
        };
        ...
        // Assert - all items should be added without exception
        octree.ShouldNotBeNull();
    }

    [Test]
    public void OctreeGpu_Remove_SingleItem_MarksTreeDirty()
    {
        // Arrange
        var bounds = new AABB(Vector3.Zero, new Vector3(100f));
        var octree = new OctreeGPU<MockOctreeItem>(bounds);
    ...
    [Test]
    public void OctreeGpu_Remake_WithNewBounds_UpdatesBounds()
    {
        // Arrange
        var initialBounds = new AABB(Vector3.Zero, new Vector3(50f));
        var octree = new OctreeGPU<MockOctreeItem>(initialBounds);

        // Act
        var newBounds = new AABB(new Vector3(-100f), new Vector3(100f));
        octree.Remake(newBounds);

        // Assert
        octree.Bounds.Min.X.ShouldBe(-100f);
        octree.Bounds.Max.X.ShouldBe(100f);
    }

    [Test]
    public void OctreeGpu_UseBvh_ToggleSetting_MarksDirty()
    {
        // Arrange
        var bounds = new AABB(Vector3.Zero, new Vector3(100f));
        var octree = new OctreeGPU<MockOctreeItem>(bounds);

        // Act - toggle BVH usage
        octree.UseBvh = false;
        octree.UseBvh = true;

        // Assert - setting should update
        octree.UseBvh.ShouldBeTrue();
    }

    [Test]
    public void OctreeGpu_BvhMode_SetToMortonPlusSah_UpdatesCorrectly()
    {
        // Arrange
        var bounds = new AABB(Vector3.Zero, new Vector3(100f));
        var octree = new OctreeGPU<MockOctreeItem>(bounds);
        ...
        // Act
        octree.BvhMode = BvhBuildMode.MortonPlusSah;

        // Assert
        octree.BvhMode.ShouldBe(BvhBuildMode.MortonPlusSah);
    }

    [Test]
    public void OctreeGpu_MaxLeafPrimitives_ClampedToMinimumOne()
    {
        // Arrange
        var bounds = new AABB(Vector3.Zero, new Vector3(100f));
        var octree = new OctreeGPU<MockOctreeItem>(bounds);

        // Act - set to 0, should clamp to 1
        octree.MaxLeafPrimitives = 0;

        // Assert
        octree.MaxLeafPrimitives.ShouldBe(1u);
    }

    [Test]
    public void OctreeGpu_MaxLeafPrimitives_AcceptsValidValue()
    {
        // Arrange
        var bounds = new AABB(Vector3.Zero, new Vector3(100f));
        var octree = new OctreeGPU<MockOctreeItem>(bounds);
        ...
        // Act
        octree.MaxLeafPrimitives = 8;

        // Assert
        octree.MaxLeafPrimitives.ShouldBe(8u);
    }

    [Test]
    public void OctreeGpu_SwapMode_AlwaysRebuild_MarksTreeForRebuild()
    {
        // Arrange
        var bounds = new AABB(Vector3.Zero, new Vector3(100f));
        var octree = new OctreeGPU<MockOctreeItem>(bounds);
        octree.SwapMode = SwapUpdateMode.AlwaysRebuild;
        ...
        // Assert - swap should not throw
        octree.SwapMode.ShouldBe(SwapUpdateMode.AlwaysRebuild);
    }

    [Test]
    public void OctreeGpu_SwapMode_RefitOnly_PreservesExistingStructure()
    {
        // Arrange
        var bounds = new AABB(Vector3.Zero, new Vector3(100f));
        var octree = new OctreeGPU<MockOctreeItem>(bounds);
        octree.SwapMode = SwapUpdateMode.RefitOnly;

        // Act & Assert - should not throw
        octree.Swap();
        octree.SwapMode.ShouldBe(SwapUpdateMode.RefitOnly);
    }
    ...
    [Test]
    public void OctreeGpu_CapacityValidation_FailsWhenExceedingMaxObjects()
    {
        // Arrange
        var octree = new OctreeGPU<MockOctreeItem>();
        uint objectCount = OctreeGPU<MockOctreeItem>.MaxObjects + 1;
        uint nodeCount = objectCount * 2u - 1u;

        // Act
        bool isValid = octree.TryValidateCapacityForTests(objectCount, nodeCount, out var reason);
    ...
    [Test]
    public void OctreeGpu_CapacityValidation_FailsWhenQueueOverflows()
    {
        // Arrange
        var octree = new OctreeGPU<MockOctreeItem>();
        uint objectCount = (OctreeGPU<MockOctreeItem>.QueueCapacity / 2u) + 10u;
        uint nodeCount = objectCount * 2u - 1u;
    ...
    [Test]
    public void OctreeGpu_CapacityValidation_SucceedsWithinLimits()
    {
        // Arrange
        var octree = new OctreeGPU<MockOctreeItem>();
        const uint objectCount = 64;
        uint nodeCount = objectCount * 2u - 1u;
    ...
    [Test]
    public void OctreeGpu_CapacityValidation_HandlesZeroObjects()
    {
        // Arrange
        var octree = new OctreeGPU<MockOctreeItem>();

        // Act
        bool isValid = octree.TryValidateCapacityForTests(0, 0, out var reason);
    ...
    [Test]
    public void OctreeGpu_CapacityValidation_HandlesSingleObject()
    {
        // Arrange
        var octree = new OctreeGPU<MockOctreeItem>();

        // Act
        bool isValid = octree.TryValidateCapacityForTests(1, 1, out var reason);
    ...
    [Test]
    public void OctreeGpu_Dispose_ReleasesResources()
    {
        // Arrange
        var bounds = new AABB(Vector3.Zero, new Vector3(100f));
        var octree = new OctreeGPU<MockOctreeItem>(bounds);
        ...
        // Assert - should not throw and subsequent operations should handle disposed state
        Should.NotThrow(() => octree.Dispose()); // Double dispose should be safe
    }

    [Test]
    public void OctreeGpu_ReleaseBvhResources_ClearsBvhState()
    {
        // Arrange
        var bounds = new AABB(Vector3.Zero, new Vector3(100f));
        var octree = new OctreeGPU<MockOctreeItem>(bounds);
        ...
        // Act
        octree.ReleaseBvhResources();

        // Assert - should not throw
        octree.BvhNodesBuffer.ShouldBeNull();
    }
