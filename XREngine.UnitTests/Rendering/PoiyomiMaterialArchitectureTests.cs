...
[TestFixture]
public sealed class PoiyomiMaterialArchitectureTests
{
    [TestCase(0, PoiyomiRenderPreset.Opaque, 2000, ETransparencyMode.Opaque, EDefaultRenderPass.OpaqueForward, EBlendingFactor.One, EBlendingFactor.Zero, true)]
    [TestCase(1, PoiyomiRenderPreset.Cutout, 2450, ETransparencyMode.Masked, EDefaultRenderPass.MaskedForward, EBlendingFactor.One, EBlendingFactor.Zero, true)]
    [TestCase(9, PoiyomiRenderPreset.TransClipping, 2460, ETransparencyMode.Masked, EDefaultRenderPass.MaskedForward, EBlendingFactor.SrcAlpha, EBlendingFactor.OneMinusSrcAlpha, true)]
    [TestCase(2, PoiyomiRenderPreset.Fade, 3000, ETransparencyMode.AlphaBlend, EDefaultRenderPass.TransparentForward, EBlendingFactor.SrcAlpha, EBlendingFactor.OneMinusSrcAlpha, false)]
    [TestCase(3, PoiyomiRenderPreset.Transparent, 3000, ETransparencyMode.PremultipliedAlpha, EDefaultRenderPass.TransparentForward, EBlendingFactor.One, EBlendingFactor.OneMinusSrcAlpha, false)]
    [TestCase(4, PoiyomiRenderPreset.Additive, 3000, ETransparencyMode.Additive, EDefaultRenderPass.TransparentForward, EBlendingFactor.One, EBlendingFactor.One, false)]
    [TestCase(5, PoiyomiRenderPreset.SoftAdditive, 3000, ETransparencyMode.AlphaBlend, EDefaultRenderPass.TransparentForward, EBlendingFactor.OneMinusDstColor, EBlendingFactor.One, false)]
    [TestCase(6, PoiyomiRenderPreset.Multiplicative, 3000, ETransparencyMode.AlphaBlend, EDefaultRenderPass.TransparentForward, EBlendingFactor.DstColor, EBlendingFactor.Zero, false)]
    [TestCase(7, PoiyomiRenderPreset.Multiplicative2X, 3000, ETransparencyMode.AlphaBlend, EDefaultRenderPass.TransparentForward, EBlendingFactor.DstColor, EBlendingFactor.SrcColor, false)]
    public void PresetMapping_IsExact(
        int mode,
        PoiyomiRenderPreset expectedPreset,
        int expectedQueue,
    ...
    {
        PoiyomiRenderStateConversion conversion = Convert(CreateDocument(("_Mode", mode)));
    ...
    {
        UnityMaterialDocument document = CreateDocument(
            ("_Mode", 0),
        ...
        PoiyomiRenderStateConversion conversion = Convert(document);
    ...
    private static PoiyomiRenderStateConversion Convert(UnityMaterialDocument document)
        => PoiyomiRenderStateConverter.Convert(document, new List<MaterialConversionDiagnostic>());

    private static MaterialPassDefinition GetPass(
        PoiyomiRenderStateConversion conversion,
    ...
    private static UnityMaterialDocument CreateDocument(params (string Name, float Value)[] properties)
    {
        UnityMaterialDocument document = new() { Name = "MaterialArchitecture" };
        foreach ((string name, float value) in properties)
            document.Floats.Add(name, value);
        return document;
    }

    private static UnityMaterialDocument WithQueue(UnityMaterialDocument source, int queue)
    {
        UnityMaterialDocument result = new()
        {
            Name = source.Name,
