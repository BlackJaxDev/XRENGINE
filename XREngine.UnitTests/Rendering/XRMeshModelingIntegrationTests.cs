...
[TestFixture]
public class XRMeshModelingIntegrationTests
{
    [Test]
    public void RoundTrip_PreservesPositionsAndTriangleIndices()
    {
        ...
        ModelingMeshDocument document = XRMeshToModelingDocumentConverter.Convert(sourceMesh);
        XRMesh roundTrippedMesh = ModelingDocumentToXRMeshConverter.Convert(document);
    ...
    [Test]
    public void ModelingDocumentToXRMesh_RejectsInvalidIndicesWithValidationFailure()
    {
        ...
        InvalidOperationException ex = Should.Throw<InvalidOperationException>(() => ModelingDocumentToXRMeshConverter.Convert(document));
        ex.Message.ShouldContain("validation");
        ex.Message.ShouldContain("triangle_index_out_of_range");
    }

    [Test]
    public void XRMeshToModelingDocument_HandlesMeshesWithNoNormalsUvColors()
    {
        ...
        ModelingMeshDocument document = XRMeshToModelingDocumentConverter.Convert(mesh);
    ...
    public void SaveContract_InvalidatesIndexCacheAndUpdatesBounds()
    {
        ...
        ModelingDocumentToXRMeshConverter.ApplyRuntimeMeshContract(mesh);
    ...
    [Test]
    public void ModelingDocumentToXRMesh_PreserveDocumentOrder_IsDeterministicAcrossRepeatedRuns()
    {
        ModelingMeshDocument document = CreateScrambledQuadDocumentWithAttributes();
        ModelingDocumentToXRMeshOptions options = new()
        {
            OrderingPolicy = ModelingDocumentToXRMeshOrderingPolicy.PreserveDocumentOrder
        };

        MeshSnapshot baseline = CaptureMeshSnapshot(ModelingDocumentToXRMeshConverter.Convert(document, options));
        for (int i = 0; i < 4; i++)
        {
            MeshSnapshot rerun = CaptureMeshSnapshot(ModelingDocumentToXRMeshConverter.Convert(document, options));
            AssertMeshSnapshotsEqual(baseline, rerun);
        }
    }

    [Test]
    public void ModelingDocumentToXRMesh_CanonicalizedOrdering_IsDeterministicAcrossRepeatedRuns()
    {
        ModelingMeshDocument document = CreateScrambledQuadDocumentWithAttributes();
        ModelingDocumentToXRMeshOptions options = new()
        {
            OrderingPolicy = ModelingDocumentToXRMeshOrderingPolicy.Canonicalized
        };
    ...
    public void MeshEditingPawn_TopologyOperations_ProduceValidTopologyAndReprojectChannelsOnSave()
    {
        ...
        ModelingDocumentToXRMeshOptions options = new()
        {
            SkinningBlendshapeFallbackPolicy = ModelingDocumentToXRMeshSkinningBlendshapeFallbackPolicy.PermissiveNearestSourceVertexReproject
        };
    ...
    [Test]
    public void ModelingDocumentToXRMesh_RejectsInvalidSkinWeightBoneIndex()
    {
        ModelingMeshDocument document = CreateScrambledQuadDocumentWithAttributes();
        document.SkinBones =
        [
    ...
    public void MeshEditingPawn_SaveToXRMesh_StrictFallback_ThrowsOnTopologyChange()
    {
        ...
        ModelingDocumentToXRMeshOptions options = new()
        {
            SkinningBlendshapeFallbackPolicy = ModelingDocumentToXRMeshSkinningBlendshapeFallbackPolicy.Strict
        };
    ...
    public void MeshEditingPawn_SaveToXRMesh_PermissiveDrop_RemovesSkinningAndBlendshapesOnTopologyChange()
    {
        ...
        ModelingDocumentToXRMeshOptions options = new()
        {
            SkinningBlendshapeFallbackPolicy = ModelingDocumentToXRMeshSkinningBlendshapeFallbackPolicy.PermissiveDropChannels
        };
    ...
    [Test]
    public void ModelingDocumentToXRMesh_SaveContract_RemainsValidWithSkinningAndBlendshapeChannels()
    {
        XRMesh sourceMesh = CreateIndexedQuadMeshWithSkinningAndBlendshapes();
        ModelingMeshDocument document = XRMeshToModelingDocumentConverter.Convert(sourceMesh);
        document.Positions[0] = new Vector3(8f, 0f, 0f);

        XRMesh convertedMesh = ModelingDocumentToXRMeshConverter.Convert(document, new ModelingDocumentToXRMeshOptions
        {
            SkinningBlendshapeFallbackPolicy = ModelingDocumentToXRMeshSkinningBlendshapeFallbackPolicy.PermissiveNearestSourceVertexReproject
        });

        convertedMesh.HasSkinning.ShouldBeTrue();
        convertedMesh.HasBlendshapes.ShouldBeTrue();
        convertedMesh.Bounds.Max.X.ShouldBeGreaterThanOrEqualTo(8f);
        convertedMesh.HasCachedIndexBuffer(EPrimitiveType.Triangles).ShouldBeFalse();
        convertedMesh.HasAccelerationCache().ShouldBeFalse();
    }
