...
[TestFixture]
[NonParallelizable]
public sealed class SourceToonParityContractTests
{
    ...
    public void SerializedAssetYamlAndTextureMetadata_ParsesOldModernAndInlineLayouts()
    {
        ...
        SerializedTextureImportDocument texture = SerializedTextureImportDocumentParser.ParseFile(
            Path.Combine(root, "texture-import-modern.meta")).ShouldNotBeNull();
        SerializedMaterialDocument inline = SerializedMaterialDocumentParser.Parse(
            """
            Material:
              m_Name: Inline
    ...
    {
        string importerPath = SourceToonParityCorpusTests.FindRepositoryFile(
    ...
    {
        string managerPath = SourceToonParityCorpusTests.FindRepositoryFile(
    ...
    public void ShaderMatcher_DetectsPinnedUnlockedLockedAndRejectsOtherVersions()
    {
        ...
        SourceToonShaderMatchResult unlocked = SourceToonShaderMatcher.Match(new()
        {
        ...
        SourceToonShaderMatchResult locked = SourceToonShaderMatcher.Match(new()
        {
        ...
        SourceToonShaderMatchResult rejected = SourceToonShaderMatcher.Match(new()
        {
    ...
    {
        ShaderAuthoringSchema first = SourceToonAuthoringSchemaCatalog.GetOrCreate(
            ShaderHelper.UberFragForward().GetUiManifest());
        ShaderAuthoringSchema second = SourceToonAuthoringSchemaCatalog.GetOrCreate(
            ShaderHelper.UberFragForward().GetUiManifest());
