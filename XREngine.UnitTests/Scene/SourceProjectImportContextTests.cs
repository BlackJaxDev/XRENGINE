...
[TestFixture]
[NonParallelizable]
public sealed class SourceProjectImportContextTests
{
    [Test]
    public void ProjectLocator_FindsNearestAssetsAncestorAndSupportsExplicitCorrection()
    {
        ...
        SourceProjectLocation automatic = SourceProjectLocator.Locate(prefabPath);
        ...
        automatic.SourceEditorVersion.ShouldBe("2022.3.22f1");

        string externalPath = sandbox.WriteAsset("External/Avatar.prefab");
        SourceProjectLocator.TryLocate(
            externalPath,
            out SourceProjectLocation? missing,
            out string? error).ShouldBeFalse();
        missing.ShouldBeNull();
        error.ShouldNotBeNull();
        error!.ShouldContain("Assets");

        SourceProjectLocation corrected = SourceProjectLocator.Locate(
            externalPath,
            sandbox.AssetsPath);
    ...
    public void GuidIndex_IndexesAssetsEmbeddedPackagesAndPackageCacheWithStablePrecedence()
    {
        ...
        SourceGuidIndex index = SourceGuidIndex.GetOrCreate(sandbox.RootPath);
        try
        {
            SourceGuidResolution duplicate = index.Resolve(duplicateGuid);
            ...
            string refreshedAsset = sandbox.WriteAssetWithMeta(
                ...
                refreshedGuid);
            SourceGuidIndex.Refresh(sandbox.RootPath);
    ...
    public void DependencyGraph_RecursesReachedClosureRetainsCyclesAndClassifiesOptionalMissingEdges()
    {
        ...
        var context = new SourceProjectImportContext(entryPath);
        SourceDependencyGraph graph = context.DiscoverDependencies();
        ...
        SourceDependencyEdge modelEdge = graph.Nodes[Path.GetFullPath(entryPath)]
        ...
        modelEdge.Kind.ShouldBe(SourceImportDependencyKind.RequiredVisual);
        ...
        graph.UnresolvedEdges.ShouldContain(edge =>
            ...
            edge.Kind == SourceImportDependencyKind.AvatarBehavior);
        ...
        context.Diagnostics.ShouldContain(diagnostic =>
            ...
            diagnostic.Severity == SourceImportDiagnosticSeverity.Warning);
    ...
    public void DependencyGraph_MissingRequiredVisualFailsBeforeConversion()
    {
        ...
        var context = new SourceProjectImportContext(prefabPath);

        Should.Throw<SourceVisualImportException>(() => context.DiscoverDependencies())
