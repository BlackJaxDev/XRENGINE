    ...
    public class XRAssetDeserializer : INodeDeserializer
    {
        ...
        internal static bool TryHandleScalarXRAsset(IParser reader, Type expectedType, Scalar scalar, out object? value)
        {
            ...
            if (TryResolveAssetPathFromScalar(candidate, out var resolvedPath, out var resolution))
            {
                ...
                try
                {
                    value = AssetSerializationServices.Current.LoadImmediate(resolvedPath, expectedType);
                }
                finally
                {
        ...
        private static bool TryRebaseAbsoluteAssetPath(string absolutePath, [NotNullWhen(true)] out string? rebased)
        {
            ...
            string? engineRoot = string.IsNullOrWhiteSpace(AssetSerializationServices.Current.EngineAssetsPath)
                ? null
                : Path.GetFullPath(AssetSerializationServices.Current.EngineAssetsPath);
            string? gameRoot = string.IsNullOrWhiteSpace(AssetSerializationServices.Current.GameAssetsPath)
                ? null
                : Path.GetFullPath(AssetSerializationServices.Current.GameAssetsPath);
        ...
        private static XRAsset? ResolveExternalReference(Guid guid, Type expectedType, string? portableReference = null)
        {
            ...
            if (AssetSerializationServices.Current.TryGetAssetById(guid, out XRAsset? asset) && asset is not null)
                return asset;
            ...
            if (string.IsNullOrWhiteSpace(assetPath))
            {
                ...
                if (!AssetSerializationServices.Current.TryResolveAssetPathById(guid, referenceAssetPath, out assetPath)
            ...
            try
            {
                return AssetSerializationServices.Current.LoadImmediate(assetPath, loadType);
            }
            finally
            {
