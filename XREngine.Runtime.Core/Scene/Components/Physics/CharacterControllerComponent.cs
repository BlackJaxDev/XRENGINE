...
namespace XREngine.Components.Physics
{
    ...
    public class CharacterControllerComponent : XRComponent, IPhysicsReplicationTarget
    {
        ...
        protected override void OnComponentActivated()
        {
            ...
            if (World is not IRuntimePhysicsWorldContext physicsWorld)
                return;

            AbstractPhysicsScene physicsScene = physicsWorld.PhysicsScene;
        ...
        private void QueueControllerCreation(AbstractPhysicsScene physicsScene, Vector3 position)
        {
            ...
            RuntimeThreadServices.Current.EnqueuePhysicsThread(
                () => CreateController(physicsScene, position, version));
        }
        ...
        private void CreateController(AbstractPhysicsScene physicsScene, Vector3 position, int version)
        {
            ...
            if (controller is not null)
                RuntimeThreadServices.Current.EnqueueUpdateThread(
                    () => BindController(physicsScene, controller, version));
        }
            ...
            int version)
        {
            if (version != Volatile.Read(ref _initVersion)
                || !IsActiveInHierarchy
                || World is not IRuntimePhysicsWorldContext physicsWorld
                || !ReferenceEquals(physicsWorld.PhysicsScene, physicsScene))
            {
                controller.RequestRelease();
                return;
            }
