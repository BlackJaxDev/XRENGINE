...
namespace XREngine.Components
{
    ...
    public class NetworkDiscoveryComponent : XRComponent
    {
        ...
        /// </summary>
        public NetworkDiscoveryConnectionSettings? AdvertisedSettings { get; set; }

        /// <summary>
        /// Factory used to create a <see cref="NetworkDiscoveryConnectionSettings"/> when connecting to a discovered host.
        /// </summary>
        public Func<NetworkDiscoveryConnectionSettings>? ClientSettingsFactory { get; set; }
        ...
        /// </summary>
        public Task<object?> ConnectAsync(DiscoveryAnnouncement announcement, CancellationToken cancellationToken = default)
        {
            ...
            void ConnectAction()
            {
                try
                {
                    ...
                    var manager = RuntimeNetworkDiscoveryHostServices.Current.ConfigureNetworking(settings);
            ...
            bool enqueued = RuntimeThreadServices.Current.InvokeOnAppThread(ConnectAction, "NetworkDiscoveryComponent.ConnectToAnnouncement", true);
            if (!enqueued && cancellationToken.IsCancellationRequested)
                tcs.TrySetCanceled(cancellationToken);

            cancellationToken.Register(static state => ((TaskCompletionSource<object?>)state!).TrySetCanceled(), tcs);
            return tcs.Task;
        }
        ...
        /// </summary>
        public Task<object?> StartServerAsync(NetworkDiscoveryConnectionSettings? settings = null, CancellationToken cancellationToken = default)
        {
            var tcs = new TaskCompletionSource<object?>();
            ...
            void StartAction()
            {
                try
                {
                    NetworkDiscoveryConnectionSettings startup = settings ?? AdvertisedSettings ?? new NetworkDiscoveryConnectionSettings();
                    startup.NetworkingType = _advertisedRole;
                    var manager = RuntimeNetworkDiscoveryHostServices.Current.ConfigureNetworking(startup);
                    tcs.TrySetResult(manager);
                }
                catch (Exception ex)
                {
            ...
            bool enqueued = RuntimeThreadServices.Current.InvokeOnAppThread(StartAction, "NetworkDiscoveryComponent.StartServer", true);
            if (!enqueued && cancellationToken.IsCancellationRequested)
                tcs.TrySetCanceled(cancellationToken);
        ...
        private NetworkDiscoveryConnectionSettings BuildSettingsForAnnouncement(DiscoveryAnnouncement announcement)
        {
            NetworkDiscoveryConnectionSettings settings = ClientSettingsFactory?.Invoke() ?? new NetworkDiscoveryConnectionSettings();
            ...
            settings.UdpClientReceivePort = announcement.UdpClientReceivePort;
        ...
        private async Task BroadcastLoopAsync(CancellationToken token)
        {
            ...
            while (!token.IsCancellationRequested)
            {
                try
                {
                    ...
                    byte[] bytes = JsonSerializer.SerializeToUtf8Bytes(payload, NetworkDiscoveryJsonContext.Default.DiscoveryAnnouncement);
        ...
        {
            NetworkDiscoveryConnectionSettings settings = AdvertisedSettings ?? new NetworkDiscoveryConnectionSettings();
            ...
            return new DiscoveryAnnouncement
            {
                ...
                UdpClientReceivePort = settings.UdpClientReceivePort,
        ...
        private static string ResolveLocalHost(string? preferred)
        {
            ...
            foreach (string ip in GetAllLocalIPv4(NetworkInterfaceType.Ethernet))
                if (!string.IsNullOrWhiteSpace(ip))
                    return ip;

            foreach (string ip in GetAllLocalIPv4(NetworkInterfaceType.Wireless80211))
                if (!string.IsNullOrWhiteSpace(ip))
                    return ip;
        ...
        private void HandleDatagram(byte[] buffer, IPEndPoint remote)
        {
            ...
            try
            {
                announcement = JsonSerializer.Deserialize(buffer, NetworkDiscoveryJsonContext.Default.DiscoveryAnnouncement);
            }
            catch (Exception ex)
            {
