...
namespace XREngine.Components.Scene.Transforms
{
    ...
    public class LaggedTranslationTransform(TransformBase? parent, Vector3 translation) : TransformBase(parent)
    {
        ...
        {
            _currentTranslation = Interp.CosineTo(_currentTranslation, _desiredTranslation, (RuntimeTransformServices.Current?.SmoothedDilatedUpdateDeltaSeconds ?? 0.0f), _invTransInterpSec);
            MarkLocalModified();
        }
