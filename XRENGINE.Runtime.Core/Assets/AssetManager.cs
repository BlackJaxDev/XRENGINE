...
namespace XREngine
{
    ...
    public partial class AssetManager
    {
        ...
        private void RunOnJobThreadBlocking(Action action, JobPriority priority = JobPriority.Normal, bool bypassJobThread = false)
            => RunOnJobThreadBlocking(() => { action(); return true; }, priority, bypassJobThread);

        private T RunOnJobThreadBlocking<T>(Func<T> work, JobPriority priority = JobPriority.Normal, bool bypassJobThread = false)
        {
        ...
        private Task RunOnJobThreadAsync(Action action, JobPriority priority = JobPriority.Normal, bool bypassJobThread = false)
            => RunOnJobThreadAsync(() => { action(); return true; }, priority, bypassJobThread);

        private Task<T> RunOnJobThreadAsync<T>(Func<T> work, JobPriority priority = JobPriority.Normal, bool bypassJobThread = false)
        {
            ArgumentNullException.ThrowIfNull(work);
        ...
        public T RunAssetJobBlocking<T>(
            Func<T> work,
            JobPriority priority = JobPriority.Normal,
            bool bypassJobThread = false)
            => RunOnJobThreadBlocking(work, priority, bypassJobThread);

        public Task<T> RunAssetJobAsync<T>(
            Func<T> work,
            JobPriority priority = JobPriority.Normal,
            bool bypassJobThread = false)
        ...
        public void NotifyExternalAssetSaved(XRAsset asset, bool newAsset)
            => PostSaved(asset, newAsset);
