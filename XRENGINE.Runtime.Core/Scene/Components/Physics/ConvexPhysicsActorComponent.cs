...
namespace XREngine.Components.Physics
{
    public abstract class ConvexPhysicsActorComponent : PhysicsActorComponent
    {
        ...
        /// <summary>
        /// Resolves collision triangles through the model/render-owner adapter. Core does
        /// not depend on authoring mesh component implementations.
        /// </summary>
        private protected virtual ConvexHullInputCollection? CollectCollisionInputCollectionForColliders()
        {
            ...
            return provider is not null && provider.TryCollect(this, out ConvexHullInputCollection inputs, out _)
                ? inputs
                : null;
        }
        ...
        {
            if (RuntimePhysicsServices.ConvexHullInputs?.TryCollect(this, out _, out string targetLabel) == true)
                return targetLabel;

            return SceneNode?.Name ?? "<unnamed>";
        }
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                if (!HasCachedHulls(defaultParams) && RuntimePhysicsServices.ConvexHullInputs?.TryCollect(this, out _, out _) == true)
